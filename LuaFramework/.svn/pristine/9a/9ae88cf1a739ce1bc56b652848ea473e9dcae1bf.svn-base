...
PowerUpView = BaseView:New("PowerUpView","PowerUpAtlas")
local this = PowerUpView
...
function GetPowerupGearPos(gear)
...
function GetPowerupGearGo(gear)
    if gear == BuyPowerCardGenre.onegear then
...
function PowerUpView:SetEnterCallback(callback)
    enterFinishCallback = callback
end

function PowerUpView:OpenView(callback,active)
...
function PowerUpView:CloseView(callback)
    Facade.SendNotification(NotifyName.CloseUI,self)
end

function PowerUpView:Awake(obj)
    self:on_init()
end

function PowerUpView:OnEnable(params)
    ...
    fun.set_alpha(self.Img_black, 0)
...
function PowerUpView:BuildFsm()
    self:DisposeFsm()
    self._fsm = Fsm.CreateFsm("PowerUpView",self,{
...
function PowerUpView:DisposeFsm()
    if self._fsm then
        self._fsm:Dispose()
        self._fsm = nil
    end
...
function PowerUpView:OnEnterPowerUp()
    AnimatorPlayHelper.Play(self.anima,"PowerUpView_in",true,0.35,function()
        ...
        fun.set_alpha(self.Img_black, 1)
        self:InitPuadData()
    end,function()
...
function PowerUpView:CheckDiscountAutoBuy(flag)
...
function PowerUpView:Play_PlayEffect()
    local cards = ModelList.CityModel:GetCardsCurrentPage(selfPuLevel)
    --local jokeCards = ModelList.CityModel:GetJokeCardsCurrentPage()
    if #cards > 1 then  -- 如果有小丑卡意味着一定有其他得卡
...
function PowerUpView:InitPowerUpEntity()
...
function PowerUpView:GetPowerUpEntity(parent)
...
function PowerUpView:SetCostInfo()
	local cost1 = 0;
	local cost2 = 0;
	local cost3 = 0;
...
function PowerUpView:RequestServer()

    --分别查询不同的FetchPowerUp
    ModelList.CityModel.C2S_FetchPowerUp(0,selfPuLevel)
end

function PowerUpView:InitPuadData() 
...
function PowerUpView.OnFetchPowerUp_Result()
...
function PowerUpView:SetCurrentPageCards(firstInit)
    local cards = {}
...
function PowerUpView:SetPreviousPageCards()
    self._fsm:GetCurState():TurnPage(self._fsm,1,3)
end

function PowerUpView:ShowPage()
...
function PowerUpView:SetPageBtn()
...
function PowerUpView:OnTurnPage(dir)
    if dir == 1 then
        if ModelList.CityModel:GetPage(selfPuLevel) > 1 then
            local cards = ModelList.CityModel:GetCardsPreviousPage(1)
            for i = 1, #card_entity_cache do
    ...
    elseif dir == 2 then
        if ModelList.CityModel:GetPage(selfPuLevel) < ModelList.CityModel:GetMaxPage() then
            local cards = ModelList.CityModel:GetCardsNextPage(1)
            for i = 1, #card_entity_cache do
...
function PowerUpView:SetNextPageCards()
    self._fsm:GetCurState():TurnPage(self._fsm,2,3)
end

function PowerUpView:OnDrawCard()
...
function PowerUpView:DrawCardDisableBtn(disable)
...
function PowerUpView:ResetPowerIndecator()
...
function PowerUpView:DoTweenPowerIndecator(cards)
    if cards then
        this.OnDoTweenPowerIndecator(cards)
    end
end

function PowerUpView.OnDoTweenPowerIndecator(powerupcard,index,lenth)
    local groupId = ModelList.CityModel:GetCardsGroupId(nil,1)
    if groupId then
        ...
        if powerupcard then
            ...
            if groupId then
                targetValue = ModelList.CityModel:GetCardsPower(nil,1) or Csv.GetData("powerup_group",groupId,"power")
            end
            ...
            function(num)
                if groupId or showStar then
                    local callback = function()
                        log.g("OnDoTweenPowerIndecator Complete_callback callback")
                    ...
                    local quality = ModelList.CityModel:GetCardsLevel(nil,1) or Csv.GetData("powerup_group",groupId,"level")
                    if turnPage then
                        if quality == 4 then
...
function PowerUpView:ShowHandPlayClick()
    self.anima_showHand.speed = 2 --加速播放showhand动画
end

function PowerUpView:DrawCardPlayClick()
    
end

function PowerUpView:OnClickDrawCard(gear)
    ...
    if gear == BuyPowerCardGenre.onegear then
        ...
        self:CheckGemCost(powerup_cost,function()
            ModelList.CityModel.C2S_BuyPowerUpCards(gear,discount_id,discount_useId,1,cardNum) 
        end)
    elseif gear == BuyPowerCardGenre.twogear then
...
function PowerUpView:OnDisable()
...
function PowerUpView:ResetGear()
    if power1gear then
...
function PowerUpView:on_close()

end

function PowerUpView:OnDestroy()
    log.r("PowerUpView:OnDestroy")
...
function PowerUpView:on_btn_right_click()
    self:SetNextPageCards()
end
function PowerUpView:on_btn_left_click()
    self:SetPreviousPageCards()
end

---flag 是否发消息
function PowerUpView.OnPowerUpGearClick(gear)
   

    this._fsm:GetCurState():ClickDrawCard(this._fsm,gear,4)
end

function PowerUpView:CheckGemCost(cost,callback)
...
function PowerUpView:on_btn_play_click()
...
function PowerUpView:onBtnPlayClick()
    this._fsm:GetCurState():Play(this ._fsm,2)
end 

function PowerUpView:SendMsg2Server2EnterGame()
    local mode = ModelList.CityModel:GetEnterGameMode()
...
function PowerUpView:OnPlay()
    local callBack = function()
        self:SendMsg2Server2EnterGame()
        Facade.SendNotification(NotifyName.CloseUI,self)
    end
    local cards = ModelList.CityModel:GetCardsCurrentPage(1)
    cards = #cards
...
function PowerUpView.BuyPowerUpCards_result()
    --_currency:SetDisable(false)
    
    this:ResetPowerIndecator()
    local cards = ModelList.CityModel:GetCardsMaxPage(1)
...
function PowerUpView.OnDrawCardComplete()
    if this._fsm then
...
function PowerUpView:PlayEpicShake()
    --self.btn_epic_anim:Play("show",0,0)
    power3gear:PlayShake()
end

function PowerUpView.OnHintGoback(flag)
...
function PowerUpView:OnClosePu()
    Facade.SendNotification(NotifyName.CloseUI,this)
end

function PowerUpView.OnNoThansk()
    this._fsm:GetCurState():SendMsg2Server2EnterGame(this._fsm)
end

function PowerUpView.OnSuccessBuy(id)
    if this.puad_instance then 
        this.puad_instance:OnSuccessBuy(id)
    end 
end

function PowerUpView.OnRefreshCouponInfo(coupon)
    power1gear:CheckCoupon(true)
    power2gear:CheckCoupon(true)
    power3gear:CheckCoupon(true)
end

function PowerUpView.OnGroupPrefixRefresh()
    this:SetCostInfo()
end 

function PowerUpView:on_btn_buy_buff_click()
    log.log("PowerUpView:on_btn_buy_buff_click")
    if ModelList.CityModel:IsPuBuffSeasonValid() then
        log.log("PowerUpView:on_btn_buy_buff_click ShowPUPack")
        ModelList.GiftPackModel:ShowPUPack()
        self:RecordBuffReaminTimeSnapshoot()
    end
end

function PowerUpView:UpdatePuBuffState()
...
function PowerUpView:ClearBuffLeftTime()
    self.buffRemainTime = 0
    self.text_buff.text = "00:00"
    self:PlayAddBtnAnim()
end

function PowerUpView:SetBuffLeftTime(remainTime)
...
function PowerUpView:ClearBuffCountDown()
...
function PowerUpView:OnBuffTimeOver()
    self:UpdatePuBuffState()
    self:PlayGearDownAnima()
end

function PowerUpView:OnBuffTimeAdd()
    self:UpdatePuBuffState()
...
function PowerUpView:SetBuffSeasonLeftTime(remainTime)
...
function PowerUpView:ClearBuffSeasonCountDown()
...
function PowerUpView:OnBuffSeasonTimeOver()
    self:ClearBuffSeasonCountDown()
    self:UpdatePuBuffState()
end

function PowerUpView:PlayGearDownAnima()
...
function PowerUpView:PlayGearUpAnima()
...
function PowerUpView:PlayAddBtnAnim()
    fun.play_animator(self.img_add, "act", true)
end

function PowerUpView:StopAddBtnAnim()
    fun.play_animator(self.img_add, "idle", true)
end

function PowerUpView.OnPowerUpBuffGifttPackClose(params)
    log.log("PowerUpView.OnPowerUpBuffGifttPackClose(params)", params)
...
function PowerUpView:RecordBuffReaminTimeSnapshoot()
    log.log("PowerUpView:RecordBuffReaminTimeSnapshoot", self.buffRemainTime)
    self.buffReaminTimeSnapshoot = self.buffRemainTime
end

function PowerUpView:SetFromAdventureFlag()
    self.fromAdventureFlag = true
end

function PowerUpView:ClearFromAdventureFlag()
    self.fromAdventureFlag = nil
end

function PowerUpView:TryPopupPuBuffPackView()
    if not ModelList.CityModel:CanCurPlayUsePuBuff() then
        log.log("PowerUpView:TryPopupPuBuffPackView fail (此玩法不可使用此buff)")
        return false
    end

    if not ModelList.CityModel:IsPuBuffSeasonValid() then
        log.log("PowerUpView:TryPopupPuBuffPackView fail (赛季不可用)")
        return false
    end

    if self.fromAdventureFlag then
        log.log("PowerUpView:TryPopupPuBuffPackView fail (from adventrue)")
        return false
    end
    ...
    if ModelList.CityModel:GetPuBuffRemainTime() <= 0 then
        ...
        if ModelList.CityModel:GetIsNeepPopupBuffPurchaseTipState() then
            log.log("PowerUpView:TryPopupPuBuffPackView succ 打开提示")
            ...
            log.log("PowerUpView:TryPopupPuBuffPackView 已经弹过Tip, 尝试直接打开礼包")
            ModelList.GiftPackModel:IsUnPopUpPuPack(false, false, false)
            self:RecordBuffReaminTimeSnapshoot()
            return false
        end
    else
        log.log("PowerUpView:TryPopupPuBuffPackView succ 直接打开")
        ModelList.GiftPackModel:IsUnPopUpPuPack(false, false, false)
        self:RecordBuffReaminTimeSnapshoot()
        return true
    end
...
function PowerUpView:UpdatePuBuffCardIcon()
...
function PowerUpView:UpdateSmallGameState()
...
function PowerUpView:UpdateExtraRewardState()
    local playId = ModelList.CityModel.GetPlayIdByCity()
...
function PowerUpView:StartSmallGameCountDown()
...
function PowerUpView:OnSuperMatchTimeOver()
    self:ClearSmallGameCountDown()
    self:UpdateSmallGameState()
end

function PowerUpView:ClearSmallGameCountDown()
...
---根据当前小游戏类型更新icon
function PowerUpView:UpdateSmallGameIcon(img)
...
this.NotifyList = {
    ...
    {notifyName = NotifyName.PowerUps.Powerup_on_play,func = this.onBtnPlayClick},
