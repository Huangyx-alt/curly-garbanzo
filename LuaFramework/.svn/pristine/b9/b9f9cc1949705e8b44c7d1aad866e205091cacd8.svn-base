local NewChristmasCardSwitchItem = BaseChildView:New()
local this = NewChristmasCardSwitchItem
this.cardPage = {}
this.itemRoots = {}

function NewChristmasCardSwitchItem:New(go)
    self = {}
    setmetatable(self, { __index = NewChristmasCardSwitchItem })
    --self._data = cellData
    return self
end

---初始化switch 分页卡牌
function NewChristmasCardSwitchItem:InitMap(go, type)
    ...
    if ref then
        ...
        self.ItemCtrlTemp_Ver = ref:Get("ItemCtrlTemp_Ver")
        fun.set_active(self.ItemCtrlTemp, false)
        fun.set_active(self.ItemCtrlTemp_Ver, false)
    end
    ...
    for i = 1, 25 do
        ...
        fun.set_gameobject_pos(go, 24 * x, -24.5 * y, 0, true)
...
function NewChristmasCardSwitchItem:ShowAll(go, orderIndex)
    local ref, ItemCtrlTemp, ItemCtrlTemp_Ver = fun.get_component(go.transform, fun.REFER)
    if ref then
        self.content = ref:Get("content")
        self.img_reap = ref:Get("img_reap")
        self.bowl1 = ref:Get("bowl1")
        ...
        ItemCtrlTemp_Ver = ref:Get("ItemCtrlTemp_Ver")
        fun.eachChild(ItemCtrlTemp_Ver.transform.parent, function(child)
            fun.set_active(child, false)
        end)
    end
    ...
    coroutine.start(function()
        ...
        table.each(groups, function(cells, groupID)
            ...
            local cfg = table.find(Csv["item_synthetic"], function(k, v)
                return v.groupid == groupID
            end)
            local part = cfg and cfg.part[1] or 1
            local temp = part == 1 and ItemCtrlTemp or ItemCtrlTemp_Ver
            local itemCtrl = fun.get_instance(temp, temp.transform.parent)
...
function NewChristmasCardSwitchItem:Refresh(pageIndex, newCard, cardId, signInfo, cardObj)
            ...
            for k, v in pairs(newCard) do
                if i == v.index then
                    ...
                    if IsAutoClick(v.cardId, v.index, cardId, signInfo) then
                        BattleEffectCache:GetFreePrefabFromCache("NewChristmasGetxiaoka", items[i].obj, 1, function(effObj)
...
---计算每个卡牌上星星的位置
function NewChristmasCardSwitchItem:InitItemRewardCtrl()
    self.itemGroups = { }
...
function NewChristmasCardSwitchItem:ShowItemRewardCtrl(pageIndex, cardID)
    --标识pageindex是否有显示
    self.pageCtrls = self.pageCtrls or {}
    if GetTableLength(self.pageCtrls[pageIndex]) > 0 then
        table.each(self.pageCtrls[pageIndex], function(ctrl)
            fun.set_active(ctrl, false)
        end)
    end
    ...
    if self.cardIems[cardID] then
        table.each(self.cardIems[cardID], function(ctrl)
        ...
        table.each(groups, function(cells, groupID)
            ...
            local part = cfg and cfg.part[1] or 1
            local temp = part == 1 and self.ItemCtrlTemp or self.ItemCtrlTemp_Ver
            local root = this.itemRoots[pageIndex]
            local itemCtrl = fun.get_instance(temp, root)
            fun.set_active(itemCtrl, true)
            fun.set_gameobject_pos(itemCtrl, pos.x, pos.y, pos.z, false)
            table.insert(self.cardIems[cardID], itemCtrl)
        end)
    end
end

function NewChristmasCardSwitchItem.OnDispose()
    this.cardPage = {}
    this.itemRoots = {}
    this:Close()
end

return NewChristmasCardSwitchItem