...
local GiftPackWorkersDayView = GiftPackBaseView:New('GiftPackWorkersDayView',"GiftPackWorkersDayViewAtlas");
local this = GiftPackWorkersDayView;
...
function GiftPackWorkersDayView:Awake(obj)
...
function GiftPackWorkersDayView:OnEnable()
	--绑定状态机
	self:BuildFsm(self);
	--上报活动界面打开
	self:Bi_Tracker();
...
--function GiftPackWorkersDayView:on_x_update()
--end

function GiftPackWorkersDayView:OnDisable()
	--销毁动态创建的对象
...
function GiftPackWorkersDayView:ClearCountDown()
...
--关闭ui流光
function GiftPackWorkersDayView:CloseUIShiny(obj)
...
function GiftPackWorkersDayView:GetLastClickTime(pos)
	return clickTimeList or 0
end

--显示详细数据
function GiftPackWorkersDayView:ShowDetailGift()
...
function GiftPackWorkersDayView.OnDestroy()
    this:Destroy()
end

function GiftPackWorkersDayView:on_btn_close_click()
	self:ClosePackView()
end

function GiftPackWorkersDayView:on_btn_buy_1_click()
...
function GiftPackWorkersDayView:on_btn_buy_2_click()
	if self._fsm:GetCurName() ~= "GiftPackEnterState" then return end
...
function GiftPackWorkersDayView:on_btn_buy_3_click()
	if self._fsm:GetCurName() ~= "GiftPackEnterState" then return end
...
--设置折扣
function GiftPackWorkersDayView:SetExtra(extraUI,extraParent , bonus)
...
--设置单个道具item
function GiftPackWorkersDayView:SetItemGrid(itemId, itemNum , itemGrid)
	...
	if itemId == Resource.coin or itemId == Resource.diamon or itemId == Resource.rocket then
		...
		if itemData.item_type == ItemType.rofy then
			iconName = Csv.GetItemOrResource(itemId, "more_icon")
...
--显示界面
function GiftPackWorkersDayView:ShowItemGrid()
	...
	self:SetButton(btn_buy_1,product_list[1], itemListPos.Up)
	self:SetButton(self.btn_buy_2,product_list[2], itemListPos.Middle)
	self:SetButton(self.btn_buy_3,product_list[3], itemListPos.Down)

	self:SetLeftTime(data.giftInfo[1].expireTime)
end

--设置购买对象
function GiftPackWorkersDayView:SetItem(itemUI,id, pos, btn)
...
function GiftPackWorkersDayView:GetItemGrid(pos,index,prefab, parent)
...
function GiftPackWorkersDayView:HideGridItem()
	for k ,v in pairs(itemList) do
		for z,w in pairs(v) do
			if w then
...
function GiftPackWorkersDayView:SetButton(button, id,pos)
...
--设置倒计时
function GiftPackWorkersDayView:SetLeftTime(endTimeStamp)
...
--排序子活动
function GiftPackWorkersDayView:CollectProduct(pId,giftInfo)
	product_list = {}
...
function GiftPackWorkersDayView:CloseFunc()
	Facade.SendNotification(NotifyName.HideDialog, self)
end

function GiftPackWorkersDayView:ClosePackView()
	if self._fsm:GetCurName() == "GiftPackShowState" then return end
	self:ClearCountDown()
	Facade.SendNotification(NotifyName.HideUI, ViewList.GiftPackWorkersDayView)
	ModelList.GiftPackModel:CloseView()
end

function  GiftPackWorkersDayView:OnBuySuccess(needClose,itemData)
	self:ChangeState(self,"GiftPackShowState",itemData)
end

function  GiftPackWorkersDayView:GetGiftPos(id)
...
function GiftPackWorkersDayView:GetItemPos(pos)
	local posTable ={}
