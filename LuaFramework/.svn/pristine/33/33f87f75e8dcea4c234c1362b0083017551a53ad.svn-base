...
PowerUpViewSilver = BaseView:New("PowerUpViewSilver","PowerUpAtlas")
...
function GetPowerupGearPos2(gear)
...
function GetPowerupGearGo2(gear)
    if gear == BuyPowerCardGenre.onegear then
...
function PowerUpViewSilver:New(viewName,atlasName)
...
function PowerUpViewSilver:SetEnterCallback(callback)
    enterFinishCallback = callback
end

function PowerUpViewSilver:OpenView(callback,active)
...
function PowerUpViewSilver:CloseView(callback)
    Facade.SendNotification(NotifyName.CloseUI,self)
end

function PowerUpViewSilver:Awake(obj)
    self:on_init()
end

function PowerUpViewSilver:OnEnable(params)
...
function PowerUpViewSilver:BuildFsm()
    self:DisposeFsm()
    self._fsm = Fsm.CreateFsm("PowerUpViewSilver",self,{
...
function PowerUpViewSilver:DisposeFsm()
    if self._fsm then
        self._fsm:Dispose()
        self._fsm = nil
    end
...
function PowerUpViewSilver:OnEnterPowerUp()
...
function PowerUpViewSilver:CheckDiscountAutoBuy(flag)
...
function PowerUpViewSilver:Play_PlayEffect()
    local cards = ModelList.CityModel:GetCardsCurrentPage(2)
    if #cards > 1 then
...
function PowerUpViewSilver:InitPowerUpEntity()
...
function PowerUpViewSilver:GetPowerUpEntity(parent)
...
function PowerUpViewSilver:SetCostInfo()
	local cost1 = 0;
	local cost2 = 0;
	local cost3 = 0;
...
function PowerUpViewSilver:RequestServer()
    ModelList.CityModel.C2S_FetchPowerUp(0,2)
end

function PowerUpViewSilver:InitPuadData() 
...
function PowerUpViewSilver.OnFetchPowerUp_Result()
...
function PowerUpViewSilver:SetCurrentPageCards(firstInit)
    local cards = {}
    if not firstInit then
        ModelList.CityModel:ResetPage(2)
        cards = ModelList.CityModel:GetCardsCurrentPage(2)
    end
    ...
    self.text_page.text = string.format("%s/%s",ModelList.CityModel:GetPage(2),ModelList.CityModel:GetMaxPage())
    self:ShowPage()
...
function PowerUpViewSilver:SetPreviousPageCards()
    self._fsm:GetCurState():TurnPage(self._fsm,1,3)
end

function PowerUpViewSilver:ShowPage()
    if ModelList.CityModel:IsPowerCardEmpty(2) then
...
function PowerUpViewSilver:SetPageBtn()
    fun.set_active(self.btn_left.transform,ModelList.CityModel:GetPage(2) > 1)
    fun.set_active(self.btn_right.transform,ModelList.CityModel:GetPage(2) < ModelList.CityModel:GetMaxPage())
end

function PowerUpViewSilver:OnTurnPage(dir)
    if dir == 1 then
        if ModelList.CityModel:GetPage() > 1 then
            local cards = ModelList.CityModel:GetCardsPreviousPage(2)
            for i = 1, #card_entity_cache do
    ...
    elseif dir == 2 then
        if ModelList.CityModel:GetPage(2) < ModelList.CityModel:GetMaxPage() then
            local cards = ModelList.CityModel:GetCardsNextPage(2)
            for i = 1, #card_entity_cache do
...
function PowerUpViewSilver:SetNextPageCards()
    self._fsm:GetCurState():TurnPage(self._fsm,2,3)
end

function PowerUpViewSilver:OnDrawCard()
...
function PowerUpViewSilver:DrawCardDisableBtn(disable)
...
function PowerUpViewSilver:ResetPowerIndecator()
...
function PowerUpViewSilver:DoTweenPowerIndecator(cards)
    if cards then
        this.OnDoTweenPowerIndecator(cards)
    end
end

function PowerUpViewSilver.OnDoTweenPowerIndecator(powerupcard,index,lenth)
    local groupId = ModelList.CityModel:GetCardsGroupId(nil,2)
    if groupId then
        ...
        if powerupcard then
            ...
            if groupId then
                targetValue = ModelList.CityModel:GetCardsPower(nil,2) or Csv.GetData("powerup_group",groupId,"power")
            end
            ...
            function(num)
                if groupId or showStar then
                    ...
                    local quality = ModelList.CityModel:GetCardsLevel(nil,2) or Csv.GetData("powerup_group",groupId,"level")
                    if turnPage then
                        if quality == 4 then
...
function PowerUpViewSilver:ShowHandPlayClick()
    self.anima_showHand.speed = 2 --加速播放showhand动画
end

function PowerUpViewSilver:DrawCardPlayClick()
    
end

function PowerUpViewSilver:OnClickDrawCard(gear)
    ...
    if gear == BuyPowerCardGenre.onegear then
        ...
        self:CheckGemCost(powerup_cost,function()
            ModelList.CityModel.C2S_BuyPowerUpCards(gear,discount_id,discount_useId,2,cardNum) 
        end)
    elseif gear == BuyPowerCardGenre.twogear then
...
function PowerUpViewSilver:OnDisable()
...
function PowerUpViewSilver:ResetGear()
    if power1gear then
...
function PowerUpViewSilver:on_close()

end

function PowerUpViewSilver:OnDestroy()
...
function PowerUpViewSilver:on_btn_help_click()
    self._fsm:GetCurState():ShowPowerupHelper(self._fsm)
end

function PowerUpViewSilver:OnShowPowerupHelper()
    Facade.SendNotification(NotifyName.ShowUI,ViewList.PowerupHelperView,nil,nil,true)
end

function PowerUpViewSilver:on_btn_close_click()
    self._fsm:GetCurState():CloseView(self._fsm,1)
end

function PowerUpViewSilver:OnClosePu()
    Facade.SendNotification(NotifyName.CloseUI,this)
end

function PowerUpViewSilver:OnCloseView()
...
function PowerUpViewSilver:on_btn_right_click()
    self:SetNextPageCards()
end
function PowerUpViewSilver:on_btn_left_click()
    self:SetPreviousPageCards()
end

---flag 是否发消息
function PowerUpViewSilver.OnPowerUpGearClick(gear)
   

    this._fsm:GetCurState():ClickDrawCard(this._fsm,gear,4)
end

function PowerUpViewSilver:CheckGemCost(cost,callback)
...
function PowerUpViewSilver:on_btn_play_click()
...
function PowerUpViewSilver:SendMsg2Server2EnterGame()
    local mode = ModelList.CityModel:GetEnterGameMode()
...
function PowerUpViewSilver:OnPlay()
    local callBack = function()
...
function PowerUpViewSilver.BuyPowerUpCards_result()
    --_currency:SetDisable(false)
    this:ResetPowerIndecator()
    local cards = ModelList.CityModel:GetCardsMaxPage(2)
...
function PowerUpViewSilver.OnDrawCardComplete()
    if this._fsm then
...
function PowerUpViewSilver:PlayEpicShake()
    --self.btn_epic_anim:Play("show",0,0)
    power3gear:PlayShake()
end

function PowerUpViewSilver.OnHintGoback(flag)
...
function PowerUpViewSilver.OnNoThansk()
    this._fsm:GetCurState():SendMsg2Server2EnterGame(this._fsm)
end

function PowerUpViewSilver.OnSuccessBuy(id)
    if this.puad_instance then 
        this.puad_instance:OnSuccessBuy(id)
    end 
end

function PowerUpViewSilver.OnRefreshCouponInfo(coupon)
    power1gear:CheckCoupon(true)
    power2gear:CheckCoupon(true)
    power3gear:CheckCoupon(true)
end

function PowerUpViewSilver.OnGroupPrefixRefresh()
    this:SetCostInfo()
end 

function PowerUpViewSilver:on_btn_buy_buff_click()
    log.log("PowerUpViewSilver:on_btn_buy_buff_click")
    if ModelList.CityModel:IsPuBuffSeasonValid() then
        log.log("PowerUpViewSilver:on_btn_buy_buff_click ShowPUPack")
        ModelList.GiftPackModel:ShowPUPack()
        self:RecordBuffReaminTimeSnapshoot()
    end
end

function PowerUpViewSilver:UpdatePuBuffState()
...
function PowerUpViewSilver:ClearBuffLeftTime()
    self.buffRemainTime = 0
    self.text_buff.text = "00:00"
    self:PlayAddBtnAnim()
end

function PowerUpViewSilver:SetBuffLeftTime(remainTime)
...
function PowerUpViewSilver:ClearBuffCountDown()
...
function PowerUpViewSilver:OnBuffTimeOver()
    self:UpdatePuBuffState()
    self:PlayGearDownAnima()
end

function PowerUpViewSilver:OnBuffTimeAdd()
    self:UpdatePuBuffState()
...
function PowerUpViewSilver:SetBuffSeasonLeftTime(remainTime)
...
function PowerUpViewSilver:ClearBuffSeasonCountDown()
...
function PowerUpViewSilver:OnBuffSeasonTimeOver()
    self:ClearBuffSeasonCountDown()
    self:UpdatePuBuffState()
end

function PowerUpViewSilver:PlayGearDownAnima()
...
function PowerUpViewSilver:PlayGearUpAnima()
...
function PowerUpViewSilver:PlayAddBtnAnim()
    fun.play_animator(self.img_add, "act", true)
end

function PowerUpViewSilver:StopAddBtnAnim()
    fun.play_animator(self.img_add, "idle", true)
end

function PowerUpViewSilver.OnPowerUpBuffGifttPackClose(params)
    log.log("PowerUpViewSilver.OnPowerUpBuffGifttPackClose(params)", params)
...
function PowerUpViewSilver:RecordBuffReaminTimeSnapshoot()
    log.log("PowerUpViewSilver:RecordBuffReaminTimeSnapshoot", self.buffRemainTime)
    self.buffReaminTimeSnapshoot = self.buffRemainTime
end

function PowerUpViewSilver:SetFromAdventureFlag()
    self.fromAdventureFlag = true
end

function PowerUpViewSilver:ClearFromAdventureFlag()
    self.fromAdventureFlag = nil
end

function PowerUpViewSilver:TryPopupPuBuffPackView()
    if not ModelList.CityModel:CanCurPlayUsePuBuff() then
        log.log("PowerUpViewSilver:TryPopupPuBuffPackView fail (此玩法不可使用此buff)")
        return false
    end

    if not ModelList.CityModel:IsPuBuffSeasonValid() then
        log.log("PowerUpViewSilver:TryPopupPuBuffPackView fail (赛季不可用)")
        return false
    end

    if self.fromAdventureFlag then
        log.log("PowerUpViewSilver:TryPopupPuBuffPackView fail (from adventrue)")
        return false
    end
    ...
    if ModelList.CityModel:GetPuBuffRemainTime() <= 0 then
        ...
        if ModelList.CityModel:GetIsNeepPopupBuffPurchaseTipState() then
            log.log("PowerUpViewSilver:TryPopupPuBuffPackView succ 打开提示")
            ...
            log.log("PowerUpViewSilver:TryPopupPuBuffPackView 已经弹过Tip, 尝试直接打开礼包")
            ModelList.GiftPackModel:IsUnPopUpPuPack(false, false, false)
            self:RecordBuffReaminTimeSnapshoot()
            return false
        end
    else
        log.log("PowerUpViewSilver:TryPopupPuBuffPackView succ 直接打开")
        ModelList.GiftPackModel:IsUnPopUpPuPack(false, false, false)
        self:RecordBuffReaminTimeSnapshoot()
        return true
    end
...
function PowerUpViewSilver:UpdatePuBuffCardIcon()
    local cardId = ModelList.CityModel:GetPuBuffCardId()
    local powerCard = Csv.GetData("powerup_card", cardId)
    log.log("PowerUpViewSilver:UpdatePuBuffCardIcon cardId ", cardId)
...
function PowerUpViewSilver:UpdateSmallGameState()
...
function PowerUpViewSilver:UpdateExtraRewardState()
    local playId = ModelList.CityModel.GetPlayIdByCity()
...
function PowerUpViewSilver:StartSmallGameCountDown()
...
function PowerUpViewSilver:OnSuperMatchTimeOver()
    self:ClearSmallGameCountDown()
    self:UpdateSmallGameState()
end

function PowerUpViewSilver:ClearSmallGameCountDown()
...
---根据当前小游戏类型更新icon
function PowerUpViewSilver:UpdateSmallGameIcon(img)
