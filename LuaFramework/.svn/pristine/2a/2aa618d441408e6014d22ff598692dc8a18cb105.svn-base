...
local LetemRollPuSkill = BaseSkill:New("LetemRollPuSkill")
setmetatable(LetemRollPuSkill, BaseSkill)
local this = LetemRollPuSkill

function LetemRollPuSkill:ShowSkill(cardId, cellIndex, powerId, skillId, serverExtraPos)
...
function LetemRollPuSkill:ShowFly(mapObj, cellObj, cardID, targetCellIndex, powerId, cellIndex)
    local targetCell = this.cardView:GetCardCell(tonumber(cardID), targetCellIndex)
    BattleEffectCache:GetSkillPrefabFromCache("LetemRollskill1", nil, function(obj)
        UISound.play("letemrollmagicbomb")
        ...
        Anim.smooth_move(obj, retPath, flyTime, delayTime, 1, 8, function()
            ...
            BattleEffectPool:Recycle("LetemRollskill1", obj)
        end)
    end, 0, cardID)
end

--控制起始角度和结束角度
function LetemRollPuSkill:GetOffsetDir(startPos, endPos)
...
function LetemRollPuSkill:GetBeizerList(pathNodes, segmentNum)
    local path = {}
...
---@param p2 table 控制点2
---@param p3 table 终点
function LetemRollPuSkill:CalculateCubicBezierPoint(t, p0, p1, p2, p3)
    local arg1 = 1 - t
