...
local GotYouBingoEffect = BaseBingoEffect:New("GotYouBingoEffect")
local this = GotYouBingoEffect
setmetatable(GotYouBingoEffect, BaseBingoEffect)
local private = {}

function GotYouBingoEffect:ShowBingoOrJackpot(bingoData)
    for k, v in pairs(bingoData) do
        local mIndex = k
        log.log("GotYouBingoEffect：ShowBingoOrJackpot mIndex is ", mIndex, v)
        local bingoType, currType = private.GetBingoCount(self, mIndex, v.bingo or 0)
        local bingoCount = #bingoType
        if v.jackpot == 0 or (v.data and v.data.type == 1 and bingoCount >= 3) then
            ...
            if bingoCount < 3 then
                log.log("GotYouBingoEffect：ShowBingoOrJackpot bingo count < 3", bingoCount, mIndex, v)
                self:BingoEffect(mIndex, anima_name, v.first_num, mIndex, bingoType, currType, v.data)
            else
                log.log("GotYouBingoEffect：ShowBingoOrJackpot bingocount >= 3", bingoCount, mIndex, v)
            ...
            if (v.data and v.data.type == 2) then
                log.log("GotYouBingoEffect：ShowBingoOrJackpot 形成jackpot", bingoCount, mIndex, v)
...
function GotYouBingoEffect:ShowBingoOrJackpotV2(bingoData)
    LuaTimer:SetDelayFunction(2, function()
        self:ShowBingoOrJackpotV1(bingoData)
    end, nil, LuaTimer.TimerType.Battle)
end

function GotYouBingoEffect:BingoEffect(mapindex, anima_name, cellIndex, cardid, bingoType, currType, ext)
...
function GotYouBingoEffect:RampageBingoEffect(mapindex, anima_name, cellIndex, cardid, ext)
    local par = self.cardView:GetCardMap(mapindex)
    local pos = par.transform.position
    BattleEffectCache:GetPrefabFromCache("GotYouJackpot", par, function(eff1)
        if eff1 then
            private.PlayBingoAudio(3, ext)
    ...
    self:HideWolf(mapindex)
end

function GotYouBingoEffect:HideWolf(mapindex) 
    local bingoView = ModelList.BattleModel:GetCurrBattleView()
    if bingoView then
        local cardView = bingoView:GetCardView()
        if cardView then
            cardView:HideWolf(mapindex)
        end
    end
end

function GotYouBingoEffect:JackpotEffect(mapindex, anima_name, cellIndex, cardid, ext)
    local par = self.cardView:GetCardMap(mapindex)
    local pos = par.transform.position
...
function GotYouBingoEffect:ShowEffectOnSmallCard(clone, cardId, effectType, anima_name)
...
function GotYouBingoEffect:ChangeSignCellColor(cardId, bowlType)
...
function GotYouBingoEffect:SetBingoShow(bingoType, obj)
...
function GotYouBingoEffect:GetBingoPrefabName()
    if ModelList.BattleModel.GetIsJokerMachine() then
        --return "GotYouBingobingoClown"
    end
    return "GotYouBingobingo"
end

function GotYouBingoEffect:GetBingoAnimaName(bingoCount, ext)
    ...
    if ext and ext.bingoType then
        --anima_name = "bingo0" .. ext.bingoType
        anima_name = "bingo01"
    end
    return anima_name
end
...
function private.PlayBingoAudio(bingoCount, ext)
    if ext and ext.bingoType then
        ...
        UISound.play("gotyoubingo")
    else
        UISound.play("bingo_firework")
        log.log("GotYouBingoEffect PlayBingoAudio bingoCount error", bingoCount)
    end
end
------------------------------------------------------------------------------------------End
return this