...
---野牛踩踏玩法
local HorseRacingModel = BaseGameModel:New()
local this = HorseRacingModel
local private = {}

------需要子类重写，设置参数------------------------------------------------------------------------------
function HorseRacingModel:InitModeOptions()
    self.gameType_ = PLAY_TYPE.PLAY_TYPE_HORSE_RACE
    self.cityPlayID_ = 28
end

function HorseRacingModel:GetQuestItemOffsetPos()
    return 26.5, -32
end

------------------------------------------------------------------------------------------------------

function HorseRacingModel:InitData()
    self:InitModeOptions()
    self:SetSelfGameType(self.gameType_)
end

function HorseRacingModel:ReqEnterGame()
    ...
    local procedure = require "Procedure/ProcedureHorseRacing"
    ...
    self.SendMessage(MSG_ID.MSG_GAME_HORSE_RACE, data)
end

function HorseRacingModel:ReqSignCard(info)
    local cardIds = {}
...
function HorseRacingModel:RefreshRoundDataByIndex(cardid, cellIndex, signType, need_fly_item, mark)
    --父类处理
...
--接受后端返回的对局数据并进行初始化
function HorseRacingModel.ResponeEnterGame(code, data)
    private.OnResEnterGame(this)
    if (code == RET.RET_SUCCESS) then
        log.log("HorseRacingModel.ResponeEnterGame bingoRuleId ", data.bingoRuleId)
...
function HorseRacingModel:ResBingoInfo(code, data)
    if (code == RET.RET_SUCCESS) then
...
function HorseRacingModel:OnReceiveSettle()
    if self:GetGameState() < GameState.ShowSettle then
        self:ShowSettle()
    end
end

function HorseRacingModel:Clear()
...
function HorseRacingModel:IsBingoShowComplete()
...
--- 战斗结算前，检查彩球叫号器
function HorseRacingModel:CheckJokerBallSprayer(callBack)
...
function HorseRacingModel:SetRoundGiftData(card_id, cell_index, new_gift, number, skill_id, self_bingo, powerId, extraPos)
...
function HorseRacingModel:UploadGameData(gameType, quiteType)
...
function HorseRacingModel:PlayBattleOverSound()
    UISound.play("minerbattleover")
end

---提前计算好猪技能的数据，上传到服务器
function HorseRacingModel:PreCalcuatePigSkill()
...
function HorseRacingModel:CheckBingoSkillExtraPosValid(cardId, extraPos)
    local ret, type = {}
...
function HorseRacingModel:GetCurrentCollectLevel()
    local level = self:GetBattleExtraInfo("longStage")
    log.log("HorseRacingModel:GetCurrentCollectLevel the level is ", level)
    level = level and tonumber(level) or 1
    return level
end

function HorseRacingModel:GetCollectLevelMapByCardId(cardId)
    if not cardId then
        log.log("HorseRacingModel:GetCollectLevelMapByCardId card id is nil")
        return {1, 0, 0, 0, 0}
    end
    local itemMaps = self:GetBattleExtraInfo("trackInfo")
    if not itemMaps then
        log.log("HorseRacingModel:GetCollectLevelMapByCardId no trackInfo data")
        return {1, 0, 0, 0, 0}
    end
    ...
    log.log("HorseRacingModel:GetCollectLevelMapByCardId no find trackInfo match cardId:", cardId)
    return {1, 0, 0, 0, 0}
end

function HorseRacingModel:GetCollectLevelByCardIdAndTrackIdx(cardId, idx)
    if not cardId or not idx then
        log.log("HorseRacingModel:GetCollectLevelByCardIdAndTrackIdx Error 1", cardId, idx)
        return 0
    end

    local itemMaps = self:GetCollectLevelMapByCardId(cardId)
    if not itemMaps then
        log.log("HorseRacingModel:GetCollectLevelByCardIdAndTrackIdx Error 2", cardId, idx)
        return 0
    else
        if itemMaps[idx] == nil then
            log.log("HorseRacingModel:GetCollectLevelByCardIdAndTrackIdx Error3 ", cardId, idx)
            return 0
        else
            return itemMaps[idx]
        end
...
function HorseRacingModel:GetCollectiveMaxCount(collectType, collectLevel)
    return 1
end

function HorseRacingModel:CellIndex2Col(idx)
...
this.BaseMsgIdList = {
    { msgid = MSG_ID.MSG_GAME_HORSE_RACE, func = this.ResponeEnterGame },
}
