...
local GiftPackSweetmeatsView = GiftPackBaseView:New('GiftPackSweetmeatsView', "GiftPackJunkiessAtlas")
local this = GiftPackSweetmeatsView
...
function GiftPackSweetmeatsView:Awake(obj)
    self:on_init()
...
function GiftPackSweetmeatsView:OnEnable()
    Facade.RegisterView(self)
 --   this:ClearMailList()
    self:BuildFsm(self)
    self:Bi_Tracker()
...
function GiftPackSweetmeatsView:OnDisable()
...
function GiftPackSweetmeatsView:ClearCountDown()
...
function GiftPackSweetmeatsView:ShowItemGrid()
...
function GiftPackSweetmeatsView:ShowItemGridData(step,flag)
    _data = ModelList.GiftPackModel:GetShowGiftPack()
...
function GiftPackSweetmeatsView:movetoGrid(step,tflag)
    step = step or 1
    local rect = fun.get_component(self.item,fun.RECT)
    local y = step-1<=0 and 50 or  (((step-1) * rect.rect.height) +130)
...
function GiftPackSweetmeatsView:ClearMailList()
...
function GiftPackSweetmeatsView:GetItemViewInstance(index)
...
function GiftPackSweetmeatsView:ShowDetailGift()
...
function GiftPackSweetmeatsView:SetLeftTime(endTimeStamp)
...
function GiftPackSweetmeatsView:OnDestroy()
    log.g("GiftPackSweetmeatsView.OnDestroy ing !")
    Facade.RemoveView(self)
    self:Destroy()
    self:ClearMailList()
end

function GiftPackSweetmeatsView:CloseFunc()
    Facade.SendNotification(NotifyName.HideUI, this)
end

function GiftPackSweetmeatsView:ClosePackView()
    ...
    self:ClearCountDown()
    Facade.SendNotification(NotifyName.HideUI, ViewList.GiftPackSweetmeatsView)
    ModelList.GiftPackModel:CloseView()
end

function GiftPackSweetmeatsView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackSweetmeatsView:OnBuySuccess(needClose,itemData)
...
function GiftPackSweetmeatsView:showGetRewardEffect(itemData,isFree)
...
function GiftPackSweetmeatsView:SetTimer()
...
--领取成功
function GiftPackSweetmeatsView.OnClaimEndlessGiftSuccess(data)
...
--领取失败
function GiftPackSweetmeatsView.OnClaimEndlessGiftFail(data)
    --提示下领取失败
end 

--查询成功
function GiftPackSweetmeatsView.OnFetchEndlessGiftSuccess(data)
    selfstep = data.step
    local tmpflag = false 
...
--领取失败
function GiftPackSweetmeatsView.OnFetchEndlessGiftFail(data)
    --提示下查询失败
end 
