...
---@class ChristmasSynthesisGameCardView : GameCardView
local ChristmasSynthesisGameCardView = GameCardView:New("ChristmasSynthesisGameCardView")
local this = ChristmasSynthesisGameCardView
...
function this:GmSignTestV0()
    ...
    if jackpotRuleId then
        log.log("ChristmasSynthesisGameCardView:GmSignTest 一键达成jackpot succ", cardCount, startCardId)
        ...
        log.log("ChristmasSynthesisGameCardView:GmSignTest 一键达成jackpot fail no jackpotRuleId", cardCount, loadData)
    end
end
...
function this:CellSignChange(cardId, cellIndex)
    cardId = tonumber(cardId)
    self:GetCard(cardId):CellSignChange(cardId, cellIndex)
    --CalculateBingoMachine.OnDataChange(cardId, cellIndex)
    --CalculateBingoMachine.CalcuateBingo(cardId, cellIndex)
end

---检查所有卡处于idle状态
function this:CheckAllCardIdle()
    for i = 1, self.model:GetCardCount() do
        local card = self:GetCard(i)
        if card and not  card:IsIdle()  then
            return false
        end
    end
    return true
...
function this:ExtraBallClickCard(cardid, cellIndex)
    ...
    ModelList.ChristmasSynthesisModel:SignCardWithExtraBall(cardIdNum, cellIndex)
end
