...
local PiggyBankLogicCard = BaseLogicCard:New("PiggyBankLogicCard")
local this = PiggyBankLogicCard
this.moduleName = "PiggyBankLogicCard"
local bowlTable = {}
local MaxCollectCount = {}

function PiggyBankLogicCard:InitLogic()
    self.model = ModelList.BattleModel:GetCurrModel()
    self:InitCardBowls()
    self:BaseInit()
end

function PiggyBankLogicCard:InitCardBowls()
...
function PiggyBankLogicCard:GetBingoType(cardId, bingoCount)
...
function PiggyBankLogicCard:IsOtherSideHaveOneJackpot(currCardId)
...
function PiggyBankLogicCard:IsTypeBingo(cardID, type)
    cardID = tonumber(cardID)
    local temp = bowlTable[cardID][type] or 0
    return temp >= MaxCollectCount[type]
end

function PiggyBankLogicCard:IsAllJackpot()
    local cardCount = self.model:GetCardCount()
    for i = 1, cardCount do
...
function PiggyBankLogicCard:CheckAllCardJackpotHandle(currCardId)
...
function PiggyBankLogicCard:CheckDoubleJackpotHandle(currCardId)
    if ModelList.BattleModel:IsRocket() then
        return
    end
...
function PiggyBankLogicCard:StopCardBackMove()
    if this.loopMoveCard then
        LuaTimer:Remove(this.loopMoveCard)
    end
end

function PiggyBankLogicCard:GetBowlCount(cardId)
    cardId = tonumber(cardId)
    for i = 1, #bowlTable[cardId] do
...
function PiggyBankLogicCard:GetMaxCount(cardId)
    return MaxCollectCount
end

function PiggyBankLogicCard:GetBowlBingoCount(cardId)
    cardId = tonumber(cardId)
    local bingoCount = 0
    local isValid = false
...
function PiggyBankLogicCard:GetFasterBowlType(cardId, excludeTypes)
    cardId = tonumber(cardId)
    local ret, temp
...
function PiggyBankLogicCard:GetSkillNeedCells(cardId, excludeTypes)
...
function PiggyBankLogicCard:GetCellsByMaterialType(cardId, type)
    cardId = tonumber(cardId)
