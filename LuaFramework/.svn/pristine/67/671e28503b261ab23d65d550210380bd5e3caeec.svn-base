...
local NewChristmasBingoEffect = BaseBingoEffect:New("NewChristmasBingoEffect")
local this = NewChristmasBingoEffect
setmetatable(NewChristmasBingoEffect, BaseBingoEffect)
local private = {}

function NewChristmasBingoEffect:ShowBingoOrJackpot(bingoData)
...
function NewChristmasBingoEffect:BingoEffect(mapindex, anima_name, cellIndex, cardid, bingoType)
...
function NewChristmasBingoEffect:JackpotEffect(mapindex, anima_name, cellIndex, cardid)
    local par = self.cardView:GetCardMap(mapindex)
    local pos =fun.get_gameobject_pos( par)
    ...
    if eff1 then
        ...
        UISound.play("newchristmasjackpot")
        BattleEffectCache:AddBindEffect(eff1, par, cardid)
        fun.set_gameobject_pos(eff1, pos.x, pos.y, pos.z, false)
...
function NewChristmasBingoEffect:RampageBingoEffect(mapindex, anima_name, cellIndex, cardid)
    local par = self.cardView:GetCardMap(mapindex)
    local pos = fun.get_gameobject_pos(par)

    local eff1 = GoPool.pop("NewChristmasJackpot")
    if eff1 then
        ...
        fun.set_gameobject_pos(eff1, pos.x, pos.y, pos.z, false)
        fun.set_rect_offset_local_pos(eff1, -10, -25)
    ...
    self:ShowEffectOnSmallCard("NewChristmasJackpot", mapindex, 2)
end

function NewChristmasBingoEffect:ShowEffectOnSmallCard(pollName, cardId, effectType, anima_name)
...
function private.PlayBingoAudio(bingoCount)
	if bingoCount == 1 then
		...
		UISound.play("newchristmasbingo")
	elseif bingoCount == 2 then
		UISound.play("bingo_firework")
		UISound.play("newchristmasdoubleBingo")
	elseif bingoCount == 3 then
		UISound.play("bingo_firework")
		UISound.play("newchristmastripleBingo")
	elseif bingoCount == 4 then
		UISound.play("bingo_firework")
		UISound.play("newchristmasquadraBingo")	
    elseif bingoCount == 5 then
		UISound.play("bingo_firework")
		UISound.play("newchristmasbingoRampage")
	end
end
