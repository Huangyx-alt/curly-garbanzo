...
--- Created by PC.
--- DateTime: 2021/9/7 10:31
---

local JackpotEnableViewCookie = BaseDialogView:New("JackpotEnableViewCookie","JackpotEnableAtlas")
local this = JackpotEnableViewCookie
...
function JackpotEnableViewCookie:Awake(obj)
    self:on_init()
...
function JackpotEnableViewCookie:OnEnable()
  ...
  self:SetPt02Texture()
    ...
    if self.bet_Item[betrate]  then 
        AnimatorPlayHelper.SetAnimationEvent(self.bet_Item[betrate],"ef_Hall_Jackpotcookie",function()
            Facade.SendNotification(NotifyName.CloseUI,ViewList.JackpotEnableViewCookie)
        end)
...
--- 设置pt01粒子系统的材质球贴图
function JackpotEnableViewCookie:SetPt02Texture(obj)
    --- 如果有赛车活动，就使用赛车的图片
    if ModelList.CarQuestModel:IsActivityAvailable() then
        self.pt02.sharedMaterials[0].mainTexture = self.texture2.mainTexture
        elseif ModelList.CompetitionModel:IsActivityAvailable() then
        self.pt02.sharedMaterials[0].mainTexture  = self.texture1.mainTexture
    end
end


function JackpotEnableViewCookie:on_close()

end

function JackpotEnableViewCookie:OnDisable()
    Event.Brocast(EventName.Event_jackpoview_hide)
end

function JackpotEnableViewCookie:OnDestroy()
    this:Close()
end

--- 当前模式下 jackpot的展示
function JackpotEnableViewCookie:SetJackpotIcon(betrate)
