...
local CollectDrinksBingoRule = BaseBingoRule:New("CollectDrinksBingoRule")
local this = CollectDrinksBingoRule
--setmetatable(CollectDrinksBingoRule, {__index = BaseBingoRule})
...
function CollectDrinksBingoRule:Start(loadData,wish_item)
    bowlCount = {}
    wishItems = {}
    currWishList = {}
    if wish_item then
    ...
    this:InitData()
    MaxWaterCount = {
        Csv.GetCollectiveMaxCount(1)
        ,Csv.GetCollectiveMaxCount(2)
        ,Csv.GetCollectiveMaxCount(3)
        ,Csv.GetCollectiveMaxCount(4)}
    this.model = ModelList.BattleModel:GetCurrModel()
end

function CollectDrinksBingoRule:CalculateBingo(cardId,cellIndex,...)
...
function CollectDrinksBingoRule:OnDataChange(cardId,cellIndex,...)
...
function CollectDrinksBingoRule:CalculateJackpot(cardId)
...
function CollectDrinksBingoRule:ForceBingo()
    local max = 0
    local type = 0
    for i = 1, #bowlCount do
...
--- 计算是否出现wish,
function CollectDrinksBingoRule:CalculateCardWish(cardId)
