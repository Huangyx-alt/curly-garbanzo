...
---  绿头人玩法
local LeetoleManModel = BaseGameModel:New()
local this = LeetoleManModel
local private = {}

------需要子类重写，设置参数------------------------------------------------------------------------------
function LeetoleManModel:InitModeOptions()
    self.gameType_ = PLAY_TYPE.PLAY_TYPE_GREEN_HEAD
    self.cityPlayID_ = 7
end

function LeetoleManModel:GetQuestItemOffsetPos()
    return 28, -28
end

------------------------------------------------------------------------------------------------------

function LeetoleManModel:InitData()
    self:InitModeOptions()
    self:SetSelfGameType(self.gameType_)
end

function LeetoleManModel:ReqEnterGame()
    ...
    local procedure = require "Procedure/ProcedureLeeToleMan"
    ...
    this.SendMessage(MSG_ID.MSG_GAME_LOAD_GREEN_HEAD, {cardNum = cardNum,rate = betRate,powerUpId = tonumber(powerupId),
                                                    cityId = city,couponId = coupon,useCouponId = useId,playId = ModelList.CityModel.GetPlayIdByCity(city),
                                                     hideCardAuto=hideCardAuto});
end

function LeetoleManModel:ReqSignCard(info)
    local cardIds = {}
...
function LeetoleManModel:RefreshRoundDataByIndex(cardid, cellIndex, signType, need_fly_item, mark)
    --父类处理
    this.__index:RefreshRoundDataByIndex(cardid, cellIndex, signType, need_fly_item, mark)
...
function LeetoleManModel.ResponeEnterGame(code,data)
...
function LeetoleManModel:ResBingoInfo(code,data)
    if(code == RET.RET_SUCCESS)then
...
function LeetoleManModel:OnReceiveSettle()
    if self:GetGameState() < GameState.ShowSettle then
        self:ShowSettle()
    end
end

function LeetoleManModel:Clear()
...
function LeetoleManModel:IsBingoShowComplete()
    local check = this.__index:IsBingoShowComplete()
    
    --cell被盖章后飞出道具及做动画的时间
    local checkTime = os.time() - self.cardSignLogicDelayTime > 4
...
--- 战斗结算前，检查彩球叫号器
function LeetoleManModel:CheckJokerBallSprayer(callBack)
...
function LeetoleManModel:SetRoundGiftData(card_id,cell_index,new_gift,number,skill_id, self_bingo,powerId,extraPos)
    this.__index:SetRoundGiftData(card_id,cell_index,new_gift,number,skill_id, self_bingo,powerId,extraPos)
    if powerId == 136 then
        --记录猪技能数据，结束上传
        table.insert(self.flyPigSkillDataCache, {
...
function LeetoleManModel:UploadGameData(gameType, quiteType)
...
---提前计算好猪技能的数据，上传到服务器
function LeetoleManModel:PreCalcuatePigSkill()
    local typeFlag = {}  --标识哪些类型已经被技能使用
    table.each(self.flyPigSkillDataCache, function(v)
        ...
        local cells, type = self:CheckFlyPigExtraPosValid(v.cardId, v.extraPos)
...
function LeetoleManModel:CheckFlyPigExtraPosValid(cardId, extraPos)
    local ret, type = {}
...
this.BaseMsgIdList = 
{
    {msgid = MSG_ID.MSG_GAME_LOAD_GREEN_HEAD, func = this.ResponeEnterGame},
}
