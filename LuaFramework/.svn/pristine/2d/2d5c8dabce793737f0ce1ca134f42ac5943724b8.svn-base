local PiggyBankJackpotView = BaseChildView:New()
local this = PiggyBankJackpotView
...
function PiggyBankJackpotView:New()
    local o = {}
    this.__index = this
    setmetatable(o, this)
    return o
...
function PiggyBankJackpotView:Init(jackpotRef, jackpotRuleId, jackPar)
...
function PiggyBankJackpotView:OutScreenOpen()
...
function PiggyBankJackpotView:Show()
    if this.isShowned then   return   end
    this.isShowned = true
    fun.set_active(this.jackpotRoot, false)
end

function PiggyBankJackpotView:OnDisable()
    this.jackpotList = nil
    this.itemList = {}
