...
---  宝石皇后玩法
local GoldenPigModel = BaseGameModel:New()
local this = GoldenPigModel
local private = {}

------需要子类重写，设置参数------------------------------------------------------------------------------
function GoldenPigModel:InitModeOptions()
    self.gameType_ = PLAY_TYPE.PLAY_TYPE_GOLDEN_PIGGY
    self.cityPlayID_ = 18
end

function GoldenPigModel:GetQuestItemOffsetPos()
    return 29, -29
end

------------------------------------------------------------------------------------------------------

function GoldenPigModel:InitData()
    self:InitModeOptions()
    self:SetSelfGameType(self.gameType_)
end

function GoldenPigModel:ReqEnterGame()
    ...
    local procedure = require "Procedure/ProcedureGoldenPig"
    ...
    self.SendMessage(MSG_ID.MSG_GAME_LOAD_GOLDEN_PIGGY, data)
end

function GoldenPigModel:ReqSignCard(info)
    local cardIds = {}
...
function GoldenPigModel:RefreshRoundDataByIndex(cardid, cellIndex, signType, need_fly_item, mark)
    --父类处理
...
function GoldenPigModel.ResponeEnterGame(code, data)
...
function GoldenPigModel:ResBingoInfo(code, data)
    if (code == RET.RET_SUCCESS) then
...
function GoldenPigModel:OnReceiveSettle()
    if self:GetGameState() < GameState.ShowSettle then
        self:ShowSettle()
    end
end

function GoldenPigModel:Clear()
...
function GoldenPigModel:IsBingoShowComplete()
...
--- 战斗结算前，检查彩球叫号器
function GoldenPigModel:CheckJokerBallSprayer(callBack)
...
function GoldenPigModel:SetRoundGiftData(card_id, cell_index, new_gift, number, skill_id, self_bingo, powerId, extraPos)
...
function GoldenPigModel:UploadGameData(gameType, quiteType)
...
---提前计算好猪技能的数据，上传到服务器
function GoldenPigModel:PreCalcuatePigSkill()
...
function GoldenPigModel:CheckBingoSkillExtraPosValid(cardId, extraPos)
    local ret, type = {}
...
this.BaseMsgIdList = {
    { msgid = MSG_ID.MSG_GAME_LOAD_GOLDEN_PIGGY, func = this.ResponeEnterGame },
}
