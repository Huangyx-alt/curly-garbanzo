...
--- DateTime: 2025/2/10 17:39
---

local SGMoneyBuildingMainView = TopBarControllerView:New('SGMoneyBuildingMainView', 'SGMoneyBuildingMainAtlas')
local Private = {}
local this = SGMoneyBuildingMainView
this.viewType = CanvasSortingOrderManager.LayerType.Machine_Dialog
this.auto_bind_ui_items = {
    "SGMoneyBuildingMainView",
    "BuildIcon",
    "btn_decreate",
    "btn_increate",
    --"btn_maxbet",
...
function SGMoneyBuildingMainView:Awake()
    self:on_init()
...
function SGMoneyBuildingMainView:OnEnable(params)
...
function SGMoneyBuildingMainView:BuildFsm()
    FsmCreator:Create("SGMoneyBuildingMainView", self, {
        require("View.Bingo.SettleModule.State.SmallGame.Normal.InitState"),
        require("View.Bingo.SettleModule.State.SmallGame.Normal.IdleState"),
        require("View.Bingo.SettleModule.State.SmallGame.Normal.ChangeBetState"),
        require("View.Bingo.SettleModule.State.SmallGame.Normal.SpinState"),
        require("View.Bingo.SettleModule.State.SmallGame.Normal.ShowSpinState"),
        require("View.Bingo.SettleModule.State.SmallGame.Normal.ExitState"),
        require("View.Bingo.SettleModule.State.SmallGame.Normal.ShowSpinRewardState"),
        require("View.Bingo.SettleModule.State.SmallGame.Normal.ShowBingoState"),
        require("View.Bingo.SettleModule.State.SmallGame.Normal.GuideState"),
    })
    self.state = {
        InitState = "InitState",
        IdleState = "IdleState",
...
--function SGMoneyBuildingMainView:GetNodeList()
--    return { self.Node1, self.Node2, self.Node3, self.Node4 }
--end


--- 初始化
function SGMoneyBuildingMainView:Init()
    --self.freeTimes = 3
...
---设置背景,白天和黑夜
function SGMoneyBuildingMainView:SetBg()
    if self.freeTimes == 0 then
...
--设置建筑icon 白天和黑夜
function SGMoneyBuildingMainView:SetBuildingIcon()
    if self.freeTimes == 0 then
...
-- 设置所有建筑格子的数据
function SGMoneyBuildingMainView:SetBuildingData(isUltrabet)
    self.Cells = {}
    self.RewardInfo = {}
    self.RewardInfo.rewardNum = {}
    local cellItem = require("View.Bingo.SettleModule.UIView.SmallGame.Normal.SGMoneyBuildingCellItem")
    local position_reward = Csv.GetData("minigame_moneymansion", self.currBet, "position_mes")
    for i = 1, 24 do
...
---设置初始bet数据
function SGMoneyBuildingMainView:SetInitMansionInfo()
    self.MansionInfo = ModelList.GameModel:GetSmallGameData2()
end

---设置初始bet数据
function SGMoneyBuildingMainView:SetInitBetData()
...
function SGMoneyBuildingMainView:ChangeBet(changeType, targetBet)
    local bet = -1
...
--设置Bet数据
function SGMoneyBuildingMainView:SetBetData()
...
---设置免费次数
function SGMoneyBuildingMainView:SetFreeTimes(initFree)
    if self.freeGameText then
        self.freeGameText.text = initFree or self.freeTimes
    end
    fun.set_active(self.PigGameZhe, true)
...
---改变免费次数
function SGMoneyBuildingMainView:ChangeFreeTimes(num)
    self.freeTimes = num
    self:SetFreeTimes()
end

---设置总奖励数值
function SGMoneyBuildingMainView:SetTotalWinNum(num)
...
function SGMoneyBuildingMainView:SaveTotalWinNumByIndex(rewardIndex)
...
---记录总奖励数值
---@param rewardIndex number 命中的格子序号
function SGMoneyBuildingMainView:SaveTotalWinNumByValue(value)
    if not self.TotalWinNum then self.TotalWinNum = 0 end
    self.TotalWinNum = self.TotalWinNum + value
end

function SGMoneyBuildingMainView:Exit()
    ViewList.GameSettleView:OpenRound4()
end

--- 进入等待摇奖结束的状态
function SGMoneyBuildingMainView:SetWaitState(isWait)
...
--- 所有球摇完，展示继续按钮
function SGMoneyBuildingMainView:CheckOver()
...
--- 检查是否等待数据时间过长
function SGMoneyBuildingMainView:GetSettleData()
    this.HadSettle = true
end

---游戏不存在的处理
function SGMoneyBuildingMainView:GameNotExist()
    if this.HadSettle then
        ViewList.GameSettleView:JumpNextView()
        Facade.SendNotification(NotifyName.HideUI, ViewList.SGMoneyBuildingMainView)
        --ViewList.SGMoneyBuildingMainView = nil
    else
        Facade.SendNotification(NotifyName.HideUI, ViewList.SGMoneyBuildingMainView)
...
---游戏不存在的处理
function SGMoneyBuildingMainView:OnCoinChange()
    if this.isOpenShop then
        this:UpdateGoldValue(false)
    end
end

function SGMoneyBuildingMainView:RegisterEvent()
...
---
function SGMoneyBuildingMainView:UnRegisterEvent()
...
function SGMoneyBuildingMainView:OnTimeOut()
...
---每次摇奖后，重置时间
function SGMoneyBuildingMainView:ResetTimeOut()
...
function SGMoneyBuildingMainView:on_btn_decreate_click()
...
function SGMoneyBuildingMainView:on_btn_increate_click()
...
function SGMoneyBuildingMainView:on_btn_maxbet_click()
    if self.freeTimes > 0 then
        return
    end
...
function SGMoneyBuildingMainView:on_btn_spin_click()
    self._fsm:GetCurState():OnSpin()
    self.btn_spin:Play("act")
end

--- 检查是否足够钻石触发spin
function SGMoneyBuildingMainView:CheckSpinCost()
...
function SGMoneyBuildingMainView:on_btn_help_click()
    --local view = require("View.HallCity.SmallGame.SGSuperMatchHelperView")
    local view = require("View.Bingo.SettleModule.UIView.SmallGame.Normal.SGMoneyBuildingInnerHelperView")
    Facade.SendNotification(NotifyName.ShowUI, view)
end

function SGMoneyBuildingMainView:PlayCellActEffect(index, isIgnoreSound)
...
function SGMoneyBuildingMainView:PlayCellActSlowEffect(index, isIgnoreSound)
    if this.isOpen and self["Cell" .. index] then
...
---播放命中格子效果
function SGMoneyBuildingMainView:PlayCellHitEffect(index)
...
---播放完中奖效果后的回调
function SGMoneyBuildingMainView:PlayHitEffectOver()
...
-----播放锤子领奖效果
--function SGMoneyBuildingMainView:PlayHammerEffect()
--end

---播放锤子领奖效果
function SGMoneyBuildingMainView:PlayHammerHitEffect(index)
end

---播放刷子刷新奖励效果
function SGMoneyBuildingMainView:PlayBrushEffect()
    Private.PlayBrushEffect(self)
end

---@return number 当前的spin奖励信息
function SGMoneyBuildingMainView:GetSpinReward()
    return self.spinReward
end

function SGMoneyBuildingMainView:RefreshMansionInfo(data)
    self.MansionInfo = data
end

function SGMoneyBuildingMainView:CloseTopBarParent()
    --self:CloseView()
    if this.isOpen then
        this.isOpen = false
        this:CloseAnimation()
...
function SGMoneyBuildingMainView:CloseReward()
...
function SGMoneyBuildingMainView:CloseEffect()
    ---暂时手动修改层级
    fun.set_sorting_order_relative(self.go, 20000)
    AnimatorPlayHelper.Play(self.SGMoneyBuildingMainView, { "end", "GotYouSmallGameViewend" }, false, function ()
...
function SGMoneyBuildingMainView:Jump2puzzle()
    Facade.SendNotification(NotifyName.ShowUI, ViewList.PuzzleView, function ()
        ...
        ModelList.GameModel:ClearSGData()
        Event.Brocast(Notes.QUIT_BATTLE)
...
---获取当前bet
function SGMoneyBuildingMainView:GetCurrBet()
    return self.currBet
end

---收到spin奖励
function SGMoneyBuildingMainView:OnSpinReward(data)
...
function SGMoneyBuildingMainView:OnDisable()
    this.isOpen = false
...
function SGMoneyBuildingMainView:CloseAnimation()
    --- 结束的时候恢复原始速度
    if this.oriTimeScale then
        Time.timeScale = this.oriTimeScale
        this.oriTimeScale = nil
...
--播放界面退场动画
function SGMoneyBuildingMainView:ForceExit()
    --if this._exit_callback then
    --    this._exit_callback()
    --    this._exit_callback = nil
    --end
    if ViewList.SGMoneyBuildingMainView then
        Facade.SendNotification(NotifyName.CloseUI, ViewList.SGMoneyBuildingMainView)
        ViewList.SGMoneyBuildingMainView = nil
    end
    --- 结束的时候恢复原始速度
    if this.oriTimeScale then
...
function SGMoneyBuildingMainView:SetBingoCount(freeTime)
...
function SGMoneyBuildingMainView:ShowGuide(guideStep)
    if guideStep == 1 then
        self.GuideText.text = Csv.GetDescription(191079)
        self.GuideBg:Get("Ban").sprite = AtlasManager:GetSpriteByName("SGMoneyBuildingGuideAtlas", "xKuang")
        self.GuideBg:Get("headBg").sprite = AtlasManager:GetSpriteByName("SGMoneyBuildingGuideAtlas", "xYuanDi")
        self.GuideBg:Get("head").sprite = AtlasManager:GetSpriteByName("SGMoneyBuildingGuideAtlas", "xJueSe")
        ...
        self.GuideText.text = Csv.GetDescription(191080)
        if self.PigGameCZFreeDi then
            fun.set_active(self.Guideyindaosuo, true)
            AnimatorPlayHelper.Play(self.PigGameCZFreeDi, { "yindao", "PigGameCZFreeDiyindao" }, false, function ()
                ---BINGOxN出现，飞到Freegame位置,锁上
                self._fsm:ChangeState("IdleState")
                ModelList.GuideModel.ReqFinishGuide(91, -1)
                --Done LwangZg 资源卸载
                --- 卸载引导资源
                -- resMgr:UnloadAssetBundle("SGMoneyBuildingGuideAtlas", true)
                ResourcesManager:UnloadAsset("SGMoneyBuildingGuideAtlas", true)
            end)
        end
    end
end
...
---播放全部格子凸出来效果
function SGMoneyBuildingMainView:AllCellUpEffect()
    for i = 1, 24 do
        if self["Cell" .. i] then
...
---判断是否spin按钮置灰
function SGMoneyBuildingMainView:CheckSpinBtnGray(isGray)
    Util.SetUIImageGray(self.btn_spin.gameObject, isGray)
end

---判断是否spin按钮置灰
function SGMoneyBuildingMainView:SetOpenState()
    this.isOpen = true
end
...
function Private.CheckPopHelpView(self)
    if not ModelList.BattleModel:IsGameing() then
        ...
        local defaultState = fun.read_value("SGMoneyBuildingHelperView" .. playerInfo.uid, 0)
        if defaultState == 0 then
            local view = require("View.HallCity.SmallGame.SGMoneyBuildingHelperView")
            --Facade.SendNotification(NotifyName.ShowUI,ViewList.SGMoneyBuildingMainView,nil,nil,2)
            Facade.SendNotification(NotifyName.ShowUI, view)
        end
    end
end
...
function Private.InitBetBtnType(self)
    -- if self.currBet < 5 then
    --     self.maxbetImg.sprite = AtlasManager:GetSpriteByName("SGMoneyBuildingMainAtlas", "PigGameZ04")
    -- elseif self.currBet == 5 then
    --     self.maxbetImg.sprite = AtlasManager:GetSpriteByName("SGMoneyBuildingMainAtlas", "PigGameZ09")
    -- elseif self.currBet >= 6 then
    --     self.maxbetImg.sprite = AtlasManager:GetSpriteByName("SGMoneyBuildingMainAtlas", "PigGameZ08")
    -- end
...
function Private.PlayBrushFlyEffect(self, cellIndex)
    self.MansionSpinInfo = ModelList.GameModel:GetExtraSmallGameData()
...
function SGMoneyBuildingMainView:PlaySpinIdelEffect()
    for i = 1, 24 do
...
function SGMoneyBuildingMainView:OnShopViewClose()
    if self and self._fsm then
        self._fsm:GetCurState():OnResumeIdle()
    end
end

function SGMoneyBuildingMainView:OnNotifyOpenShopView()
