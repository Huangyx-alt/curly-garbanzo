...
local GiftPackNewLeetoleManWView = GiftPackBaseView:New('GiftPackNewLeetoleManWView')
local this = GiftPackNewLeetoleManWView
...
function GiftPackNewLeetoleManWView:Awake(obj)
    self:on_init()
...
function GiftPackNewLeetoleManWView:OnEnable()
    Facade.RegisterView(self)
    --   this:ClearMailList()
    self:BuildFsm(self)
    self:Bi_Tracker()
...
function GiftPackNewLeetoleManWView:OnDisable()
...
function GiftPackNewLeetoleManWView:ClearCountDown()
...
function GiftPackNewLeetoleManWView:ShowItemGrid()
    ...
    self:ShowTitleSprite(xls.icon)
end

function GiftPackNewLeetoleManWView:ShowTitleSprite(iconName) 
    local iconToTitleMap = {
        cEnnewleetolemandiamondIcon = "cEnnewleetolemandiamond",
        cEnnewleetolemancoinIcon = "cEnnewleetolemancoin",
    }
    self.candysweets2:ShowSprite(iconToTitleMap[iconName] or "cEnnewleetolemancoin")
end

function GiftPackNewLeetoleManWView:ShowItemGridData(step,flag)
    _data = ModelList.GiftPackModel:GetShowGiftPack() 
    local tmpData = Csv.getPopInfiniteForId(_data.pId)
    local count = 0
...
function GiftPackNewLeetoleManWView:movetoGrid(step,tflag)
    step = step or 1
...
function GiftPackNewLeetoleManWView:ClearMailList()
...
function GiftPackNewLeetoleManWView:GetItemViewInstance(index)
...
function GiftPackNewLeetoleManWView:ShowDetailGift()
...
function GiftPackNewLeetoleManWView:SetLeftTime(endTimeStamp)
...
function GiftPackNewLeetoleManWView:OnDestroy()
    log.g("GiftPackNewLeetoleManWView.OnDestroy ing !")
    Facade.RemoveView(self)
    self:Destroy()
    self:ClearMailList()
end

function GiftPackNewLeetoleManWView:CloseFunc()
    Facade.SendNotification(NotifyName.HideUI, this)
end

function GiftPackNewLeetoleManWView:ClosePackView()
...
function GiftPackNewLeetoleManWView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackNewLeetoleManWView:OnBuySuccess(needClose,itemData)
...
function GiftPackNewLeetoleManWView:showGetRewardEffect(itemData,isFree)
...
function GiftPackNewLeetoleManWView:SetTimer()
...
--领取成功
function GiftPackNewLeetoleManWView.OnClaimEndlessGiftSuccess(data)
...
--领取失败
function GiftPackNewLeetoleManWView.OnClaimEndlessGiftFail(data)
    --提示下领取失败
end

--查询成功
function GiftPackNewLeetoleManWView.OnFetchEndlessGiftSuccess(data)
    selfstep = data.step
    local tmpflag = false
...
--领取失败
function GiftPackNewLeetoleManWView.OnFetchEndlessGiftFail(data)
    --提示下查询失败
end
