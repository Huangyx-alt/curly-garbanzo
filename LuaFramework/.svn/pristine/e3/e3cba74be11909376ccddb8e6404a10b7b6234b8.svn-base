...
local RewardCollectView = BaseView:New('RewardCollectView')
local this = RewardCollectView;
this.__index = this
...
function RewardCollectView:New()
    local o = {}
    o.__index = self
    setmetatable(o, this)
    return o
...
function RewardCollectView:Awake()
    Facade.RegisterView(self)
    self:on_init()
end

function RewardCollectView:OnEnable()
    this.collectedRewardList = {}
    fun.set_active(self.go, true)
    fun.set_active(self.Default, true)
    fun.set_active(self.Active, false)
    fun.set_active(self.Default2, false)
    fun.set_active(self.Active2, false)
end

function RewardCollectView:OnDisable()

end

function RewardCollectView:OnDestroy()
...
function RewardCollectView:ChangeBgLength()
    local count = ModelList.BattleModel:GetCurrModel():GetCardCount()
    if count == 4 then
        return
    end
    
    local showItemCount = GetTableLength(this.collectedRewardList)
    local bgSize = fun.get_rect_delta_size(self.Bg)
    
    local target = 100 + (showItemCount - 1) * 120 + 60
    if bgSize.y < target then
        if self.bgAnim then self.bgAnim:Kill() end
        self.bgAnim = Anim.do_smooth_float_update(bgSize.y, target , 0.15, function(newY)
            fun.set_recttransform_native_size(self.Bg, bgSize.x, newY)
        end, function()
            fun.set_recttransform_native_size(self.Bg, bgSize.x, target)
...
function RewardCollectView:AddReward(rewardItemID)
    ...
    fun.set_active(self.Active, true)
    ...
    if not target then
        ...
        if len < childCount then
            item = fun.get_child(self.Items, len)
        else
            item = fun.get_instance(self.Item, self.Items)
        end
...
function RewardCollectView:RewardShow(rewardItemID, cardId, cellIndex)
    local target = this.collectedRewardList[rewardItemID]
    if target then
...
function RewardCollectView.OnPutRewardToCell(cardId, cellIndex, rewardItemID)
    this:AddReward(rewardItemID)
end

function RewardCollectView.SignCollectReward(cardId, cellIndex, rewards)
