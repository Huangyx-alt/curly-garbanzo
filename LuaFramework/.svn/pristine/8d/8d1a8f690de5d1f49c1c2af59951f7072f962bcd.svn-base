local GemQueenCardSwitchItem = BaseChildView:New()
local this = GemQueenCardSwitchItem
this.cardPage = {}

function GemQueenCardSwitchItem:New(go)
    self = {}
    setmetatable(self, { __index = GemQueenCardSwitchItem })
    --self._data = cellData
    return self
end
...
---初始化switch 分页卡牌
function GemQueenCardSwitchItem:InitMap(go, type)
...
function GemQueenCardSwitchItem:ShowAll(go, orderIndex)
    local ref = fun.get_component(go, fun.REFER)
    if ref then
        self.content = ref:Get("content")
        self.img_reap = ref:Get("img_reap")
...
function GemQueenCardSwitchItem:Refresh(pageIndex, newCard, cardId, signInfo, cardObj)
            ...
            for k, v in pairs(newCard) do
                if i == v.index then
                    ...
                    if IsAutoClick(v.cardId, v.index, cardId, signInfo) then
                        ...
                        BattleEffectCache:GetFreePrefabFromCache("GemGameGetxiaoka", items[i].obj, 1, function(effObj)
                            if effObj then
...
function GemQueenCardSwitchItem.OnDispose()
    --self._gameObject = nil
    this.cardPage = {}
    this:Close()
end

return GemQueenCardSwitchItem