local BisonCardSwitchItem = BaseChildView:New()
local this = BisonCardSwitchItem
this.cardPage = {}

function BisonCardSwitchItem:New(go)
    self = {}
    setmetatable(self, { __index = BisonCardSwitchItem })
    --self._data = cellData
    return self
end
...
---初始化switch 分页卡牌
function BisonCardSwitchItem:InitMap(go, type)
...
function BisonCardSwitchItem:ShowAll(go, orderIndex)
    local ref = fun.get_component(go, fun.REFER)
    if ref then
        self.content = ref:Get("content")
        self.img_reap = ref:Get("img_reap")
    ...
    local bingoCount = BattleLogic.GetLogicModule(LogicName.Card_logic):GetBowlBingoCount(orderIndex)
...
function BisonCardSwitchItem:Refresh(pageIndex, newCard, cardId, signInfo, cardObj)
            ...
            for k, v in pairs(newCard) do
                if i == v.index then
                    ...
                    if IsAutoClick(v.cardId, v.index, cardId, signInfo) then
                        ...
                        BattleEffectCache:GetFreePrefabFromCache("Bisonxiaoka", items[i].obj, 1, function(effObj)
                            if effObj then
    ...
    local bingoCount = BattleLogic.GetLogicModule(LogicName.Card_logic):GetBowlBingoCount(cardId)
...
function BisonCardSwitchItem.OnDispose()
    --self._gameObject = nil
    this.cardPage = {}
    this:Close()
end

return BisonCardSwitchItem