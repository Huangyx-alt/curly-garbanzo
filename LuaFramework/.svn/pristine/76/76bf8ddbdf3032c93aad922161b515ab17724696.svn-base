...
local NewLeetoleManAStar = require("Combat.BattleLogic.NewLeetoleMan.NewLeetoleManAStar")

---  新绿头人寻路移动
local NewLeetoleManMoveMachine = {}
local this = NewLeetoleManMoveMachine
this.normalSpeed = 2
this.quickSpeed = 2
this.curStayCell = {}
    ...
    table.each(treasureQueue, function(cellIndexList, cardID)
        ...
        table.each(cellIndexList, function(v)
            ...
            if cellData and not cellData.isGained then
                local aStar = CreateInstance_(NewLeetoleManAStar)
...
function this:OnMoveEnd(cardID, endCellIndex)
    UISound.stop_loop("newleetolemancar")
    ...
    coroutine.start(function()
        ...
        if not this:IsMoving(cardID) then
            log.g(string.format("卡牌:%s,移动到达目标点：%s,重新寻路。", cardID, endCellIndex))
            --重新计算路径
...
function this:OnCellSigned(cardID, cellIndex)
    ...
    if treasureItems then
        ...
        if 41 == data.result[1] then
            ...
            log.g(string.format("卡牌:%s,格子发现宝箱:%s,当前宝箱队列:%s", cardID, cellIndex, table.print(treasureQueue[cardID])))
        end
    end

    if not this.forbidCalculatePath[cardID] and not this:IsMoving(cardID) then
        log.g(string.format("卡牌:%s,有格子盖章:%s,重新寻路。", cardID, cellIndex))
        this:ReCalculatePath(cardID)
    end
end
...
function this:ReCalculatePath(cardID)
    ...
    table.each(treasureQueue[cardID], function(v)
        ...
        if cellData and not cellData.isGained then
            ...
            if GetTableLength(path) > 0 then
                log.g(string.format("卡牌:%s,寻路到宝箱:%s,路径:%s", cardID, cellIndex, table.print(path)))
    ...
    if GetTableLength(retPath) > 0 then
        log.g(string.format("卡牌:%s,重新寻路结果：寻路到宝箱%s,路径:%s", cardID, retCellIndex, table.print(retPath)))
        --找到取宝箱的路
    ...
    --看到的宝箱都已经获得
    if isAllGained then
        log.g(string.format("卡牌:%s,重新寻路结果：看到的宝箱都已经获得,原地不动。", cardID))
        return
    end
    ...
    if retNode then
        if retNode.index == curStayCell then
            log.g(string.format("卡牌:%s,重新寻路结果：是当前所在的点，不用移动。", cardID))
            --是当前所在的点，不用移动
        ...
        if GetTableLength(path) > 0 then
            log.g(string.format("卡牌:%s,重新寻路结果：去距离宝箱最近的点%s,路径:%s", cardID, retNode.index, table.print(path)))
            --去距离宝箱最近的点
...
function this:SetPath(cardID, path)
    ...
    coroutine.start(function()
        coroutine.wait(0.2)
        UISound.play_loop("newleetolemancar")
        coroutine.wait(0.2)
...
function private.InitMoveCtrl(self)
    ...
    for cardID = 1, cardCount do
        ...
        self.curTurnDir[cardID] = MoveDirection.Left
