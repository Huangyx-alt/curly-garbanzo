...
local GotYouBingoRule = BaseBingoRule:New("GotYouBingoRule")
local this = GotYouBingoRule
local treasureData = {}
local MaxCollectCount = {}

function GotYouBingoRule:Start(loadData, ruleData)
    --jackpot盖章图形
...
--某些玩法需要特殊处理
function GotYouBingoRule:HandleSpecial(collectCount)
    local playType = ModelList.BattleModel:GetGameType()
    return collectCount
end

--某些玩法需要特殊处理
function GotYouBingoRule:HandleSpecialMax()

end

function GotYouBingoRule:GetBingoType(cardId, cellIndex)
    local bingoType = 1
...
function GotYouBingoRule:CalculateBingo(cardId, cellIndex, ...)
...
function GotYouBingoRule:OnDataChange(cardId, cellIndex, ...)
    local arg = { ... }
...
function GotYouBingoRule:CalculateJackpot(cardId, cellIndex)
