...
HorseRacingConsoleView = JackpotConsoleView:New(nil)
local this = HorseRacingConsoleView
...
function HorseRacingConsoleView:OnEnable()
...
function HorseRacingConsoleView:OnDisable()
...
function HorseRacingConsoleView:SetJackpotInfo()
    local playId = ModelList.CityModel.GetPlayIdByCity()
    local reward = Csv.GetData("city_play", playId, "jackpot_reward_new")
    if reward then
        self:SetJackpotReward()
        self:SetJackpotImages("HorseRacingHallAtlas")
    end
end

function HorseRacingConsoleView:SetMaxBetRateJackpot()
...
function HorseRacingConsoleView:SetSmallBetRateJackpot()
    if self.betrate ~= ModelList.CityModel:GetBetRate() then
        self.betrate = ModelList.CityModel:GetBetRate()
        if self.go.name == "ultra_jackpot_console" then
            AnimatorPlayHelper.Play(self.anima, {"jackpot_consolein", "ultra_jackpot_consolein"}, false, nil)
...
function HorseRacingConsoleView:OnMaxBetRateEnableJackpot(betRate)
