...
---  宝石皇后玩法
local EasterModel = BaseGameModel:New()
local this = EasterModel
local private = {}

------需要子类重写，设置参数------------------------------------------------------------------------------
function EasterModel:InitModeOptions()
    self.gameType_ = PLAY_TYPE.PLAY_TYPE_EASTER_DAY
    self.cityPlayID_ = 21
end

function EasterModel:GetQuestItemOffsetPos()
    return 29, -29
end

------------------------------------------------------------------------------------------------------

function EasterModel:InitData()
    self:InitModeOptions()
    self:SetSelfGameType(self.gameType_)
end

function EasterModel:ReqEnterGame()
    ...
    local procedure = require "Procedure/ProcedureEaster"
    ...
    self.SendMessage(MSG_ID.MSG_GAME_EASTER_DAY, data)
end

function EasterModel:ReqSignCard(info)
    local cardIds = {}
...
function EasterModel:RefreshRoundDataByIndex(cardid, cellIndex, signType, need_fly_item, mark)
    --父类处理
...
function EasterModel.ResponeEnterGame(code, data)
...
function EasterModel:ResBingoInfo(code, data)
    if (code == RET.RET_SUCCESS) then
...
function EasterModel:OnReceiveSettle()
    if self:GetGameState() < GameState.ShowSettle then
        self:ShowSettle()
    end
end

function EasterModel:Clear()
...
function EasterModel:IsBingoShowComplete()
...
--- 战斗结算前，检查彩球叫号器
function EasterModel:CheckJokerBallSprayer(callBack)
...
function EasterModel:SetRoundGiftData(card_id, cell_index, new_gift, number, skill_id, self_bingo, powerId, extraPos)
...
function EasterModel:UploadGameData(gameType, quiteType)
...
function EasterModel:PreCalculateBingoSkill()
...
function EasterModel:CheckBingoSkillExtraPosValid(cardId, extraPos)
    local ret, type = {}
...
this.BaseMsgIdList = {
    { msgid = MSG_ID.MSG_GAME_EASTER_DAY, func = this.ResponeEnterGame },
}
