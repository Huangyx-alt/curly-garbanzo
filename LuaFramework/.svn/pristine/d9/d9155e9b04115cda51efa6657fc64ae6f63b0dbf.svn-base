...
local HorseRacingSignEffect = BaseSignEffect:New("HorseRacingSignEffect")
setmetatable(HorseRacingSignEffect, BaseSignEffect)
local this = HorseRacingSignEffect
local itemShowCache = {}

function HorseRacingSignEffect:RegisterEvent()
    itemShowCache = {}
    Event.AddListener(EventName.Event_Sign_Cell_Background, self.CellBgEffect, self)
end

function HorseRacingSignEffect:UnRegisterEvent()
    Event.RemoveListener(EventName.Event_Sign_Cell_Background, self.CellBgEffect, self)
end

function HorseRacingSignEffect:TriggerSingleBingo(cardId, cellIndex)
    --ModelList.BattleModel:GetCurrModel():GetRoundData(cardId)
    Event.Brocast(EventName.Event_Show_Skill_Bingo, cardId, cellIndex)
end

--格子盖章效果
function HorseRacingSignEffect:SignCardEffect(cardId, index, cardCell, signType, delay, self_bingo, giftLen)
...
local function GetDiEffectName(itemType, indexList)
    if itemType == 1 then
        if indexList and #indexList == 2 then
            if math.abs(indexList[2] - indexList[1]) == 5 then
                return "Bison_Item1Show_Hor"
            else
                return "Bison_Item1Show_Ver"
            end
        else
            log.log("Bison GetDiEffectName Error 1", indexList)
            return "Bison_Item1Show_Hor"
        end
    elseif itemType == 2 then
        return "Bison_Item2Show"
    elseif itemType == 3 then
        return "Bison_Item2Showjin"
    elseif itemType == 4 then
        return "Bison_Item3Show"
    elseif itemType == 5 then
        return "Bison_Item3Showjin"
    else
        log.log("Bison GetDiEffectName Error 2", itemType, indexList)
        return "Bison_Item1Show_Hor"
    end
end
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          
--- 格子底部印章效果
function HorseRacingSignEffect:CellBgEffect(treasureType, cardId, cellIndex, indexList, part, pos)
    --[=[
        ...
        if not part then
            if itemShowCache[cardId] and itemShowCache[cardId][groupID] then
                ...
                if indexList and #indexList == 0 then
                    ...
                    --]]
                    log.log("HorseRacingSignEffect:CellBgEffect play in anim 1")
                end
                log.log("HorseRacingSignEffect:CellBgEffect 00", cardId, cellIndex, part)
                return
            end
        else
            if itemShowCache[cardId] and itemShowCache[cardId][groupID] then
                ...
                if indexList and #indexList == 0 then
                    ...
                    anim:Play("in")
                    log.log("HorseRacingSignEffect:CellBgEffect play in anim 2")
                end
                log.log("HorseRacingSignEffect:CellBgEffect 01 不重复创建", cardId, cellIndex, part)
                return
            end
        end
    end
    ...
    if pos then
        ...
        log.log("HorseRacingSignEffect:CellBgEffect 02 setPos", diEffectName, pos, treasureType, cardId, cellIndex, part)
    else
        local cellObj = cellData:GetCellObj()
        pos = cellObj.transform.position
        fun.set_gameobject_pos(diEffect, pos.x, pos.y, pos.z, false)
        log.log("HorseRacingSignEffect:CellBgEffect 03 setPos", diEffectName, treasureType, cardId, cellIndex, part)
    end
...
--- 显示格子点击提示
function HorseRacingSignEffect:ShowNormalCellTip(cell, cardid, cellIndex)
...
--设置新的JokerBg
function HorseRacingSignEffect:SetJokerBgOnSign(ref_temp, cardId, cellIndex)
