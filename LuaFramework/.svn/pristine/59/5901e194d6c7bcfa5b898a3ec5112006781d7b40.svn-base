...
local BisonFlyItemEffect = BaseFlyItemEffect:New("BisonFlyItemEffect")
local this = BisonFlyItemEffect
...
function this:CreateTreasure(cardId, cellIndex)
    ...
    if treasureItems then
        ...
        if 50 == data.result[1] then
            UISound.play("bisonflyin")
            ...
            if groupCells then
                ...
                if not isAllSigned then
                    pos = pos / #groupCells
                    log.log("BisonFlyItemEffect:CreateTreasure 00 cardId, cellIndex, #groupCells, indexList, pos ", cardId, cellIndex, #groupCells, indexList, pos)
                    Event.Brocast(EventName.Event_Sign_Cell_Background, data.result[2], cardId, cellIndex, indexList, treasureItems.part, pos)
                    return
                end
            end

            --收集到的道具
            local effectName = self:GetFlyItemName(data.result[2], groupCells)            
            ...
            log.log("BisonFlyItemEffect:CreateTreasure 01 cardId, cellIndex, data.result[2] ", cardId, cellIndex, data.result[2])
...
function this:GetFlyItemName(itemType, groupCells)
