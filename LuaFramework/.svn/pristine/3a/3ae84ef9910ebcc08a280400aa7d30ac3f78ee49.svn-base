...
local GiftPackLimitFireDragonView = GiftPackBaseView:New("GiftPackLimitFireDragonView", "GiftPack_HotFixPack_GiftPackLimitEaster_AutoAtlasInMain")
local this = GiftPackLimitFireDragonView
...
function GiftPackLimitFireDragonView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackLimitFireDragonView:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    UISound.play("sale_ultrabet")
end

function GiftPackLimitFireDragonView:OnDisable()
...
function GiftPackLimitFireDragonView:ClearCountDown()
...
function GiftPackLimitFireDragonView:CloseUIShiny(obj)
...
function GiftPackLimitFireDragonView:GetLastClickTime()
    return lastClickTime or 0
end

function GiftPackLimitFireDragonView:on_btn_up_buy_click()
...
function GiftPackLimitFireDragonView:on_btn_buy2_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackLimitFireDragonView:SetExtra(extraUI, extraParent, bonus)
...
function GiftPackLimitFireDragonView:SetItemGrid(itemId, itemNum, itemGrid)
...
function GiftPackLimitFireDragonView:ShowItemGrid()
...
function GiftPackLimitFireDragonView:SetSellInfo(id,buyCount)
...
function GiftPackLimitFireDragonView:SetItem(itemUI, id, pos, btn)
...
function GiftPackLimitFireDragonView:SetCanBuyCount(cout)
   self.txt_soldnum.text = tostring(cout)
end


 

function GiftPackLimitFireDragonView:GetItemGrid(pos,index,prefab, parent)
...
function GiftPackLimitFireDragonView:HideGridItem()
    for k, v in pairs(itemList) do
        for z, w in pairs(v) do
            if w then
...
function GiftPackLimitFireDragonView:ShowDetailGift()
...
function GiftPackLimitFireDragonView:SetButton(button, id, pos)
...
function GiftPackLimitFireDragonView:SetLeftTime(endTimeStamp)
...
function GiftPackLimitFireDragonView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackLimitFireDragonView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, self)
end

function GiftPackLimitFireDragonView:ClosePackView()
...
function GiftPackLimitFireDragonView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackLimitFireDragonView:OnBuySuccess(needClose, itemData)
    self:ChangeState(self, "GiftPackShowState", itemData)
end

function GiftPackLimitFireDragonView:GetGiftPos(id)
...
function GiftPackLimitFireDragonView:GetItemPos(pos)
    local posTable ={}
