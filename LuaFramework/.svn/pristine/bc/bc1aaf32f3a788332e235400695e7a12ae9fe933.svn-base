...
---@class MonopolyGameCardView : GameCardView
local MonopolyGameCardView = GameCardView:New("MonopolyGameCardView")
local this = MonopolyGameCardView
            ...
            if treasureItems then
                ...
                if 62 == data.result[1] then
                    ...
                    if groupId then
                        ...
                        log.log("MonopolyGameCardView:InitCardData group error", treasureItems, data)
                    end
                else
                    log.log("MonopolyGameCardView:InitCardData treasureItems error", treasureItems)
                end
            end
        end)
...
--undo
function this:PreviewBankLocation()
    local cardCount = self.model:GetCardCount()
    for i = 1, cardCount do
        self:GetCard(i):PreviewBankLocation(i)
    end
    UISound.play("monopolybankshow")
end
