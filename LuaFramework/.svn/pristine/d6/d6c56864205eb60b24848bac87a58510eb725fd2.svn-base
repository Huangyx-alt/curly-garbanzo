...
local MoleMinerBingoEffect = BaseBingoEffect:New("MoleMinerBingoEffect")
local this = MoleMinerBingoEffect
setmetatable(MoleMinerBingoEffect, BaseBingoEffect)
local private = {}

function MoleMinerBingoEffect:ShowBingoOrJackpot(bingoData)
    for k, v in pairs(bingoData) do
        ...
        if v.jackpot == 0 then
            ...
            if bingoCount < 4 then
                log.log("MoleMinerBingoEffect：ShowBingoOrJackpot bingo count < 4", bingoCount, mIndex, v)
                self:BingoEffect(mIndex, anima_name, v.first_num, mIndex, bingoType, currType)
            else
                log.log("MoleMinerBingoEffect：ShowBingoOrJackpot bingocount >= 4", bingoCount, mIndex, v)
            ...
            log.log("MoleMinerBingoEffect：ShowBingoOrJackpot 形成jackpot", bingoCount, mIndex, v)
...
function MoleMinerBingoEffect:BingoEffect(mapindex, anima_name, cellIndex, cardid, bingoType, currType)
...
function MoleMinerBingoEffect:RampageBingoEffect(mapindex, anima_name, cellIndex, cardid)
    local par = self.cardView:GetCardMap(mapindex)
    local pos = par.transform.position
    BattleEffectCache:GetPrefabFromCache("MoleMinerJackpot", par, function(eff1)
        if eff1 then
...
function MoleMinerBingoEffect:JackpotEffect(mapindex, anima_name, cellIndex, cardid)
    local par = self.cardView:GetCardMap(mapindex)
    local pos = par.transform.position
...
function MoleMinerBingoEffect:ShowEffectOnSmallCard(clone, cardId, effectType, anima_name)
...
function MoleMinerBingoEffect:ChangeSignCellColor(cardId, bowlType)
...
function MoleMinerBingoEffect:SetBingoShow(bingoType, obj)
...
function private.PlayBingoAudio(bingoCount)
    if bingoCount == 1 then
        ...
        UISound.play("minerbingo")
    elseif bingoCount == 2 then
        UISound.play("bingo_firework")
        UISound.play("miner2bingo")
    elseif bingoCount == 3 then
        UISound.play("bingo_firework")
        UISound.play("miner3bingo")
    elseif bingoCount == 4 then
        UISound.play("bingo_firework")
        UISound.play("miner4bingo")
    else
        UISound.play("bingo_firework")
        log.log("MoleMinerBingoEffect PlayBingoAudio bingoCount error", bingoCount)
    end
end
------------------------------------------------------------------------------------------End
return this