...
local DrinkingFrenzyLogicSwitchCard = BaseSwitchCardLogic:New("DrinkingFrenzyLogicSwitchCard")
local this = DrinkingFrenzyLogicSwitchCard
this.moduleName = "DrinkingFrenzyLogicSwitchCard"

function DrinkingFrenzyLogicSwitchCard:InitLogic()
    self.__index:InitLogic()
    self.PageOneCardIds = { 1, 2 }
    self.BackTwoCardIds = { 3, 4 }
end

function DrinkingFrenzyLogicSwitchCard:StartSwitchCard()
...
function DrinkingFrenzyLogicSwitchCard:IsShowing(cardId)
...
function DrinkingFrenzyLogicSwitchCard:EnterDoubleJackpotChangeCard()
    if self.isSwitching then
        log.r("EnterDoubleJackpotChangeCard")
    end
    self.isSwitching = true
...
function DrinkingFrenzyLogicSwitchCard:ExitDoubleJackpotChangeCard()
...
function DrinkingFrenzyLogicSwitchCard:StartHideJackpotCardBackground(currCardId, otherJackpotId)
    self.loopMoveCard = LuaTimer:SetDelayLoopFunction(5, 0.3, -1, function()
        ...
        if BattleEffectCache:CheckEffectAllHideWithoutBingo(currCardId) then
            self:RemoveHideJackpotCardBackground()
            local moveCardId = 0
            local currIndex = 0
            local moveIndex = 0
...
function DrinkingFrenzyLogicSwitchCard:RemoveHideJackpotCardBackground()
...
function DrinkingFrenzyLogicSwitchCard:OnDisable()
    self:RemoveHideJackpotCardBackground()
end

function DrinkingFrenzyLogicSwitchCard:GetsCurrCardOrder()
