...
local MonopolyLogicSwitchCard = BaseSwitchCardLogic:New("MonopolyLogicSwitchCard")
local this = MonopolyLogicSwitchCard
this.moduleName = "MonopolyLogicSwitchCard"

function MonopolyLogicSwitchCard:InitLogic()
    self.__index:InitLogic()
    self.PageOneCardIds = { 1, 2 }
    self.BackTwoCardIds = { 3, 4 }
end

function MonopolyLogicSwitchCard:StartSwitchCard()
...
function MonopolyLogicSwitchCard:IsShowing(cardId)
...
function MonopolyLogicSwitchCard:EnterDoubleJackpotChangeCard()
    if self.isSwitching then
...
function MonopolyLogicSwitchCard:ExitDoubleJackpotChangeCard()
...
function MonopolyLogicSwitchCard:StartHideJackpotCardBackground(currCardId, otherJackpotId)
    self.loopMoveCard = LuaTimer:SetDelayLoopFunction(5, 0.3, -1, function()
        if ModelList.MonopolyModel:IsGameOver() then
            self:RemoveHideJackpotCardBackground()
            return
        end
...
function MonopolyLogicSwitchCard:RemoveHideJackpotCardBackground()
...
function MonopolyLogicSwitchCard:OnDisable()
    self:RemoveHideJackpotCardBackground()
end

function MonopolyLogicSwitchCard:GetsCurrCardOrder()
