...
local GiftPackDiamondView = GiftPackBaseView:New("GiftPackDiamondView")
local this = GiftPackDiamondView
...
function GiftPackDiamondView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackDiamondView:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    UISound.play("diamondsalepop")
end

function GiftPackDiamondView:OnDisable()
...
function GiftPackDiamondView:ClearCountDown()
...
function GiftPackDiamondView:CloseUIShiny(obj)
...
function GiftPackDiamondView:GetLastClickTime()
    return lastClickTime or 0
end

function GiftPackDiamondView:on_btn_buy1_click()
...
function GiftPackDiamondView:SetExtra(extraUI, extraParent, bonus)
...
function GiftPackDiamondView:SetItemGrid(itemId, itemNum, itemGrid)
    ...
    local text = fun.format_money_reward({id = itemId, value = itemNum})
    self:SetItemText(textNum, text)
end

function GiftPackDiamondView:SetItemText(uiText, text)
    local fontSize = 58
    if #text > 9 then
        fontSize = 36
    elseif #text > 8 then
...
function GiftPackDiamondView:ShowItemGrid()
...
function GiftPackDiamondView:SetItem(itemUI, id, pos, btn)
...
function GiftPackDiamondView:GetItemGrid(pos, index, prefab, parent)
...
function GiftPackDiamondView:HideGridItem()
    for k, v in pairs(itemList) do
        for z, w in pairs(v) do
            if w then
...
function GiftPackDiamondView:ShowDetailGift()
...
function GiftPackDiamondView:SetButton(button, id, pos)
...
function GiftPackDiamondView:SetLeftTime(endTimeStamp)
...
function GiftPackDiamondView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackDiamondView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, self)
end

function GiftPackDiamondView:ClosePackView()
    if self._fsm:GetCurName() == "GiftPackShowState" then
        return
    end

    self:ClearCountDown()
    Facade.SendNotification(NotifyName.HideUI, ViewList.GiftPackDiamondView)
    ModelList.GiftPackModel:CloseView()
end

function GiftPackDiamondView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackDiamondView:OnBuySuccess(needClose, itemData)
    self:ChangeState(self, "GiftPackShowState", itemData)
    if needClose then
        Facade.SendNotification(NotifyName.DiamondPromotion.ActivityEnd)
    end
end

function GiftPackDiamondView:GetGiftPos(id)
...
function GiftPackDiamondView:GetItemPos(pos)
    local posTable ={}
