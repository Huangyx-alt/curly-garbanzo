...
local GiftPackWomanDaysBoothView = GiftPackBaseView:New("GiftPackWomanDaysBoothView", "GGiftPackWomanDaysBoothAtlas")
local this = GiftPackWomanDaysBoothView
...
function GiftPackWomanDaysBoothView:New(viewName, atlasName)
    local o = {}
    o.viewName = viewName
    o.atlasName = atlasName
    setmetatable(o, { __index = GiftPackWomanDaysBoothView })
    return o
end

function GiftPackWomanDaysBoothView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackWomanDaysBoothView:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    UISound.play("saleboth")
end

function GiftPackWomanDaysBoothView:OnDisable()
...
function GiftPackWomanDaysBoothView:ClearCountDown()
...
function GiftPackWomanDaysBoothView:CloseUIShiny(obj)
...
function GiftPackWomanDaysBoothView:GetLastClickTime()
    return lastClickTime or 0
end

function GiftPackWomanDaysBoothView:on_btn_buy1_click()
...
function GiftPackWomanDaysBoothView:on_btn_buy2_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackWomanDaysBoothView:on_btn_buy3_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackWomanDaysBoothView:on_btn_buy_all_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackWomanDaysBoothView:SetExtra(extraUI, extraParent, bonus)
...
function GiftPackWomanDaysBoothView:SetItemGrid(itemId, itemNum, itemGrid)
...
function GiftPackWomanDaysBoothView:ShowItemGrid()
...
function GiftPackWomanDaysBoothView:ClearItems()
    for i = 1, 3 do
...
function GiftPackWomanDaysBoothView:SetItem(itemUI, id, pos, btn)
    ...
    log.log("促销内容检查 a" , id , xls)
    log.log("促销内容检查 b" , id , xls.item_description)
...
function GiftPackWomanDaysBoothView:SetLastItem(id, pos)
    local xls = Csv.GetData("pop_up", id)
    if xls then
...
function GiftPackWomanDaysBoothView:GetItemGrid(pos, index, prefab, parent)
...
function GiftPackWomanDaysBoothView:HideGridItem()
    for k, v in pairs(itemList) do
        for z, w in pairs(v) do
            if w then
...
function GiftPackWomanDaysBoothView:ShowDetailGift()
...
function GiftPackWomanDaysBoothView:SetButton(button, id, pos)
...
function GiftPackWomanDaysBoothView:SetLeftTime(endTimeStamp)
...
function GiftPackWomanDaysBoothView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackWomanDaysBoothView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, self)
end

function GiftPackWomanDaysBoothView:ClosePackView()
...
function GiftPackWomanDaysBoothView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackWomanDaysBoothView:OnBuySuccess(needClose, itemData)
    self:ChangeState(self, "GiftPackShowState", itemData)
end

function GiftPackWomanDaysBoothView:GetGiftPos(id)
    ...
    elseif id == product_list[4] then
        ...
        log.log("GiftPackWomanDaysBoothView:GetGiftPos(id) id error", id)
        return self:GetItemPos(itemListPos.Up)
    end
end

function GiftPackWomanDaysBoothView:GetItemPos(pos)
    local posTable = {}
    if pos ~= 4 then
