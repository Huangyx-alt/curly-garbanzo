...
NormalBingoRule = BaseBingoRule:New("NormalBingoRule")
local this = NormalBingoRule
setmetatable(NormalBingoRule, {__index = BaseBingoRule})
...
function NormalBingoRule:Start(loadData)
    ...
    for i = 1, count do
        NormalBingoRule.OnInitCalcuateWish(i)
    end
    this.soundFlag = {}
end
...
local GetBingoCellIndexList = function(cellIndex, cardInfo,cardId)
    ...
    for i = 1, #this.bingoCell do
        if this.bingoCell[i].cardId == cardId  and this.bingoCell[i].index == cellIndex then
            local cellData = cardInfo.cards[cellIndex]
            if cellData.sign > 0 and not cellData.isUsedForSelfBingoData then
                cellData.isUsedForSelfBingoData = true
                local data = this:CreateBingoInfo(this.BingoType.BINGO, { cardInfo.cards[cellIndex].index }, cardId)
                if data then
                    data.isSelfBingoCellTrigger = true
                end
                table.insert(newBingoInfoList, data)
            end
        end
    end
...
--获取卡牌内可形成bingo列表的格子ID
local GetBingoIndexList = function(cellIndex, cardInfo,cardId, num, options)
    options = options or {}
    ...
    for i = 1, #this.cellBingoKeyArray[cellIndex] do
        ...
        if otherHasSign then
            --不是只展示selfbingo
            if not options.onlyCheckSelfBingoCell then
                local bingoInfo = this:CreateBingoInfo(this.BingoType.BINGO, keyArrayTemp, cardId)
...
--- 计算是否出现bingo  盖章使用
---@param index
function NormalBingoRule:CalculateBingo(cardId, cellIndex, options)
    options = options or {}
    local totalBingoInfoList = {}
    for k, v in pairs(this.data) do
        local caidid = tostring(cardId)
        if k == caidid then
            local newBingoInfoList = GetBingoIndexList(cellIndex, v, cardId, v.num, options)
            ...
            if jackpot ~= nil then
                local bingoInfo = { type = this.BingoType.JACKPOT, numbers = jackpot,
                                    cardId = cardId, totolCount = self.totalBingoCount[cardId], th = 0}
...
--- 计算是否出现bingo  盖章使用
---@param index
function NormalBingoRule.OnInitCalcuateWish(cardId)
    this.data = ModelList.BattleModel:GetCurrModel():GetRoundData()
    for k, v in pairs(this.data) do
...
--- 计算是否出现wish,
function NormalBingoRule:CalculateWishByCard(cardId)
    local curModel = ModelList.BattleModel:GetCurrModel()
...
--- 计算是否出现wish,
function NormalBingoRule:CalculateCardWish(cardId,cardInfo)
    local curModel = ModelList.BattleModel:GetCurrModel()
    if curModel and not curModel:CanShowSkillCellWish() then
        return
    end
...
function NormalBingoRule.CalculateJackpot(cardId, cellIndex,cardInfo)
                ...
                if k == caidid then
                    ...
                    if allHasSign then
                        ...
                        if curModel and curModel:CanShowNormalCellJackpotWish() then
                            ...
                            this:CheckLessAchievedJackpotCount(lessAchievedJackpotCount, needCount)
                            if lessAchievedJackpotCount <= needCount then
                                table.each(oneLessAchievedJackpotList, function(cellI)
                                    --log.e(string.format("cellWishXX cardId:%s, cellIndex:%s ", cardId, cellI))
...
--- 获取计算bingo可以忽略的坐标格子
function NormalBingoRule: GetIgnorePos(skillId, cellIndex, ignoreList, cardData,cardId)
    local skill_type = Csv.GetData("new_skill", skillId, "skill_type")
    cardId = tostring(cardId)
    if skill_type == 1 then return end
    if skill_type == 7 then return end
    if skill_type == 8 then return end
    local skillXYZ = Csv.GetData("new_skill", skillId, "skill_xyz")
...
function NormalBingoRule:PreCalSkillBingo(skillId, cellIndex, cardInfo)
    local ignoreList = {}
    for i = 1, #skillId do
...
function NormalBingoRule:GetSkillIgnoreCellList(skillId, cellIndex, cardInfo,cardId)
    local ignoreList = {}
    for i = 1, #skillId do
...
function NormalBingoRule:PreCalculateBingo(num)
    for k, v in pairs(this.data) do
        for j = 1, #v.cards do
...
function NormalBingoRule:BeBingoCell(cardId,cellIndex,cardNum)
    local newCell = {cardId =cardId,index = cellIndex, num =cardNum}
    table.insert(this.bingoCell, newCell)
end

function NormalBingoRule:CancelAllBingoCell()
    log.log("NormalBingoRule:CancelAllBingoCell")
    this.bingoCell = {}
end

function NormalBingoRule:CheckLessAchievedJackpotCount(lessAchievedJackpotCount, needCount)
    local playType = ModelList.BattleModel:GetGameType()
    if playType == PLAY_TYPE.PLAY_TYPE_VICTORY_BEATS then
        if lessAchievedJackpotCount == needCount then
            if this.soundFlag and not this.soundFlag["winzone5daubs"] then
                this.soundFlag["winzone5daubs"] = true
                UISound.play("winzone5daubs")
            end
            if not ModelList.GuideModel:IsGuideComplete(69) then
                ModelList.GuideModel:TriggerWinZoneBattleGuide(329)
            end
        elseif lessAchievedJackpotCount < needCount then
            if this.soundFlag and not this.soundFlag["winzoneCloser"] then
                this.soundFlag["winzoneCloser"] = true
                --UISound.play("`winzoneCloser`")
            end
        end
    end
end

--某卡某格是否能行成普通连线bingo的wish
function NormalBingoRule:CheckNormalLineWish(cardId, cellIndex)
    local cardInfo
    for k, v in pairs(this.data) do
        if k == tostring(cardId) then
            cardInfo = v
        end
    end

    if not cardInfo then
        log.log("NormalBingoRule:CheckNormalLineWish 数据有误！", cardId, cellIndex, this.data)
        return false
    end
    ...
    for i = 1, #this.cellBingoKeyArray[cellIndex] do
        local keyArrayTemp = this.cellBingoKeyArray[cellIndex][i]
        local allOtherHasSign = true
        for n = 1, #keyArrayTemp do
            local cell_index = keyArrayTemp[n]
            if cardInfo.cards[cell_index].sign == 0 and cell_index ~= cellIndex then
                allOtherHasSign = false
            end
        end

        if allOtherHasSign then
            return true
        else
        end
    end
