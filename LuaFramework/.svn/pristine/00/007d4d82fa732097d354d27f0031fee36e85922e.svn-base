local TournamentSettleCheckTierState = require "State/TournamentSettle/TournamentSettleBaseState"
...
local TournamentBlackGoldPlayHelperView = BaseView:New("TournamentBlackGoldPlayHelperView","TournamentAtlas")

local this = TournamentBlackGoldPlayHelperView
...
function TournamentBlackGoldPlayHelperView:Awake(obj)
    self:on_init()
end

function TournamentBlackGoldPlayHelperView:OnEnable(params)
...
function TournamentBlackGoldPlayHelperView:OnDisable()
    Event.RemoveListener(NotifyName.Tournament.GuidePlayHelpViewClose,self.PlayExite,self)
    self:DisposeFsm()
end

function TournamentBlackGoldPlayHelperView:SetDescription()
    ...
    if fun.is_android_platform() then 
        self.explain4.text = Csv.GetDescription(36)
    else
        self.explain4.text = Csv.GetDescription(1089)
    end
end

function TournamentBlackGoldPlayHelperView:BuildFsm()
    self:DisposeFsm()
    self._fsm = Fsm.CreateFsm("TournamentBlackGoldPlayHelperView",self,{
...
function TournamentBlackGoldPlayHelperView:DisposeFsm()
    if self._fsm then
        self._fsm:Dispose()
        self._fsm = nil
    end
...
function TournamentBlackGoldPlayHelperView:PlayEnter()
...
function TournamentBlackGoldPlayHelperView:OnExitPlayHelper()
    AnimatorPlayHelper.Play(self.anima,{"end","HelpView_end"},false,function()
        Facade.SendNotification(NotifyName.CloseUI,self)
    end)
end

function TournamentBlackGoldPlayHelperView:on_btn_PlayNow_click()
    self._fsm:GetCurState():ClimbNext(self._fsm)
end

function TournamentBlackGoldPlayHelperView:on_btn_Close_click()
    self._fsm:GetCurState():PlayExite(self._fsm)
end

function TournamentBlackGoldPlayHelperView:PlayExite()
    AnimatorPlayHelper.Play(self.anima,{"end","TournamentExplainView_end"},false,function()
        Facade.SendNotification(NotifyName.CloseUI,this)
    end)
end

function TournamentBlackGoldPlayHelperView:PlayExiteToCity()
