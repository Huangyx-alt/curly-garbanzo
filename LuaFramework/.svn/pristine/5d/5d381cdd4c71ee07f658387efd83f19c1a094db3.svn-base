...
local JackpotEnableView = BaseDialogView:New("JackpotEnableView","JackpotEnableAtlas")
local this = JackpotEnableView
...
function JackpotEnableView:Awake(obj)
    self:on_init()
...
function JackpotEnableView:OnEnable()
    ...
    if self.bet_Item[betrate]  then 
        AnimatorPlayHelper.SetAnimationEvent(self.bet_Item[betrate],"ef_Hall_Jackpot",function()
            Facade.SendNotification(NotifyName.CloseUI,ViewList.JackpotEnableView)
        end)
...
function JackpotEnableView:on_close()

end

function JackpotEnableView:OnDisable()
    Event.Brocast(EventName.Event_jackpoview_hide)
end

function JackpotEnableView:OnDestroy()
    this:Close()
end

--- 当前模式下 jackpot的展示
function JackpotEnableView:SetJackpotIcon(betrate)
