...
local ChristmasCardSwitchItem = BaseChildView:New() --Clazz(BaseChildView,"ChristmasCardSwitchItem")
local this = ChristmasCardSwitchItem
this.cardPage = {}     --switch 分页1

function ChristmasCardSwitchItem:New(go)
    self = {}
    setmetatable(self,{__index = ChristmasCardSwitchItem})
    --self._data = cellData
    return self
end
...
---初始化switch 分页卡牌
function ChristmasCardSwitchItem:InitMap(go,type)
    ...
    for i = 1, 25 do
        ...
        fun.set_gameobject_pos(go,27.8*x,-27.5*y+5,0,true)
        fun.set_active(go,true)
    end
    table.insert(this.cardPage,page)
end

function ChristmasCardSwitchItem:ShowAll(go,orderIndex)
    local ref = fun.get_component(go.transform,fun.REFER)
    if ref then
        self.content = ref:Get("content")
        self.img_reap = ref:Get("img_reap")
...
function ChristmasCardSwitchItem:Refresh(pageIndex, newCard,cardId,signInfo,cardObj)
...
function ChristmasCardSwitchItem.OnDispose()
...
return ChristmasCardSwitchItem