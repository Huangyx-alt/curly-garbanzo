...
--- 普通玩法 Model
---@class HawaiiGameModel : BaseGameModel
local HawaiiGameModel = BaseGameModel:New()
local this = HawaiiGameModel
this.rocketSettleData = nil

function HawaiiGameModel:InitData()
    this:SetSelfGameType(PLAY_TYPE.PLAY_TYPE_COCKTAIL)
end

function HawaiiGameModel:GetQuestItemOffsetPos()
    return 29, -32
end

--请求进入游戏   ratio_str = “2,3” :卡片数量,倍率
function HawaiiGameModel.ReqEnterGame( )
    ...
    Facade.SendNotification(NotifyName.ShowUI, ViewList.SceneLoadingGameView,nil,nil,ProcedureHawaiiGame:New())
    ...
    this.SendMessage(MSG_ID.MSG_GAME_LOAD_COCKTAIL, {cardNum = cardNum,rate = betRate,powerUpId = tonumber(powerupId),
...
--请求卡片签章
function HawaiiGameModel:ReqSignCard(info)
    local cardIds = {}
...
function HawaiiGameModel:RefreshRoundDataByIndex(cardid, cellIndex, signType, need_fly_item, mark)
    --父类处理
...
--收到进入游戏返回，允许进入Game场景
function HawaiiGameModel.ResEnterGame(code,data)
...
--后端Bingo同步
function HawaiiGameModel:ResBingoInfo(code,data)
    if(code == RET.RET_SUCCESS)then
...
function HawaiiGameModel:OnReceiveSettle()
...
function HawaiiGameModel:Clear()
    this.rocketSettleData = nil
    this.__index:Clear()
end
...
this.MsgIdList =
{
    { msgid = MSG_ID.MSG_GAME_LOAD_COCKTAIL, func = this.ResEnterGame },
}
