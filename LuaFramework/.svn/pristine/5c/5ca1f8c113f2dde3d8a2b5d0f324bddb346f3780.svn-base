local TournamentShowNextRewardView = {}
...
function TournamentShowNextRewardView:OnDestroy()
...
function TournamentShowNextRewardView:OnEnable(params)
...
function TournamentShowNextRewardView:GetMyNewReward()
    local data = ModelList.TournamentModel:GetSettleClimbData(climbIndex + 1)
    local myLastSectionIndex = data.lastSectionIndex
    local rewardData = nil
    if myLastSectionIndex == 1 and self:CheckIsBlackTier() then
        local topOneData = ModelList.TournamentModel:GetTrouamentClimbRankTopOneData(climbIndex + 1)
...
function TournamentShowNextRewardView:CheckIsBlackTier()
    local lastTier = ModelList.TournamentModel:GetClimbTiers(climbIndex + 1)
...
function TournamentShowNextRewardView:InitMyNewReward()
...
-- local testNum = 0
function TournamentShowNextRewardView:GetTopOneReward()
    local topOnePlayerData = ModelList.TournamentModel:GetTrouamentClimbRankTopOneData(climbIndex + 1)
...
function TournamentShowNextRewardView:InitTopOneReward()
...
function TournamentShowNextRewardView:InitView()
    self:InitMyNewReward()
    self:InitTopOneReward()
end

function TournamentShowNextRewardView:GetItemUiData(totalNum , index)
...
return TournamentShowNextRewardView