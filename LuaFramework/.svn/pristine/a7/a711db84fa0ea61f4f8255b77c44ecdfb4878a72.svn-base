...
local GiftPackBisonSaleView = GiftPackBaseView:New("GiftPackBisonSaleView")
local this = GiftPackBisonSaleView
...
function GiftPackBisonSaleView:New(viewName, atlasName)
    local o = {}
    o.viewName = viewName
    o.atlasName = atlasName
    setmetatable(o,{__index = GiftPackBisonSaleView})
    return o
end

function GiftPackBisonSaleView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackBisonSaleView:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    UISound.play("saleboth")
end

function GiftPackBisonSaleView:OnDisable()
...
function GiftPackBisonSaleView:ClearCountDown()
...
function GiftPackBisonSaleView:CloseUIShiny(obj)
...
function GiftPackBisonSaleView:GetLastClickTime()
    return lastClickTime or 0
end

function GiftPackBisonSaleView:on_btn_buy1_click()
...
function GiftPackBisonSaleView:on_btn_buy2_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackBisonSaleView:on_btn_buy3_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackBisonSaleView:on_btn_buy_all_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackBisonSaleView:SetExtra(extraUI, extraParent, bonus)
...
function GiftPackBisonSaleView:SetItemGrid(itemId, itemNum, itemGrid)
...
function GiftPackBisonSaleView:ShowItemGrid()
...
function GiftPackBisonSaleView:ClearItems()
    for i = 1, 3 do
...
function GiftPackBisonSaleView:SetItem(itemUI, id, pos, btn)
...
function GiftPackBisonSaleView:SetLastItem(id, pos)
    local xls = Csv.GetData("pop_up", id)
    if xls then
...
function GiftPackBisonSaleView:GetItemGrid(pos,index,prefab, parent)
...
function GiftPackBisonSaleView:HideGridItem()
    for k, v in pairs(itemList) do
        for z, w in pairs(v) do
            if w then
...
function GiftPackBisonSaleView:ShowDetailGift()
...
function GiftPackBisonSaleView:SetButton(button, id, pos)
...
function GiftPackBisonSaleView:SetLeftTime(endTimeStamp)
...
function GiftPackBisonSaleView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackBisonSaleView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, self)
end


function GiftPackBisonSaleView:ClosePackView()
...
function GiftPackBisonSaleView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackBisonSaleView:OnBuySuccess(needClose, itemData)
    self:ChangeState(self, "GiftPackShowState", itemData)
end

function GiftPackBisonSaleView:GetGiftPos(id)
    ...
    elseif id == product_list[4] then
        ...
        log.log("GiftPackBisonSaleView:GetGiftPos(id) id error", id)
        return self:GetItemPos(itemListPos.Up)
    end
end

function GiftPackBisonSaleView:GetItemPos(pos)
    local posTable ={}
    if pos ~= 4 then
