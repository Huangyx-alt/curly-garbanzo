...
-- local GiftPackSpringBoothView = GiftPackBaseView:New("GiftPackSpringBoothView", "GGiftPackWomanDaysBoothAtlas")
local GiftPackSpringBoothView = GiftPackBaseView:New("GiftPackSpringBoothView", "GiftPackSpringBoothAtlas")

local this = GiftPackSpringBoothView
...
function GiftPackSpringBoothView:New(viewName, atlasName)
    local o = {}
    o.viewName = viewName
    o.atlasName = atlasName
    setmetatable(o, { __index = GiftPackSpringBoothView })
    return o
end

function GiftPackSpringBoothView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackSpringBoothView:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    UISound.play("saleboth")
end

function GiftPackSpringBoothView:OnDisable()
...
function GiftPackSpringBoothView:ClearCountDown()
...
function GiftPackSpringBoothView:CloseUIShiny(obj)
...
function GiftPackSpringBoothView:GetLastClickTime()
    return lastClickTime or 0
end

function GiftPackSpringBoothView:on_btn_buy1_click()
...
function GiftPackSpringBoothView:on_btn_buy2_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackSpringBoothView:on_btn_buy3_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackSpringBoothView:on_btn_buy_all_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackSpringBoothView:SetExtra(extraUI, extraParent, bonus)
...
function GiftPackSpringBoothView:SetItemGrid(itemId, itemNum, itemGrid)
...
function GiftPackSpringBoothView:ShowItemGrid()
...
function GiftPackSpringBoothView:ClearItems()
    for i = 1, 3 do
...
function GiftPackSpringBoothView:SetItem(itemUI, id, pos, btn)
...
function GiftPackSpringBoothView:SetLastItem(id, pos)
    local xls = Csv.GetData("pop_up", id)
    if xls then
...
function GiftPackSpringBoothView:GetItemGrid(pos, index, prefab, parent)
...
function GiftPackSpringBoothView:HideGridItem()
    for k, v in pairs(itemList) do
        for z, w in pairs(v) do
            if w then
...
function GiftPackSpringBoothView:ShowDetailGift()
...
function GiftPackSpringBoothView:SetButton(button, id, pos)
...
function GiftPackSpringBoothView:SetLeftTime(endTimeStamp)
...
function GiftPackSpringBoothView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackSpringBoothView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, self)
end

function GiftPackSpringBoothView:ClosePackView()
...
function GiftPackSpringBoothView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackSpringBoothView:OnBuySuccess(needClose, itemData)
    self:ChangeState(self, "GiftPackShowState", itemData)
end

function GiftPackSpringBoothView:GetGiftPos(id)
    ...
    elseif id == product_list[4] then
        ...
        log.log("GiftPackSpringBoothView:GetGiftPos(id) id error", id)
        return self:GetItemPos(itemListPos.Up)
    end
end

function GiftPackSpringBoothView:GetItemPos(pos)
    local posTable = {}
    if pos ~= 4 then
