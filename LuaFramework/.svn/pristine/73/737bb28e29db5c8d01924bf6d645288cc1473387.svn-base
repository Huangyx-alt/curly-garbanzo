...
local EasterLogicSwitchCard = BaseSwitchCardLogic:New("EasterLogicSwitchCard")
local this = EasterLogicSwitchCard
this.moduleName = "EasterLogicSwitchCard"

function EasterLogicSwitchCard:InitLogic()
    self.__index:InitLogic()
    self.PageOneCardIds = { 1, 2 }
    self.BackTwoCardIds = { 3, 4 }
end

function EasterLogicSwitchCard:StartSwitchCard()
...
function EasterLogicSwitchCard:IsShowing(cardId)
...
function EasterLogicSwitchCard:EnterDoubleJackpotChangeCard()
    if self.isSwitching then
...
function EasterLogicSwitchCard:ExitDoubleJackpotChangeCard()
...
function EasterLogicSwitchCard:StartHideJackpotCardBackground(currCardId, otherJackpotId)
    self.loopMoveCard = LuaTimer:SetDelayLoopFunction(5, 0.3, -1, function()
        if ModelList.EasterModel:IsGameOver() then
            self:RemoveHideJackpotCardBackground()
            return
        end
...
function EasterLogicSwitchCard:RemoveHideJackpotCardBackground()
...
function EasterLogicSwitchCard:OnDisable()
    self:RemoveHideJackpotCardBackground()
end

function EasterLogicSwitchCard:GetsCurrCardOrder()
