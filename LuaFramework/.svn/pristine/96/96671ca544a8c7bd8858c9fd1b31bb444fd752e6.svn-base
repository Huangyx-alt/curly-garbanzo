...
---@class  GiftPackRocketTripleView: GiftPackBaseView
local GiftPackRocketTripleView = GiftPackBaseView:New('GiftPackRocketTripleView', "GiftPackRocketAtlas")
local this = GiftPackRocketTripleView
...
function GiftPackRocketTripleView:Awake(obj)
...
function GiftPackRocketTripleView.OnEnable()
...
function GiftPackRocketTripleView:on_x_update()
...
function GiftPackRocketTripleView:ShowDetailGift()
...
function GiftPackRocketTripleView:CollectProduct(pId,giftInfo)
    product_list = {}
...
function GiftPackRocketTripleView:SetUpProduct(id)
...
function GiftPackRocketTripleView:SetContent(id,txt,content)
    id = tonumber(id)
...
function GiftPackRocketTripleView:SetDownLeftProduct(id)
    local xls = Csv.GetData("pop_up", id)
    local item1 = xls.item_description[1][1]
...
function GiftPackRocketTripleView:SetDownRightProduct(id)
    local xls = Csv.GetData("pop_up", id)
    local item1 = xls.item_description[1][1]
...
function GiftPackRocketTripleView:SetLeftTime(endTimeStamp)
...
function GiftPackRocketTripleView:SetBounsAndMore(obj, bonus, more)
...
function GiftPackRocketTripleView.OnDisable()
    --Facade.RemoveView(this)
    this:stop_x_update()
    --Event.Brocast(Notes.CARD_COLLIDER_OPEN, true)
end

function GiftPackRocketTripleView.OnDestroy()
    this:Destroy()
end

function GiftPackRocketTripleView:on_close()
    this:stop_x_update()
    --Event.Brocast(Notes.CARD_COLLIDER_OPEN, true)
end

function GiftPackRocketTripleView:CloseFunc()
    --this.main_effect:Play("end")
    Facade.SendNotification(NotifyName.HideDialog, this)

end

function GiftPackRocketTripleView:ShowTip(tipName)

end

function GiftPackRocketTripleView:InitTip()

end

function GiftPackRocketTripleView:CutDonwTarget()
    this:CloseFunc()
    this.main_effect:Play("end")
end

function GiftPackRocketTripleView:on_btn_sure_click()
...
function GiftPackRocketTripleView:on_btn_cancle_click()
...
function GiftPackRocketTripleView:on_btn_close_click()
    if this._fsm:GetCurName() == "GiftPackShowState" then return end
    if this.loopTime then
        LuaTimer:Remove(this.loopTime)
        this.loopTime = nil
    end
    Facade.SendNotification(NotifyName.CloseUI, ViewList.GiftPackRocketTripleView)
    ModelList.GiftPackModel:CloseView()
end

function GiftPackRocketTripleView:on_btn_buy_click()
end

this.last_click_time1 = 0
function GiftPackRocketTripleView:on_btn_up_buy_click()
...
function GiftPackRocketTripleView:on_btn_bl_buy_click()
    if this._fsm:GetCurName() ~= "GiftPackEnterState" then return end
    if canBLClick then
        local click_time = UnityEngine.Time.time
...
function GiftPackRocketTripleView:on_btn_br_buy_click()
    if this._fsm:GetCurName() ~= "GiftPackEnterState" then return end
    if canBRClick then
        local click_time = UnityEngine.Time.time
...
function GiftPackRocketTripleView:CloseUIShiny(obj)
...
function  GiftPackRocketTripleView:OnBuySuccess(needClose,itemData)
    this:ChangeState(this,"GiftPackShowState",itemData)
end

function  GiftPackRocketTripleView:GetGiftPos(id)
    ...
    elseif id == product_list[2] then
        return this.bl_reward_icon.transform.position,this.bl_cost_icon.transform.position, this.cityExp2.transform.position,nil
    else
        return this.br_reward_icon.transform.position,this.br_cost_icon.transform.position,
        this.cityExp3.transform.position,nil
    end
end
