...
local GiftPackSuperBowlSaleView = GiftPackBaseView:New('GiftPackSuperBowlSaleView',"GiftPackSuperBowlSaleAtlas")
local this = GiftPackSuperBowlSaleView
...
function GiftPackSuperBowlSaleView:New()
    local o = {}
    setmetatable(o,{__index = GiftPackSuperBowlSaleView})
    return o
end

function GiftPackSuperBowlSaleView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackSuperBowlSaleView:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    UISound.play("sale_joker")
end

function GiftPackSuperBowlSaleView:OnDisable()
...
function GiftPackSuperBowlSaleView:ClearCountDown()
...
function GiftPackSuperBowlSaleView:CloseUIShiny(obj)
...
function GiftPackSuperBowlSaleView:on_btn_buy1_click()
...
function GiftPackSuperBowlSaleView:GetLastClickTime(pos)
    return clickTimeList or 0
end

function GiftPackSuperBowlSaleView:on_btn_buy2_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then return end
...
function GiftPackSuperBowlSaleView:on_btn_buy3_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then return end
...
function GiftPackSuperBowlSaleView:SetExtra(extraUI,extraParent , bonus)
...
function GiftPackSuperBowlSaleView:SetItemGrid(itemId, itemNum , itemGrid)
...
function GiftPackSuperBowlSaleView:ShowItemGrid()
...
function GiftPackSuperBowlSaleView:SetItem(itemUI,id, pos, btn)
    ...
    local textExtra2 = itemUI:Get("textExtra2")
...
function GiftPackSuperBowlSaleView:GetItemGrid(pos,index,prefab, parent)
...
function GiftPackSuperBowlSaleView:HideGridItem()
    for k ,v in pairs(itemList) do
        for z,w in pairs(v) do
            if w then
...
function GiftPackSuperBowlSaleView:ShowDetailGift()
...
function GiftPackSuperBowlSaleView:SetButton(button, id,pos)
...
function GiftPackSuperBowlSaleView:SetLeftTime(endTimeStamp)
...
function GiftPackSuperBowlSaleView:CollectProduct(pId,giftInfo)
    product_list = {}
...
function GiftPackSuperBowlSaleView:OnDestroy()
    self:Destroy()
end

function GiftPackSuperBowlSaleView:on_close()
    --Event.Brocast(Notes.CARD_COLLIDER_OPEN, true)
end

function GiftPackSuperBowlSaleView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, this)
end

function GiftPackSuperBowlSaleView:ClosePackView()
...
function GiftPackSuperBowlSaleView:on_btn_close_click()
    self:ClosePackView()
end

function  GiftPackSuperBowlSaleView:OnBuySuccess(needClose,itemData)
    self:ChangeState(self,"GiftPackShowState",itemData)
end

function  GiftPackSuperBowlSaleView:GetGiftPos(id)
...
function GiftPackSuperBowlSaleView:GetItemPos(pos)
    local posTable ={}
