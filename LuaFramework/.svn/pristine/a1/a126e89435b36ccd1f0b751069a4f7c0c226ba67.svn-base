...
this.atlasName = "LetemRollPurchaseAtlas"
...
function PassPurchaseView:CloseSelf(closeMethod)
    if closeMethod == self.CloseMethod.normal and self.isActiveByPopup then
        log.log("Facade.SendNotification(NotifyName.GamePlayShortPassView.PopupPassPurchaseOrderFinish) 1")
        Event.Brocast(NotifyName.GamePlayShortPassView.PopupPassPurchaseOrderFinish)
    end
...
function PassPurchaseView:OnActivateGoldenPassPayResult(code)
    log.log("PassPurchaseView:OnActivateGoldenPassPayResult code is ", code)
    if code == RET.RET_SUCCESS then
        ...
        if PurchaseHelper.IsEditorPurchase() then
            ...
            if not payData.pid or tostring(payData.pid) == "" or tostring(payData.pid) == "0" then
                ...
                log.log("PassPurchaseView:OnActivateGoldenPassPayResult Editor productId, purchaseId, payData.pid",
                    productId, self:GetModel():get_productId(), payData.pid)
            ...
            if not payData.pid or tostring(payData.pid) == "" then
                ...
                local product_name = Csv.GetData("appstorepurchaseconfig", self:GetModel():get_productId(),
                    "product_name")
...
function PassPurchaseView:Purchasing_success()
    ...
    if self.isActiveByPopup then
        params.onCloseCallback = function ()
            log.log("Facade.SendNotification(NotifyName.GamePlayShortPassView.PopupPassPurchaseOrderFinish) 2")
            Event.Brocast(NotifyName.GamePlayShortPassView.PopupPassPurchaseOrderFinish)
        end
    end
