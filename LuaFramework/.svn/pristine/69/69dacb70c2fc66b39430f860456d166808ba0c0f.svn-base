...
local BisonPuSkill = BaseSkill:New("BisonPuSkill")
local this = BisonPuSkill
local private = {}

function BisonPuSkill:ShowSkill(cardId, cellIndex, powerId, skillId, serverExtraPos)
    if skillId == 133 then
...
-----------------------------------------------------------skill 1-----------------------------------------------------------Begin
function BisonPuSkill:ShowSkill01(cardId, cellIndex, powerId, skillId)
    log.log("BisonPuSkill:ShowSkill01(cardId, cellIndex, , )", cardId, cellIndex)
    ...
    BattleEffectCache:GetSkillPrefabFromCache("Bisonskill1", target, function(obj)
        UISound.play("bisonwhip")
        local cellIndexs = self:GetSkill01Cells(cellIndex)
        local signDelays = {1.3, 1.3, 1.3, 1.3}
...
function BisonPuSkill:GetSkill01Cells(startCellIndex)
    local ret = {}
    for i = 1, 4 do
...
-----------------------------------------------------------skill 2-----------------------------------------------------------Begin
function BisonPuSkill:GetSkill02Cells(startCellIndex)
    local ret = {}
    for i = 1, 4 do
        local tmpIdx = startCellIndex - i
        if tmpIdx % 5 > 0 then
...
function BisonPuSkill:ShowSkill02(cardId, cellIndex, powerId, skillId, serverExtraPos)
            ...
            table.each(cardsInfo and cardsInfo.beginSkillData, function(data)
                if not extraPos then
                    table.each(data.effect, function(v)
                        if not extraPos then
                            if v.itemId == 2026 and fun.is_include(pos, v.posList) then
                                extraPos = v.extraPos
                            end
                        end
                    end)
    ...
    log.log("BisonPuSkill:ShowSkill02(cardId, cellIndex, , , serverExtraPos), extraPos", cardId, cellIndex, serverExtraPos, extraPos)
    ----测试用代码
    ...
    BattleEffectCache:GetSkillPrefabFromCache("Bisonskill2deng", cellObj, function(obj)
        fun.set_same_position_with_but_z_zero(obj, cellObj)
        self:SameScaleWithCard(obj, cardId)
        UISound.play("bisonstampede")
        LuaTimer:SetDelayFunction(1, function()
            table.each(extraPos, function(cellID, k)
                ...
                if cellData:IsNotSign() then
                    ...
                    BattleEffectCache:GetSkillPrefabFromCache("Bisonskill2dengget", cellData.obj, function()
