...
this.atlasName = "SolitairePassTaskAtlas"
this.longTaskAtlasName = "SolitairePassTaskAtlas"
this.viewName = "PassTaskView"

function PassTaskView:OnEnable(params)
    self:BaseEnable(params)
    --- flag:短令牌 每次增加新的短令牌都要修改
    if not ModelList.GuideModel:IsGuideComplete(91) then
        ModelList.GuideModel:TriggerPassTaskView(604)
    end
end
