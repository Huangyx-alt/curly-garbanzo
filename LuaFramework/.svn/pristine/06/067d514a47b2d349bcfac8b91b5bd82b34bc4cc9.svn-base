...
---@class  GiftPackGotYouBuffView: GiftPackBaseView
local GiftPackGotYouBuffView = GiftPackBaseView:New('GiftPackGotYouBuffView', "GiftPackGotYouBuffAtlas")
local this = GiftPackGotYouBuffView
...
function GiftPackGotYouBuffView:Awake(obj)
    this:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackGotYouBuffView.OnEnable()
    this:BuildFsm(this)
    this:Bi_Tracker()
end

function GiftPackGotYouBuffView:on_x_update()
end

function GiftPackGotYouBuffView:ShowDetailGift()
...
function GiftPackGotYouBuffView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackGotYouBuffView.GetItemOrResource(id, param)
    if id > 1000 then
...
function GiftPackGotYouBuffView:SetLeftProduct(id)
...
function GiftPackGotYouBuffView:SetRightProduct(id)
    local xls = Csv.GetData("pop_up", id)
...
function GiftPackGotYouBuffView:SetLeftTime(endTimeStamp)
...
function GiftPackGotYouBuffView:SetContent(id, txt, content)
    id = tonumber(id)
    if fun.is_null(txt) then
        return
    end
    --- flag:短令牌 每次增加新的短令牌都要修改
    if id == 23 or id == 24 or id == 903202 then
...
function GiftPackGotYouBuffView.OnDisable()
    this:stop_x_update()
end

function GiftPackGotYouBuffView.OnDestroy()
    this:Destroy()
end

function GiftPackGotYouBuffView:on_close()
    this:stop_x_update()
end

function GiftPackGotYouBuffView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, this)
end

function GiftPackGotYouBuffView:CutDonwTarget()
    this:CloseFunc()
    this.main_effect:Play("end")
end

function GiftPackGotYouBuffView:on_btn_close_click()
...
function GiftPackGotYouBuffView:on_btn_left_buy_click()
...
function GiftPackGotYouBuffView:on_btn_right_buy_click()
    if this._fsm:GetCurName() ~= "GiftPackEnterState" then return end
    if canDownBuy then
        local click_time = UnityEngine.Time.time
...
function GiftPackGotYouBuffView:CloseUIShiny(obj)
...
function GiftPackGotYouBuffView:SetUIImageGray(btn_obj,active)
    if active then
...
function GiftPackGotYouBuffView:OnBuySuccess(needClose,itemData)
    this:ChangeState(this,"GiftPackShowState",itemData)
end

function GiftPackGotYouBuffView:GetGiftPos(id)
