local SGExtraBallItem = BaseChildView:New()
local this = SGExtraBallItem
this.__index = this
...
--@param num 格子数字
function SGExtraBallItem:SetType( num)
    if num <= 9 then
...
--- 隐藏
function SGExtraBallItem:SetActive(isActive)
    if self then
        fun.set_active(self.go, isActive)
    end
end

function SGExtraBallItem:SetNumEffect(isEffect)
