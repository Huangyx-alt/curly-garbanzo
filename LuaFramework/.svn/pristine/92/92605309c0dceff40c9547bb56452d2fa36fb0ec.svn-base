...
local ThievesBingoRule = BaseBingoRule:New("ThievesBingoRule")
local this = ThievesBingoRule
setmetatable(ThievesBingoRule, {__index = BaseBingoRule})

local wolfData = {}    --缓存狼人锁链解锁数据
local MaxBreakCount = 4  --每个狼人解锁所需数量
local wolfCellPos = {7, 9, 17, 19}

function ThievesBingoRule:Start(loadData,wish_item)
    wolfData = {}
...
function ThievesBingoRule:CalculateBingo(cardId, cellIndex, ...)
...
function ThievesBingoRule:OnDataChange(cardId, cellIndex, ...)
    local arg = {...}
...
function ThievesBingoRule:CalculateJackpot(cardId)
    local data = wolfData[cardId]
...
function ThievesBingoRule:GetTotalBingoCount(cardId)
