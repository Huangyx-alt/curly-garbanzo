...
local HallofFameGoldSettleView = BaseDialogView:New("HallofFameGoldSettleView", "TournamentSettleTopPlayerAtlas")
local this = HallofFameGoldSettleView
...
function HallofFameGoldSettleView:Awake(obj)
    self:on_init()
end

function HallofFameGoldSettleView:OnEnable(params)
    AddLockCountOneStep(true)
    Event.Brocast(NotifyName.HallofFame.FameShowResultToCloseRankView)
    ...
    self:SetMyTrophy()
    CommonState.BuildFsm(self, "HallofFameGoldSettleView")
    self:PlayEnterAnim()
    self:SetCollectInfo()
...
function HallofFameGoldSettleView:SetIsTopPlayer()
    ...
    local topPlayerInfo = ModelList.HallofFameModel:GetResultTopPlayerData()
...
function HallofFameGoldSettleView:PlayEnterAnim()
    local myRank = ModelList.HallofFameModel:GetMyRank()
    local clipName = ""
    local motionName = ""
    if myRank <= 3 and isTopPlayer then
        clipName = "topstart"
        motionName = "TournamentBlackGoldSettleTopPlayerView_topstart"
    else
        clipName = "lowstart"
        motionName = "TournamentBlackGoldSettleTopPlayerView_lowstart"
    end
...
function HallofFameGoldSettleView:OnDisable()
    AddLockCountOneStep(false)
    isTopPlayer = false
    flyItemList = {}
    self.topPlayerReward = {}
    Event.RemoveListener(NotifyName.HallofFame.FameTopPlayerCollectRewardCloseView, self.TopPlayerCollectRewardCloseView, self)
    Event.Brocast(EventName.Event_popup_halloffame_reward)
    Facade.RemoveView(self)
    CommonState.DisposeFsm(self)
end

function HallofFameGoldSettleView:DelayShowRewardByRank()
    local myRank = ModelList.HallofFameModel:GetMyRank()
...
function HallofFameGoldSettleView:StartCountDown()
    local nextOpenTime = ModelList.HallofFameModel:GetResultNextOpenTime()
...
function HallofFameGoldSettleView:SetTopPlayerInfo()
    local topPlayerInfo = ModelList.HallofFameModel:GetResultTopPlayerData()
    local myRank = ModelList.HallofFameModel:GetMyRank()
...
function HallofFameGoldSettleView:SetPlayerInfo(index, playerInfo, playerObj, myRank)
    if not playerInfo then
        return
    end
    log.log("奖励道具检查", myRank, playerInfo)
    ...
    local blackCard = ref:Get("blackCard")
    local textCardMoney = ref:Get("textCardMoney")
    ...
    local hasBlackCardReward, cardId = self:CheckHasBlackCardReward(playerInfo.reward)
    if hasBlackCardReward then
        fun.set_active(blackCard, true)
        textCardMoney.text = fun.GetAmazonCardDescription(cardId)
        local cardImg = fun.get_component(blackCard, fun.IMAGE)
        if fun.is_ios_platform() then
            cardImg.sprite = AtlasManager:GetSpriteByName("TournamentSettleTopPlayerAtlas", "ZBzhenjinjiangli4old")
        else
            --cardImg.sprite = AtlasManager:GetSpriteByName("TournamentSettleTopPlayerAtlas", "ZBduanweijiangli2")
        end
    else
        fun.set_active(blackCard, false)
    end
    self:ReBindRankImg(playerObj, index)
end

function HallofFameGoldSettleView:ReBindRankImg(playerObj, rank)
    if playerObj then
...
function HallofFameGoldSettleView:CheckHasBlackCardReward(reward)
    if not reward then
        return false
    end
...
--排名在前三 放大背景 下移节点
function HallofFameGoldSettleView:SetViewNear()
    local nearVlaue = 1.3
...
--排名在前三 放大背景 上移界面
function HallofFameGoldSettleView:SetViewFar()
    local myRank = ModelList.HallofFameModel:GetMyRank()
    local nearVlaue = 1
    local farMoveDownPos = self.moveDown.transform.localPosition
...
function HallofFameGoldSettleView:SetMyRewardInfo()
    local myRank = ModelList.HallofFameModel:GetMyRank()
    if myRank <= 3 and isTopPlayer then
    ...
    local rewards = ModelList.HallofFameModel:GetMyRewardItem()  --实际使用
    flyItemList = {}
...
function HallofFameGoldSettleView:SetCollectInfo()
    local myRank = ModelList.HallofFameModel:GetMyRank()
    if myRank <= 3 and isTopPlayer then
...
function HallofFameGoldSettleView:SetMyTrophy()
    local tiers = ModelList.HallofFameModel:GetMyTrophy()
    local trophyName = string.format("MRTht0%s",tiers)
    Cache.load_sprite(AssetList["trophyName"], trophyName, function(sprite)
        if fun.is_not_null(sprite) and self and fun.is_not_null(self.imgTorphy) then
            self.imgTorphy.sprite = sprite
        end
    end)
end

function HallofFameGoldSettleView:TopPlayerCollectRewardCloseView()
...
function HallofFameGoldSettleView:OepnTopPlayerBox(myRank)
    Event.AddListener(NotifyName.HallofFame.FameTopPlayerCollectRewardCloseView, self.TopPlayerCollectRewardCloseView, self)
    local rewardBox = self:GetTopRewardBox(myRank)
    local startMovePos = rewardBox.transform.position
    Facade.SendNotification(NotifyName.ShowUI, ViewList.HallofFameGoldSettleRewardView, nil, false, { startMovePos = startMovePos, rewardBox = rewardBox })
end

function HallofFameGoldSettleView:on_btn_collect_click()
...
function HallofFameGoldSettleView:DelayReturnState()
...
function HallofFameGoldSettleView:ResphoneRewardRequest()
    local rewards = ModelList.HallofFameModel:GetMyRewardItem()
    self:FlyRewardToTopArea()
end

function HallofFameGoldSettleView:FlyRewardToTopArea()
    LuaTimer:SetDelayFunction(1, function()
...
function HallofFameGoldSettleView:RequestReward()
    Event.AddListener(NotifyName.HallofFame.FameResphoneRewardRequest, self.ResphoneRewardRequest, self) --实际使用
    ModelList.HallofFameModel:C2S_RequestRewardInfo()
end

function HallofFameGoldSettleView:GetTopRewardBox(rank)
    return self.topRewardBoxList[rank]
end

function HallofFameGoldSettleView:PlayExit()
    if not fun.is_null(self.anima) then
        AnimatorPlayHelper.Play(self.anima, { "end", "TournamentBlackGoldSettleTopPlayerView_end" }, false, function()
            Facade.SendNotification(NotifyName.CloseUI, self)
        end)
    end
end

function HallofFameGoldSettleView:on_btn_continue_click()
    local animBtnConeTinue = fun.get_component(self.btn_continue, fun.ANIMATOR)
...
function HallofFameGoldSettleView:on_btn_box1_click()
    local offset = Vector3.New(0, -200, 0)
    self:OpenTopPlayerRewardTip(1, offset, RewardShowTipOrientation.up, Vector2.New(0.5, 0.5))
end

function HallofFameGoldSettleView:on_btn_box2_click()
    local offset = Vector3.New(-50, -100, 0)
    self:OpenTopPlayerRewardTip(2, offset, RewardShowTipOrientation.leftUp, Vector2.New(0, 1))
end

function HallofFameGoldSettleView:on_btn_box3_click()
    local offset = Vector3.New(50, -100, 0)
    self:OpenTopPlayerRewardTip(3, offset, RewardShowTipOrientation.rightUp, Vector2.New(1, 1))
end

function HallofFameGoldSettleView:on_btn_box4_click()
    local offset = Vector3.New(-50, -50, 0)
    self:OpenTopPlayerRewardTip(4, offset, RewardShowTipOrientation.leftUp, Vector2.New(0, 1))
end

function HallofFameGoldSettleView:on_btn_box5_click()
    local offset = Vector3.New(50, -50, 0)
    self:OpenTopPlayerRewardTip(5, offset, RewardShowTipOrientation.rightUp, Vector2.New(1, 1))
end

function HallofFameGoldSettleView:UpdateClickBoxBtnEnable(state)
    fun.enable_button(self.btn_box1, state)
    fun.enable_button(self.btn_box2, state)
    fun.enable_button(self.btn_box3, state)
    fun.enable_button(self.btn_box4, state)
    fun.enable_button(self.btn_box5, state)
end

function HallofFameGoldSettleView:UpdateTargetClickBoxBtnEnable(index, state)
    fun.enable_button(self["btn_box" .. index], state)
end

function HallofFameGoldSettleView:OpenTopPlayerRewardTip(index, offset, dir, pivot)
...
function HallofFameGoldSettleView:SetTopPlayerUseToShow()
    self.topPlayerReward = {}
    local reward = ModelList.HallofFameModel:GetResultTopPlayerData()
...
function HallofFameGoldSettleView:GetTopPlayerUseToShow(rankIndex)
...
function HallofFameGoldSettleView:GetShowTipWidth(rewardsNum)
    return (rewardsNum - 1) * 150 + 180
end

function HallofFameGoldSettleView.ReqWeekSettleError()
