local NewLeetoleManCardSwitchItem = BaseChildView:New()
local this = NewLeetoleManCardSwitchItem
this.cardPage = {}
this.itemRoots = {}

function NewLeetoleManCardSwitchItem:New(go)
    self = {}
    setmetatable(self, { __index = NewLeetoleManCardSwitchItem })
    --self._data = cellData
    return self
end

---初始化switch 分页卡牌
function NewLeetoleManCardSwitchItem:InitMap(go, type)
...
function NewLeetoleManCardSwitchItem:ShowAll(go, orderIndex)
...
function NewLeetoleManCardSwitchItem:Refresh(pageIndex, newCard, cardId, signInfo, cardObj)
            ...
            for k, v in pairs(newCard) do
                if i == v.index then
                    ...
                    if IsAutoClick(v.cardId, v.index, cardId, signInfo) then
                        BattleEffectCache:GetFreePrefabFromCache("NewLeetoleManGetxiaoka", items[i].obj, 1, function(effObj)
...
function NewLeetoleManCardSwitchItem:InitItemRewardCtrl()
    self.itemGroups = {}
...
function NewLeetoleManCardSwitchItem:ShowItemRewardCtrl(pageIndex, cardID)
    --其他卡片显示中的道具
...
function NewLeetoleManCardSwitchItem.OnDispose()
    this.cardPage = {}
    this.itemRoots = {}
    this:Close()
end

return NewLeetoleManCardSwitchItem