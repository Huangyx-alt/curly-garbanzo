...
local GiftPackMinerLimitedView = GiftPackBaseView:New("GiftPackMinerLimitedView", "GiftPackMinerLimitedAtlas")
local this = GiftPackMinerLimitedView
...
function GiftPackMinerLimitedView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackMinerLimitedView:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    UISound.play("sale_ultrabet")
end

function GiftPackMinerLimitedView:OnDisable()
...
function GiftPackMinerLimitedView:ClearCountDown()
...
function GiftPackMinerLimitedView:CloseUIShiny(obj)
...
function GiftPackMinerLimitedView:GetLastClickTime()
    return lastClickTime or 0
end

function GiftPackMinerLimitedView:on_btn_up_buy_click()
...
function GiftPackMinerLimitedView:on_btn_buy2_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackMinerLimitedView:SetExtra(extraUI, extraParent, bonus)
...
function GiftPackMinerLimitedView:SetItemGrid(itemId, itemNum, itemGrid)
...
function GiftPackMinerLimitedView:ShowItemGrid()
...
function GiftPackMinerLimitedView:SetSellInfo(id,buyCount)
...
function GiftPackMinerLimitedView:SetItem(itemUI, id, pos, btn)
...
function GiftPackMinerLimitedView:SetCanBuyCount(cout)
...
function GiftPackMinerLimitedView:GetItemGrid(pos,index,prefab, parent)
...
function GiftPackMinerLimitedView:HideGridItem()
    for k, v in pairs(itemList) do
        for z, w in pairs(v) do
            if w then
...
function GiftPackMinerLimitedView:ShowDetailGift()
...
function GiftPackMinerLimitedView:SetButton(button, id, pos)
...
function GiftPackMinerLimitedView:SetLeftTime(endTimeStamp)
...
function GiftPackMinerLimitedView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackMinerLimitedView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, self)
end

function GiftPackMinerLimitedView:ClosePackView()
...
function GiftPackMinerLimitedView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackMinerLimitedView:OnBuySuccess(needClose, itemData)
    self:ChangeState(self, "GiftPackShowState", itemData)
end

function GiftPackMinerLimitedView:GetGiftPos(id)
...
function GiftPackMinerLimitedView:GetItemPos(pos)
    local posTable ={}
