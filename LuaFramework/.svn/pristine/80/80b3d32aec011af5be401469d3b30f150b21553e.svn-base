...
local GoldenPigSignEffect = BaseSignEffect:New("GoldenPigSignEffect")
setmetatable(GoldenPigSignEffect, BaseSignEffect)
local this = GoldenPigSignEffect
local itemShowCache = {}

function GoldenPigSignEffect:RegisterEvent()
    itemShowCache = {}
    Event.AddListener(EventName.Event_Sign_Cell_Background, self.CellBgEffect, self)
end

function GoldenPigSignEffect:UnRegisterEvent()
    Event.RemoveListener(EventName.Event_Sign_Cell_Background, self.CellBgEffect, self)
end

function GoldenPigSignEffect:TriggerSingleBingo(cardId, cellIndex)
    --ModelList.BattleModel:GetCurrModel():GetRoundData(cardId)
    Event.Brocast(EventName.Event_Show_Skill_Bingo, cardId, cellIndex)
end

function GoldenPigSignEffect:SignCardEffect(cardId, index, cardCell, signType, delay, self_bingo, giftLen)
...
function GoldenPigSignEffect:CellBgEffect(treasureType, cardId, cellIndex, part, pos)
...
function GoldenPigSignEffect:SetJokerBgOnSign(ref_temp, cardId, cellIndex)
    --local jokerBg = ref_temp:Get("JokerBg")
    --if not fun.is_null(jokerBg) then
