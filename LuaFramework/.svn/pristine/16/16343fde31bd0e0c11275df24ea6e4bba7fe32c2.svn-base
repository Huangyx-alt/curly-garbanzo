...
--****** 主玩法-gameLoad *********--
 ...
 PB_GameLoadVictoryBeats_Res  =  {
    ...
    ext                                          =  2,-- string ext 8,--拓展字段json格式 狼人玩法json参考：['wolfData':[['cardId' = >1,'wolfPos' = >22,'freeMarkPos' = >[21]],['cardId' = >1,'wolfPos' = >24,'freeMarkPos' = >[11]],['cardId' = >1,'wolfPos' = >42,'freeMarkPos' = >[41]],['cardId' = >1,'wolfPos' = >44,'freeMarkPos' = >[51]],***其它卡依次类推 ]]
    jokerData                 =  2,-- repeated PB_JokerCardData 9,--所有小丑卡数据
    jackpotLeftTick                      =  3,-- repeated int32 0 ,--扣减时间线 客户端 按照游戏间隔换算 单位：1/100秒
    userBingoTick             =  3,-- repeated PB_UserBingoTick 1,--扣减时间线 客户端 按照游戏间隔换算 单位：1/100秒
    jackpotLeft                                   =  3,-- int32 jackpotLeft 2,--剩余jackpotLeft数量（晋级人数）
    oneAwayTick                 =  3,-- repeated PB_OneAwayTick 3,--差一个位置的人数Tick
}

 PB_UserBingoTick  =  {
    tick  =  1,-- int32 tick , -- 时间线
    uid  =  2,-- int32 uid , -- 用户ID
    content  =  3,-- string content , -- 描述内容
}

 PB_OneAwayTick  =  {
    tick  =  1,-- int32 tick , -- 时间线
    count  =  2,-- int32 count , -- 人数数量
}