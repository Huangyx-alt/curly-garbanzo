...
function PassPurchaseView:CloseSelf(closeMethod)
    if closeMethod == self.CloseMethod.normal and self.isActiveByPopup then
        log.log("Facade.SendNotification(NotifyName.GamePlayShortPassView.PopupGemQueenPassPurchaseOrderFinish) 1")
        Event.Brocast(NotifyName.GamePlayShortPassView.PopupGemQueenPassPurchaseOrderFinish)  
    end
    ...
    if code == RET.RET_SUCCESS then
        ...
        if PurchaseHelper.IsEditorPurchase() then
            ...
            if not payData.pid or tostring(payData.pid) == "" or tostring(payData.pid) == "0" then
                log.log("PassPurchaseView:OnActivateGoldenPassPayResult Editor payData.pid illegal", payData)
                self:Purchasing_success()
            else
                local productId = Csv.GetData("appstorepurchaseconfig", self:GetModel():get_productId(), "product_id")
                log.log("PassPurchaseView:OnActivateGoldenPassPayResult Editor productId, purchaseId, payData.pid", productId, self:GetModel():get_productId(), payData.pid)
...
function PassPurchaseView:Purchasing_success()
    ...
    if self.isActiveByPopup then
        params.onCloseCallback = function()
            log.log("Facade.SendNotification(NotifyName.GamePlayShortPassView.PopupGemQueenPassPurchaseOrderFinish) 2")
            Event.Brocast(NotifyName.GamePlayShortPassView.PopupGemQueenPassPurchaseOrderFinish)    
        end
    end
