---  新绿头人玩法寻路算法
local EasterAStar = Clazz(ClazzBase, "EasterAStar")
local this = EasterAStar
local private = {}

local dirOrder = { -1, 5, 1, -5 } --上、右、下、左

function this:FindPath(cardID, startIndex, endIndex)
    ...
    self.startNode = private.CreateNode(self.startIndex, nil, 0, h + w, { h, w })
...
function this:AddCellToOpenList(parentNode, cellIndex, g)
    ...
    if oldNode then
        ...
        local newNode = private.CreateNode(cellIndex, parentNode, g, h + w, { h, w })
...
--创建一个node
function private.CreateNode(cellIndex, parentNode, g, h, hw)
    local node = {}
