...
---@class  GiftPackPirateShipBuffView: GiftPackBaseView
local GiftPackPirateShipBuffView = GiftPackBaseView:New('GiftPackPirateShipBuffView', "GiftPackMoleMinerBuffAtlasInMain")
local this = GiftPackPirateShipBuffView
...
function GiftPackPirateShipBuffView:Awake(obj)
    this:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackPirateShipBuffView.OnEnable()
    this:BuildFsm(this)
    this:Bi_Tracker()
end

function GiftPackPirateShipBuffView:on_x_update()
end

function GiftPackPirateShipBuffView:ShowDetailGift()
...
function GiftPackPirateShipBuffView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackPirateShipBuffView.GetItemOrResource(id, param)
    if id > 1000 then
...
function GiftPackPirateShipBuffView:SetLeftProduct(id)
...
function GiftPackPirateShipBuffView:SetRightProduct(id)
    local xls = Csv.GetData("pop_up", id)
...
function GiftPackPirateShipBuffView:SetLeftTime(endTimeStamp)
...
function GiftPackPirateShipBuffView:SetContent(id, txt, content)
    id = tonumber(id)
    if fun.is_null(txt) then
        return
    end
    --- flag:短令牌 每次增加新的短令牌都要修改
    if id == 23 or id == 24 or id == 902302 then
...
function GiftPackPirateShipBuffView.OnDisable()
    this:stop_x_update()
end

function GiftPackPirateShipBuffView.OnDestroy()
    this:Destroy()
end

function GiftPackPirateShipBuffView:on_close()
    this:stop_x_update()
end

function GiftPackPirateShipBuffView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, this)
end

function GiftPackPirateShipBuffView:CutDonwTarget()
    this:CloseFunc()
    this.main_effect:Play("end")
end

function GiftPackPirateShipBuffView:on_btn_close_click()
...
function GiftPackPirateShipBuffView:on_btn_left_buy_click()
...
function GiftPackPirateShipBuffView:on_btn_right_buy_click()
    if this._fsm:GetCurName() ~= "GiftPackEnterState" then return end
    if canDownBuy then
        local click_time = UnityEngine.Time.time
...
function GiftPackPirateShipBuffView:CloseUIShiny(obj)
...
function GiftPackPirateShipBuffView:SetUIImageGray(btn_obj,active)
    if active then
...
function GiftPackPirateShipBuffView:OnBuySuccess(needClose,itemData)
    this:ChangeState(this,"GiftPackShowState",itemData)
end

function GiftPackPirateShipBuffView:GetGiftPos(id)
