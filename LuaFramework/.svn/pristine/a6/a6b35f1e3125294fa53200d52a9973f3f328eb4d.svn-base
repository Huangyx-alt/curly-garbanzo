...
---@class EasterGameCardView : GameCardView
local EasterGameCardView = GameCardView:New("EasterGameCardView")
local this = EasterGameCardView
        ...
        table.each(cardData.cards, function(cellData)
            ...
            if treasureItems then
                ...
                if 42 == data.result[1] then
                    local temp_ref = fun.get_component(cellData:GetCellObj(), fun.REFER)
                    if temp_ref then
                        local bg_tip = temp_ref:Get("bg_tip")
                        bg_tip.sprite = AtlasManager:GetSpriteByName("EasterBingoAtlas", "Easterbcard05Egg")
                    end
                end
            end
        end)
...
function this:CollectItem(cardId, flyToType)
    cardId = tonumber(cardId)
    local addCount, maxCount = 1, 1
    if itemCount[cardId][flyToType] < maxCount and itemCount[cardId][flyToType] + addCount >= maxCount then
        table.insert(newBingoType[cardId], flyToType)
    end

    if flyToType then
        self:GetCard(cardId):AddBowlDrink(itemCount, cardId, flyToType, addCount)
    else
        this.nextFlyTargetType = this.nextFlyTargetType or {}
        this.nextFlyTargetType[cardId] = this.nextFlyTargetType[cardId] or 1
        
        self:GetCard(cardId):AddBowlDrink(itemCount, cardId, this.nextFlyTargetType[cardId], addCount)
       
        this.nextFlyTargetType[cardId] = this.nextFlyTargetType[cardId] + 1
        if this.nextFlyTargetType[cardId] > 5 then
            this.nextFlyTargetType[cardId] = 5
        end
    end
end
...
function this:RegisterExtraEvent()
    Event.AddListener(EventName.Event_View_Collect_Item, self.CollectItem, self)
...
function this:UnRegisterExtraEvent()
    Event.RemoveListener(EventName.Event_View_Collect_Item, self.CollectItem, self)
...
function this:GetFlyTargetPos(cardId, itemType)
    ...
    if fun.is_not_null(flyTarget) then
        local temp = this.nextFlyTargetType[cardId]
        
        this.nextFlyTargetType[cardId] = temp + 1
        if this.nextFlyTargetType[cardId] > 5 then
            this.nextFlyTargetType[cardId] = 5
        end
        
        return flyTarget.transform.position, temp
    end
    
    log.e(string.format("Cant find Fly Target, cardId:%s,itemType:%s, nextFlyTargetType:%s", cardId, itemType, this.nextFlyTargetType[cardId]))
    return Vector3.zero
end
