...
this.atlasName = "PirateShipPurchaseAtlas"
...
function PassPurchaseView:CloseSelf(closeMethod)
    if closeMethod == self.CloseMethod.normal and self.isActiveByPopup then
        log.log("Facade.SendNotification(NotifyName.GamePlayShortPassView.PopupPirateShipPassPurchaseOrderFinish) 1")
        Event.Brocast(NotifyName.GamePlayShortPassView.PopupPirateShipPassPurchaseOrderFinish)
    end
...
function PassPurchaseView:Purchasing_success()
    ...
    if self.isActiveByPopup then
        params.onCloseCallback = function()
            log.log("Facade.SendNotification(NotifyName.GamePlayShortPassView.PopupPirateShipPassPurchaseOrderFinish) 2")
            Event.Brocast(NotifyName.GamePlayShortPassView.PopupPirateShipPassPurchaseOrderFinish)
        end
    end
