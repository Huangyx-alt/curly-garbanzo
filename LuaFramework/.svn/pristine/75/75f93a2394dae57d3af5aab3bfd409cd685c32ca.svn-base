...
---@class  GiftPackChristmasSynthesisBuffView: GiftPackBaseView
local GiftPackChristmasSynthesisBuffView = GiftPackBaseView:New('GiftPackChristmasSynthesisBuffView', "GiftPackChristmasSynthesisBuffAtlas")
local this = GiftPackChristmasSynthesisBuffView
...
function GiftPackChristmasSynthesisBuffView:Awake(obj)
    this:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackChristmasSynthesisBuffView.OnEnable()
    this:BuildFsm(this)
    this:Bi_Tracker()
end

function GiftPackChristmasSynthesisBuffView:on_x_update()
end

function GiftPackChristmasSynthesisBuffView:ShowDetailGift()
...
function GiftPackChristmasSynthesisBuffView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackChristmasSynthesisBuffView.GetItemOrResource(id, param)
    if id > 1000 then
...
function GiftPackChristmasSynthesisBuffView:SetLeftProduct(id)
...
function GiftPackChristmasSynthesisBuffView:SetRightProduct(id)
    local xls = Csv.GetData("pop_up", id)
...
function GiftPackChristmasSynthesisBuffView:SetLeftTime(endTimeStamp)
...
function GiftPackChristmasSynthesisBuffView:SetContent(id, txt, content)
    id = tonumber(id)
    if fun.is_null(txt) then
        return
    end
    --- flag:短令牌 每次增加新的短令牌都要修改
    if id == 23 or id == 24 or id == 903102 then
...
function GiftPackChristmasSynthesisBuffView.OnDisable()
    this:stop_x_update()
end

function GiftPackChristmasSynthesisBuffView.OnDestroy()
    this:Destroy()
end

function GiftPackChristmasSynthesisBuffView:on_close()
    this:stop_x_update()
end

function GiftPackChristmasSynthesisBuffView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, this)
end

function GiftPackChristmasSynthesisBuffView:CutDonwTarget()
    this:CloseFunc()
    this.main_effect:Play("end")
end

function GiftPackChristmasSynthesisBuffView:on_btn_close_click()
...
function GiftPackChristmasSynthesisBuffView:on_btn_left_buy_click()
...
function GiftPackChristmasSynthesisBuffView:on_btn_right_buy_click()
    if this._fsm:GetCurName() ~= "GiftPackEnterState" then return end
    if canDownBuy then
        local click_time = UnityEngine.Time.time
...
function GiftPackChristmasSynthesisBuffView:CloseUIShiny(obj)
...
function GiftPackChristmasSynthesisBuffView:SetUIImageGray(btn_obj,active)
    if active then
...
function GiftPackChristmasSynthesisBuffView:OnBuySuccess(needClose,itemData)
    this:ChangeState(this,"GiftPackShowState",itemData)
end

function GiftPackChristmasSynthesisBuffView:GetGiftPos(id)
