...
local GiftPackFlagDay01View = GiftPackBaseView:New('GiftPackFlagDay01View', "GiftPackFlagDay01ViewAtlas")
local this = GiftPackFlagDay01View
...
function GiftPackFlagDay01View:Awake(obj)
    self:on_init()
end

function GiftPackFlagDay01View:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    UISound.play("sale_flagday")
end

function GiftPackFlagDay01View:OnDisable()
...
function GiftPackFlagDay01View:ClearCountDown()
...
function GiftPackFlagDay01View:CloseUIShiny(obj)
...
function GiftPackFlagDay01View:on_btn_buy1_click()
...
function GiftPackFlagDay01View:GetLastClickTime(pos)
    return clickTimeList or 0
end

function GiftPackFlagDay01View:on_btn_buy2_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then return end
...
-- function GiftPackFlagDay01View:on_btn_buy3_click()
...
function GiftPackFlagDay01View:SetExtra(extraUI,extraParent , bonus)
...
function GiftPackFlagDay01View:SetItemGrid(itemId, itemNum , itemGrid)
...
function GiftPackFlagDay01View:ShowItemGrid()
...
function GiftPackFlagDay01View:SetItem(itemUI,id, pos, btn)
...
function GiftPackFlagDay01View:GetItemGrid(pos,index,prefab, parent)
...
function GiftPackFlagDay01View:HideGridItem()
    for k ,v in pairs(itemList) do
        for z,w in pairs(v) do
            if w then
...
function GiftPackFlagDay01View:ShowDetailGift()
    self:ShowItemGrid()
end

function GiftPackFlagDay01View:SetButton(button, id,pos)
...
function GiftPackFlagDay01View:SetLeftTime(endTimeStamp)
...
function GiftPackFlagDay01View:CollectProduct(pId,giftInfo)
    product_list = {}
...
--function GiftPackFlagDay01View.OnDestroy()
--    self:Destroy()
--end

function GiftPackFlagDay01View:on_close()
    --Event.Brocast(Notes.CARD_COLLIDER_OPEN, true)
end

function GiftPackFlagDay01View:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, self)
end

function GiftPackFlagDay01View:ClosePackView()
...
function GiftPackFlagDay01View:on_btn_close_click()
    self:ClosePackView()
end

function  GiftPackFlagDay01View:OnBuySuccess(needClose,itemData)
    self:ChangeState(self,"GiftPackShowState",itemData)
end

function  GiftPackFlagDay01View:GetGiftPos(id)
...
function GiftPackFlagDay01View:GetItemPos(pos)
    local posTable ={}
