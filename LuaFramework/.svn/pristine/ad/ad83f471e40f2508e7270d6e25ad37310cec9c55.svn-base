...
local GiftPackHalloweenBoothView = GiftPackBaseView:New("GiftPackHalloweenBoothView", "GiftPackHalloweenBoothViewAtlas")
local this = GiftPackHalloweenBoothView
...
function GiftPackHalloweenBoothView:New(viewName, atlasName)
    local o = {}
    o.viewName = viewName
    o.atlasName = atlasName
    setmetatable(o, { __index = GiftPackHalloweenBoothView })
    return o
end

function GiftPackHalloweenBoothView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackHalloweenBoothView:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    self:InitReportData()
...
function GiftPackHalloweenBoothView:OnDisable()
...
function GiftPackHalloweenBoothView:ClearCountDown()
...
function GiftPackHalloweenBoothView:CloseUIShiny(obj)
...
function GiftPackHalloweenBoothView:GetLastClickTime()
    return lastClickTime or 0
end

function GiftPackHalloweenBoothView:on_btn_buy1_click()
...
function GiftPackHalloweenBoothView:on_btn_buy2_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackHalloweenBoothView:on_btn_buy3_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackHalloweenBoothView:on_btn_buy_all_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackHalloweenBoothView:SetExtra(extraUI, extraParent, bonus)
...
function GiftPackHalloweenBoothView:SetItemGrid(itemId, itemNum, itemGrid)
...
function GiftPackHalloweenBoothView:ShowItemGrid()
...
function GiftPackHalloweenBoothView:ClearItems()
    for i = 1, 3 do
...
function GiftPackHalloweenBoothView:SetItem(itemUI, id, pos, btn)
...
function GiftPackHalloweenBoothView:SetLastItem(id, pos)
    local xls = Csv.GetData("pop_up", id)
    if xls then
...
function GiftPackHalloweenBoothView:GetItemGrid(pos, index, prefab, parent)
...
function GiftPackHalloweenBoothView:HideGridItem()
    for k, v in pairs(itemList) do
        for z, w in pairs(v) do
            if w then
...
function GiftPackHalloweenBoothView:ShowDetailGift()
...
function GiftPackHalloweenBoothView:SetButton(button, id, pos)
...
function GiftPackHalloweenBoothView:SetLeftTime(endTimeStamp)
...
function GiftPackHalloweenBoothView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackHalloweenBoothView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, self)
end

function GiftPackHalloweenBoothView:ClosePackView()
...
function GiftPackHalloweenBoothView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackHalloweenBoothView:OnBuySuccess(needClose, itemData)
    self:SetPurchaseState(BingoBangEntry.reportPurchaseState.PurchaseSuccess)
    self:ChangeState(self, "GiftPackShowState", itemData)
end

function GiftPackHalloweenBoothView:GetGiftPos(id)
    ...
    elseif id == product_list[4] then
        ...
        log.log("GiftPackHalloweenBoothView:GetGiftPos(id) id error", id)
        return self:GetItemPos(itemListPos.Up)
    end
end

function GiftPackHalloweenBoothView:GetItemPos(pos)
    local posTable = {}
    if pos ~= 4 then
