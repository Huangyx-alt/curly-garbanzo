...
local Base64 = Base64
local Base64Key = Base64Key
local string = string

function Base64.getCode(key)
    local code = {}
    for i = 1, string.len(key) do
        code[#code + 1] = string.sub(key, i, i)
    end
    return code
end

function Base64.getDecode(key)
    local code = Base64.getCode(key)
    local decode = {}
    for k,v in pairs(code) do
        decode[string.byte(v, 1)] = k - 1
    end
    return decode
end

function Base64.encode(text, key)
    key = key or Base64Key
    local code = Base64.getCode(key)
    ...
    for i = 1, len, 3 do
        ...
        for j = 1, 4 do
            ...
            res[index] = code[curPos]
            index = index + 1
        end
    end

    if left == 1 then
        Base64.__left1(res, index, text, len, code)
    elseif left == 2 then
        Base64.__left2(res, index, text, len, code)        
    end
    return table.concat(res)
end

function Base64.__left2(res, index, text, len, code)
    ...
    curPos = tmp2 % 64 + 1
    res[index + 1] = code[curPos]

    curPos = num % 64 + 1
    res[index + 2] = code[curPos]
    
    res[index + 3] = "=" 
end

function Base64.__left1(res, index,text, len, code)
...
function Base64.decode(text, key)
    key = key or Base64Key
    local len = string.len(text)
    local left = 0 
    ...
    local res = {}
    local index = 1
    local decode = Base64.getDecode(key)
    for i =1, len, 4 do
    ...
    if left == 1 then
        Base64.__decodeLeft1(res, index, text, len, decode)
    elseif left == 2 then
        Base64.__decodeLeft2(res, index, text, len, decode)
    end
    return table.concat(res)
end

function Base64.__decodeLeft1(res, index, text, len, decode)
...
function Base64.__decodeLeft2(res, index, text, len, decode)
    local a = decode[string.byte(text, len + 1)] 
    local b = decode[string.byte(text, len + 2)]
    local num = a * 64 + b
