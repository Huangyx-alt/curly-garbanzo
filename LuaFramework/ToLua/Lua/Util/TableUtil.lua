...
-- 设置数组长度
--若数组有空洞或非连续索引（如 {1, nil, 3}），# 可能返回错误长度（例如返回 1 而非 3）。此时 array.n 可作为手动记录长度的替代方案
---@param array table 目标数组
---@param n number 手动记录长度
...
---@generic K, V 从哈希表查找指定值，返回其键，没找到返回nil
---@param hashtable table<K, V> 哈希表
...
-- 找出符合条件的第一个项：不对原表执行操作
-- return value , key 按照老bingo的返回顺序
...
-----------------------------------------------------------------------------------------------------
--------------------------------------  table转换string  --------------------------------------------
function table.toStringEx(value)
...
function table.tableToStr(tab)
	...
	for key, value in pairs(tab) do
		...
		if key == i then
			retstr = retstr .. signal .. (table.toStringEx(value))
		else
			if type(key) == 'number' or type(key) == 'string' then
				retstr = retstr .. signal .. '[' .. (table.toStringEx(key)) .. "]=" .. (table.toStringEx(value))
			else
				if type(key) == 'userdata' then
					retstr = retstr .. signal .. "*s" .. (table.toStringEx(key)) .. "*e" ..
						"=" .. (table.toStringEx(value))
				else
					retstr = retstr .. signal .. key .. "=" .. (table.toStringEx(key))
				end
			end
		end
		i = i + 1
