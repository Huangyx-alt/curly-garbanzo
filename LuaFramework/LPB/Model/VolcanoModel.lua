...
local VolcanoModel = BaseGameModel:New()
local this = VolcanoModel
local private = {}

------需要子类重写，设置参数------------------------------------------------------------------------------
function VolcanoModel:InitModeOptions()
    self.gameType_ = PLAY_TYPE.PLAY_TYPE_VOLCANO_PATH
    self.cityPlayID_ = 22
end

function VolcanoModel:GetQuestItemOffsetPos(itemId)
...
--音符道具投放位置
function VolcanoModel:GetWinZoneItemOffsetPos()
    return -31.7, 26.7
end

------------------------------------------------------------------------------------------------------

function VolcanoModel:InitData()
    self:InitModeOptions()
    self:SetSelfGameType(self.gameType_)
end

function VolcanoModel:ReqEnterGame()
    ...
    local procedure = require "Procedure/ProcedureVolcano"
    ...
    self.SendMessage(MSG_ID.MSG_GAME_VOLCANO_PATH, data)
end

function VolcanoModel:ReqSignCard(info)
    local cardIds = {}
...
function VolcanoModel:RefreshRoundDataByIndex(cardid, cellIndex, signType, need_fly_item, mark)
    --父类处理
...
function VolcanoModel.ResponeEnterGame(code, data)
    private.OnResEnterGame(this)
    if (code == RET.RET_SUCCESS) then
        this.dragonController = require("Combat.BattleLogic.Volcano.VolcanoDragonController")
...
function VolcanoModel:ResBingoInfo(code, data)
    if (code == RET.RET_SUCCESS) then
...
function VolcanoModel:OnReceiveSettle()
    if self:GetGameState() < GameState.ShowSettle then
        self:ShowSettle()
    end
end

function VolcanoModel:Clear()
    table.each(self.loop_delay_checks, function(v)
        LuaTimer:Remove(v)
    end)
    
    if self.dragonController then
        self.dragonController:Clear()
        self.dragonController = nil
    end
...
function VolcanoModel:IsBingoShowComplete()
    local check = this.__index:IsBingoShowComplete()

    --cell被盖章后飞出道具及做动画的时间
    local checkTime = os.time() - self.cardSignLogicDelayTime > 2.5
    
    local checkDragonMove = self.dragonController and self.dragonController:HaveMovingCtrl()
    if not checkDragonMove then
        checkDragonMove = self.dragonController and self.dragonController:HaveMoveTarget()
    end
    return check and checkTime and not checkDragonMove
end

--- 需要等特效还有bingo画面啥的都得等播放完再弹出结算界面
--- 战斗结算前，检查彩球叫号器
function VolcanoModel:CheckJokerBallSprayer(callBack)
...
function VolcanoModel:SetRoundGiftData(card_id, cell_index, new_gift, number, skill_id, self_bingo, powerId, extraPos)
...
function VolcanoModel:UploadGameData(gameType, quiteType)
...
function VolcanoModel:ReqGameReady()
    BattleMachineList.StartBattleMachine()
    if self.dragonController then
        self.dragonController:CheckInit()
        self.dragonController:Start()
    end
...
---4066上传数据时检查是否有效
function VolcanoModel:CheckHideGiftUpload(cellData, itemID)
...
function VolcanoModel:PreCalculateBingoSkill()
...
function VolcanoModel:CheckBingoSkillExtraPosValid(cardId, extraPos)
    local havePathBySkillRet = {} --通过技能位置盖章导致通路的龙
    local ret = {}
    table.each(extraPos, function(v)
        ...
        if self.dragonController then
            local dragon = self.dragonController:GetDragon(cardId, cellID)
            if dragon then
                checkPath = dragon:CheckHavePathToEnd()
                if checkPath then
                    table.insert(havePathBySkillRet, cellID)
                end
            end
        end
        
        if not checkPath and not cellData.isGained and not cellData.isMoveTarget then
    ...
    if #cells == 0 then
        --格子无效
        --如果有龙是因为技能打通路径，那么可以使用这些龙来播放技能
        if #havePathBySkillRet > 0 then
            return havePathBySkillRet
        end
        return ret
    end
...
this.BaseMsgIdList = {
    { msgid = MSG_ID.MSG_GAME_VOLCANO_PATH, func = this.ResponeEnterGame },
}
