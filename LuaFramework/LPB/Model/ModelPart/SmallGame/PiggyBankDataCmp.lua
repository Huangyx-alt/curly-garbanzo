...
--- DateTime: 2025/2/10 15:11
---
local PiggyBankDataCmp = {}
local this = PiggyBankDataCmp

function PiggyBankDataCmp:Init()
    self:RegEvent()
end


function PiggyBankDataCmp:RegEvent()
...
function PiggyBankDataCmp:UnRegEvent()
    -- if(self.MsgIdList )then
    --     for k1, v1 in ipairs(self.MsgIdList) do
...
--刮刮乐玩法，收到摇球机数据
function PiggyBankDataCmp.ResponeMansionInfo(code, data)
...
--刮刮乐玩法，返回摇奖结果
function PiggyBankDataCmp.ResponeMansionSpin(code, data)
    -- if code == RET.RET_SUCCESS then
...
function PiggyBankDataCmp:ReqSGInfo(...)
...
function PiggyBankDataCmp:ClearData()
...
function PiggyBankDataCmp:SaveScratchBallShakerData(data)
...
--在小火箭界面,需要用到的小游戏数据
function PiggyBankDataCmp:GetSmallGameData()
    log.log("小猪需要的代码 GetSmallGameData")
    return nil
end

--在正式结算之后,需要用到的小游戏数据
function PiggyBankDataCmp:GetSmallGameData2()
...
--获取其他数据的接口
function PiggyBankDataCmp:GetExtraSmallGameData()
