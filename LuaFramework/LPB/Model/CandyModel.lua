...
---  宝石皇后玩法
local CandyModel = BaseGameModel:New()
local this = CandyModel
local private = {}

------需要子类重写，设置参数------------------------------------------------------------------------------
function CandyModel:InitModeOptions()
    self.gameType_ = PLAY_TYPE.PLAY_TYPE_LOOK_FOR_CANDY
    self.cityPlayID_ = 13
end

function CandyModel:GetQuestItemOffsetPos()
    return 29, -29
end

------------------------------------------------------------------------------------------------------

function CandyModel:InitData()
    self:InitModeOptions()
    self:SetSelfGameType(self.gameType_)
end

function CandyModel:ReqEnterGame()
    ...
    local procedure = require "Procedure/ProcedureCandy"
    ...
    self.SendMessage(MSG_ID.MSG_GAME_LOAD_LOOK_FOR_CANDY, data)
end

function CandyModel:ReqSignCard(info)
    local cardIds = {}
...
function CandyModel:RefreshRoundDataByIndex(cardid, cellIndex, signType, need_fly_item, mark)
    --父类处理
...
function CandyModel.ResponeEnterGame(code, data)
...
function CandyModel:ResBingoInfo(code, data)
    if (code == RET.RET_SUCCESS) then
...
function CandyModel:OnReceiveSettle()
    if self:GetGameState() < GameState.ShowSettle then
        self:ShowSettle()
    end
end

function CandyModel:Clear()
...
function CandyModel:IsBingoShowComplete()
...
--- 战斗结算前，检查彩球叫号器
function CandyModel:CheckJokerBallSprayer(callBack)
...
function CandyModel:SetRoundGiftData(card_id, cell_index, new_gift, number, skill_id, self_bingo, powerId, extraPos)
...
function CandyModel:UploadGameData(gameType, quiteType)
...
---提前计算好猪技能的数据，上传到服务器
function CandyModel:PreCalcuatePigSkill()
...
function CandyModel:CheckBingoSkillExtraPosValid(cardId, extraPos)
    local ret, type = {}
...
this.BaseMsgIdList = {
    { msgid = MSG_ID.MSG_GAME_LOAD_LOOK_FOR_CANDY, func = this.ResponeEnterGame },
}
