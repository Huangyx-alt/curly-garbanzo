...
---  宝石皇后玩法
local PiggyBankModel = BaseGameModel:New()
local this = PiggyBankModel
local private = {}

------需要子类重写，设置参数------------------------------------------------------------------------------

function PiggyBankModel:GetQuestItemOffsetPos()
    return 29, -29
end
...
function PiggyBankModel:InitData()
    self:SetSelfGameType(PLAY_TYPE.PLAY_TYPE_PIGGY_BANK)
end

function PiggyBankModel:ReqSignCard(info)
    local cardIds = {}
...
function PiggyBankModel:ResEnterGame(code, data)
    private.OnResEnterGame(self)
    BaseGameModel.ResEnterGame(self, code, data)
end

function PiggyBankModel:OnReceiveSettle()
    if self:GetGameState() < GameState.ShowSettle then
        self:ShowSettle()
    end
end

function PiggyBankModel:Clear()
...
function PiggyBankModel:IsBingoShowComplete()
...
--- 战斗结算前，检查彩球叫号器
function PiggyBankModel:CheckJokerBallSprayer(callBack)
...
function PiggyBankModel:UploadGameData(gameType, quiteType, cb)
    if self.roundData ~= nil and self.roundData.GetRoundData ~= nil then
        self:PreCalcuatePigSkill()
    end
    this.__index.UploadGameData(self, gameType, quiteType, cb)
end

---提前计算好猪技能的数据，上传到服务器
function PiggyBankModel:PreCalcuatePigSkill()
...
function PiggyBankModel:CheckBingoSkillExtraPosValid(cardId, extraPos)
    local ret, type = {}
...
function PiggyBankModel:CollectCardBingo(cardId, diceType)
    log.r("PiggyBankModelcoo1 ",cardId,diceType)
...
---@param diceType number 骰子类型 1-普通骰子 2-额外骰子
---@param diceIndex number 骰子索引, 1-5
function PiggyBankModel:CheckCardBingo(cardId, diceType, diceIndex)
    log.r("PiggyBankModelcoo2 ",cardId,diceType)
    CalculateBingoMachine:SetInfo(2, cardId, diceType, diceIndex)
end

---@param cardId number
---bingo效果结束后调用
function PiggyBankModel:CheckCardBingo2(cardId)
    log.r("PiggyBankModelcoo3 ",cardId)
    CalculateBingoMachine.SeekInfo(1, cardId)
end

function PiggyBankModel:UpdateUpLoadDataExt(ext)
    ext = JsonToTable(ext)
    local bingoView = ModelList.BattleModel:GetCurrBattleView()
    if bingoView then
        ...
        local cardView = bingoView:GetCardView()
        if cardView then
            local bingoItems = cardView:GetUpgradeBingoItems()
...
function PiggyBankModel:IsTriggerExtraReward()
    ...
    if not maxRate or not curRate then
        log.log("PiggyBankModel:IsTriggerExtraReward 判断条件未达成", maxRate, curRate)
        return false
    end
...
function PiggyBankModel:GetExtraRewardTipIconSprite()
    local curSprite = AtlasManager:GetSpriteByName("PiggyBankHallAtlas", "PiggyBankTitleSdm")
    return curSprite
end
...
function private.GetCellItemType(cellData)
    ...
    table.each(cellData.hide_gift, function(v2)
        if 4 == math.floor(v2.id / 10000000) then
            local itemId = Csv.GetData("new_item_synthetic", v2.id, "itemid")
...
this.BaseMsgIdList = {
    { msgid = MSG_ID.MSG_GAME_LOAD_PIGGY_BANK, func = this.ResponeEnterGame },
}
