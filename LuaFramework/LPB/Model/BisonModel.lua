...
---野牛踩踏玩法
local BisonModel = BaseGameModel:New()
local this = BisonModel
local private = {}

------需要子类重写，设置参数------------------------------------------------------------------------------
function BisonModel:InitModeOptions()
    self.gameType_ = PLAY_TYPE.PLAY_TYPE_BISON
    self.cityPlayID_ = 27
end

function BisonModel:GetQuestItemOffsetPos()
    return 26.5, -32
end

------------------------------------------------------------------------------------------------------

function BisonModel:InitData()
    self:InitModeOptions()
    self:SetSelfGameType(self.gameType_)
end

function BisonModel:ReqEnterGame()
    ...
    local procedure = require "Procedure/ProcedureBison"
    ...
    self.SendMessage(MSG_ID.MSG_GAME_BISON, data)
end

function BisonModel:ReqSignCard(info)
    local cardIds = {}
...
function BisonModel:RefreshRoundDataByIndex(cardid, cellIndex, signType, need_fly_item, mark)
    --父类处理
...
--接受后端返回的对局数据并进行初始化
function BisonModel.ResponeEnterGame(code, data)
    private.OnResEnterGame(this)
    if (code == RET.RET_SUCCESS) then
        log.log("BisonModel.ResponeEnterGame bingoRuleId ", data.bingoRuleId)
...
function BisonModel:ResBingoInfo(code, data)
    if (code == RET.RET_SUCCESS) then
...
function BisonModel:OnReceiveSettle()
    if self:GetGameState() < GameState.ShowSettle then
        self:ShowSettle()
    end
end

function BisonModel:Clear()
...
function BisonModel:IsBingoShowComplete()
...
--- 战斗结算前，检查彩球叫号器
function BisonModel:CheckJokerBallSprayer(callBack)
...
function BisonModel:SetRoundGiftData(card_id, cell_index, new_gift, number, skill_id, self_bingo, powerId, extraPos)
...
function BisonModel:UploadGameData(gameType, quiteType)
...
function BisonModel:PlayBattleOverSound()
    UISound.play("minerbattleover")
end

---提前计算好猪技能的数据，上传到服务器
function BisonModel:PreCalcuatePigSkill()
...
function BisonModel:CheckBingoSkillExtraPosValid(cardId, extraPos)
    local ret, type = {}
...
function BisonModel:GetCurrentCollectLevel()
    local level = self:GetBattleExtraInfo("longStage")
    log.log("BisonModel:GetCurrentCollectLevel the level is ", level)
    level = level and tonumber(level) or 1
    return level
end

function BisonModel:GetBisonImageNameByIdxAndLevel(idx, level)
    return "Gold" ..level .. "Bullion0" .. idx
end

function BisonModel:GetCollectItemMapByCardId(cardId)
    if not cardId then
        return
    end
    local itemMaps = self:GetBattleExtraInfo("bingoItemMaps")
...
function BisonModel:GetCollectiveMaxCount(collectType, collectLevel)
    return 1
end
-----------------私有方法----------------------------------------------
...
this.BaseMsgIdList = {
    { msgid = MSG_ID.MSG_GAME_BISON, func = this.ResponeEnterGame },
}
