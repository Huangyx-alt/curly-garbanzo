...
---  宝石皇后玩法
local GemQueenModel = BaseGameModel:New()
local this = GemQueenModel
local private = {}

------需要子类重写，设置参数------------------------------------------------------------------------------
function GemQueenModel:InitModeOptions()
    self.gameType_ = PLAY_TYPE.PLAY_TYPE_JEWEL_QUEEN
    self.cityPlayID_ = 12
end

function GemQueenModel:GetQuestItemOffsetPos()
    return 26.5, -32
end

------------------------------------------------------------------------------------------------------

function GemQueenModel:InitData()
    self:InitModeOptions()
    self:SetSelfGameType(self.gameType_)
end

function GemQueenModel:ReqEnterGame()
    ...
    local procedure = require "Procedure/ProcedureGemQueen"
    ...
    self.SendMessage(MSG_ID.MSG_GAME_LOAD_JEWEL_QUEEN, data)
end

function GemQueenModel:ReqSignCard(info)
    local cardIds = {}
...
function GemQueenModel:RefreshRoundDataByIndex(cardid, cellIndex, signType, need_fly_item, mark)
    --父类处理
...
function GemQueenModel.ResponeEnterGame(code, data)
...
function GemQueenModel:ResBingoInfo(code, data)
    if (code == RET.RET_SUCCESS) then
...
function GemQueenModel:OnReceiveSettle()
    if self:GetGameState() < GameState.ShowSettle then
        self:ShowSettle()
    end
end

function GemQueenModel:Clear()
...
function GemQueenModel:IsBingoShowComplete()
...
--- 战斗结算前，检查彩球叫号器
function GemQueenModel:CheckJokerBallSprayer(callBack)
...
function GemQueenModel:SetRoundGiftData(card_id, cell_index, new_gift, number, skill_id, self_bingo, powerId, extraPos)
...
function GemQueenModel:UploadGameData(gameType, quiteType)
...
---提前计算好猪技能的数据，上传到服务器
function GemQueenModel:PreCalcuatePigSkill()
...
function GemQueenModel:CheckBingoSkillExtraPosValid(cardId, extraPos)
    local ret, type = {}
...
this.BaseMsgIdList = {
    { msgid = MSG_ID.MSG_GAME_LOAD_JEWEL_QUEEN, func = this.ResponeEnterGame },
}
