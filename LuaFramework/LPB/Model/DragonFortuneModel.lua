...
---  宝石皇后玩法
local DragonFortuneModel = BaseGameModel:New()
local this = DragonFortuneModel
local private = {}

------需要子类重写，设置参数------------------------------------------------------------------------------
function DragonFortuneModel:InitModeOptions()
    self.gameType_ = PLAY_TYPE.PLAY_TYPE_GOLDEN_DRAGON
    self.cityPlayID_ = 19
end

function DragonFortuneModel:GetQuestItemOffsetPos()
    return 29, -29
end

------------------------------------------------------------------------------------------------------

function DragonFortuneModel:InitData()
    self:InitModeOptions()
    self:SetSelfGameType(self.gameType_)
end

function DragonFortuneModel:ReqEnterGame()
    ...
    local procedure = require "Procedure/ProcedureDragonFortune"
    ...
    self.SendMessage(MSG_ID.MSG_GAME_LOAD_GOLDEN_DRAGON, data)
end

function DragonFortuneModel:ReqSignCard(info)
    local cardIds = {}
...
function DragonFortuneModel:RefreshRoundDataByIndex(cardid, cellIndex, signType, need_fly_item, mark)
    --父类处理
...
function DragonFortuneModel.ResponeEnterGame(code, data)
...
function DragonFortuneModel:ResBingoInfo(code, data)
    if (code == RET.RET_SUCCESS) then
...
function DragonFortuneModel:OnReceiveSettle()
    if self:GetGameState() < GameState.ShowSettle then
        self:ShowSettle()
    end
end

function DragonFortuneModel:Clear()
...
function DragonFortuneModel:IsBingoShowComplete()
...
--- 战斗结算前，检查彩球叫号器
function DragonFortuneModel:CheckJokerBallSprayer(callBack)
...
function DragonFortuneModel:SetRoundGiftData(card_id, cell_index, new_gift, number, skill_id, self_bingo, powerId, extraPos)
...
function DragonFortuneModel:UploadGameData(gameType, quiteType)
...
---提前计算好猪技能的数据，上传到服务器
function DragonFortuneModel:PreCalcuatePigSkill()
...
function DragonFortuneModel:CheckBingoSkillExtraPosValid(cardId, extraPos)
    local ret, type = {}
...
this.BaseMsgIdList = {
    { msgid = MSG_ID.MSG_GAME_LOAD_GOLDEN_DRAGON, func = this.ResponeEnterGame },
}
