...
local ShopModel = BaseModel:New("ShopModel")
local this = ShopModel
...
function ShopModel:CancelInitData()
    Timer:Stop()
end

function ShopModel:SetLoginData(data)
    for key, value in pairs(data.groceryInfo) do
        groceryList[value.id] = deep_copy(value)
    end
end

function ShopModel:GetPayData()
...
function ShopModel:CheckFreeRewardAvailable()
...
function ShopModel:OnTimerCall(sub)
...
function ShopModel:ClearShopData()
    --不能清空，红点需要用到数据
    --shopData = {}
    --shopDataDic = {}
end

function ShopModel:GetShopDataById(id)
    if id then
...
function ShopModel:GetShopData(index)
    local temData = {}
    temData["top"] = {}
    temData["main"] = {}
...
function ShopModel.C2S_FetchShopInfo(shopType)
...
function ShopModel.Login_C2S_FetchShopInfo(shopType)
    cur_shopType = shopType
    --红点需要用到相关数据，所有没有清空数据，打开刷新吧
    --if shopData[shopType] then
        --Facade.SendNotification(NotifyName.ShopView.FetchShopDataResut)
...
function ShopModel.C2S_RefreshShopInfo(shopType)
    this.SendMessage(MSG_ID.MSG_FETCH_SHOP,{index = shopType or cur_shopType})
    this.OnFetchShopResult_Notify = function()
...
function ShopModel.S2C_FetchShopResult(code,data)
    if code == RET.RET_SUCCESS and data then
        shopData[data.index] = {}
        shopData[data.index]["top"] = {}
        shopData[data.index]["main"] = {}

        this:UpdatePromotionInfo(data.index, data)

        local top = deep_copy(data.top)
        ...
        for key, value in pairs(top) do
            ...
            table.insert(shopData[data.index]["top"],value.id)
            count = count + 1
        end
        for key, value in pairs(main) do
            shopDataDic[value.id] = value
            table.insert(shopData[data.index]["main"],value.id)
            count = count + 1
        end
        if count >= 2 then
...
function ShopModel:UpdatePromotionInfo(index, data)
    if not index then
        log.log("ShopModel:UpdatePromotionInfo index is nil")
        return
    end

    if not data then
        log.log("ShopModel:UpdatePromotionInfo data is nil")
        return
    end
...
function ShopModel:IsPromotionValid(index)
...
function ShopModel:GetPromotionRemainTime(index)
    if not promoInfo then
        return 0
    end
...
function ShopModel.C2S_RequestBuyItem(shopItemId)
    this.cacheShopItemId = shopItemId

    this.SendMessage(MSG_ID.MSG_BUY_ITEM,{shopItemId = shopItemId})
end

function ShopModel.S2C_ResponeBuyItem(code,data)
    Event.Brocast(EventName.Event_ShopDataUpdata)
...
function ShopModel.get_pay_channel()
...
function ShopModel.C2S_NotifyServerIAPSuccess(orderId,productId,payload,token,currency,receipt,success_callback,failure_callback,errorcode)
...
function ShopModel.S2C_ResponeServerIAPSuccess(code,data)
...
function ShopModel.S2C_NotifyShopItemUpdata(code,data)
...
function ShopModel.Purchasing_success()
...
function ShopModel.Purchasing_failure()
    if this.cacheShopItemId then
...
function ShopModel.C2S_RequestActivityPay(activityId,activityName)
    this.SendMessage(MSG_ID.MSG_ACTIVITY_PAY,{itemId = activityId, activityName = activityName})
end

function ShopModel.S2C_ResponeActivityPay(code,data)
    if code == RET.RET_SUCCESS and data then
        if data.code == 0 then
...
function ShopModel.C2S_RequestGroceryNotify()
    this.SendMessage(MSG_ID.MSG_GROCERY_NOTIFY,{})
end

function ShopModel.S2C_ResponeGroceryNotify(code,data)
    if code == RET.RET_SUCCESS and data then
...
function ShopModel.C2S_RequestBuyGrocery(id)
    this.SendMessage(MSG_ID.MSG_BUY_GROCERY,{id =id})
end

function ShopModel.S2C_ResponeBuyGrocery(code,data)
    if code == RET.RET_SUCCESS and data then
        this.BuyId = data.id
...
function ShopModel.Purchasing_Grocery_success()
    log.r("购买礼包成功")
...
function ShopModel.Purchasing_Grocery_failure()
    log.r("购买礼包失败")
    Facade.SendNotification(NotifyName.Grocery.GroceryBuyFailure, this.BuyId )
end

function ShopModel.GetGroceryInfo(id,params)
...
function ShopModel.ResetNewPayCallBack(success_callback,failure_callback)
    this._iap_success_callback = success_callback
    this._iap_failure_callback = failure_callback
end
...
--- 音符兑换返回
function ShopModel.C2S_VICTORY_BEATS_NOTE_EXCHANGE(itemId)
    this.SendMessage(MSG_ID.MSG_VICTORY_BEATS_NOTE_EXCHANGE,{itemId = itemId},false)
end

--- 音符兑换返回
function ShopModel.S2C_Respone_VICTORY_BEATS_NOTE_EXCHANGE(code,data)
...
function ShopModel.Purchasing_failure_new()
    if this.cacheShopItemId then
        Facade.SendNotification(NotifyName.ShopView.BuyFailureReshone,this.cacheShopItemId)
        --SDK.purchase_failed({itemType = this.cacheShopItemId})
    end
