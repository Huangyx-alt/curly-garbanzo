...
---鼹鼠矿工玩法
local MoleMinerModel = BaseGameModel:New()
local this = MoleMinerModel
local private = {}

------需要子类重写，设置参数------------------------------------------------------------------------------
function MoleMinerModel:InitModeOptions()
    self.gameType_ = PLAY_TYPE.PLAY_TYPE_MINING
    self.cityPlayID_ = 26
end

function MoleMinerModel:GetQuestItemOffsetPos()
    return 26.5, -32
end

------------------------------------------------------------------------------------------------------

function MoleMinerModel:InitData()
    self:InitModeOptions()
    self:SetSelfGameType(self.gameType_)
end

function MoleMinerModel:ReqEnterGame()
    ...
    local procedure = require "Procedure/ProcedureMoleMiner"
    ...
    self.SendMessage(MSG_ID.MSG_GAME_MINING, data)
end

function MoleMinerModel:ReqSignCard(info)
    local cardIds = {}
...
function MoleMinerModel:RefreshRoundDataByIndex(cardid, cellIndex, signType, need_fly_item, mark)
    --父类处理
...
function MoleMinerModel.ResponeEnterGame(code, data)
    private.OnResEnterGame(this)
    if (code == RET.RET_SUCCESS) then
        log.log("MoleMinerModel.ResponeEnterGame bingoRuleId ", data.bingoRuleId)
...
function MoleMinerModel:ResBingoInfo(code, data)
    if (code == RET.RET_SUCCESS) then
...
function MoleMinerModel:OnReceiveSettle()
    if self:GetGameState() < GameState.ShowSettle then
        self:ShowSettle()
    end
end

function MoleMinerModel:Clear()
...
function MoleMinerModel:IsBingoShowComplete()
...
--- 战斗结算前，检查彩球叫号器
function MoleMinerModel:CheckJokerBallSprayer(callBack)
...
function MoleMinerModel:SetRoundGiftData(card_id, cell_index, new_gift, number, skill_id, self_bingo, powerId, extraPos)
...
function MoleMinerModel:UploadGameData(gameType, quiteType)
...
function MoleMinerModel:PlayBattleOverSound()
    UISound.play("minerbattleover")
end

---提前计算好猪技能的数据，上传到服务器
function MoleMinerModel:PreCalcuatePigSkill()
...
function MoleMinerModel:CheckBingoSkillExtraPosValid(cardId, extraPos)
    local ret, type = {}
...
function MoleMinerModel:GetCurrentCollectLevel()
...
function MoleMinerModel:GetMineralImageNameByIdxAndLevel(idx, level)
...
this.BaseMsgIdList = {
    { msgid = MSG_ID.MSG_GAME_MINING, func = this.ResponeEnterGame },
}
