...
---64位so 暂时没有编译进去,先注释
local pb = require "Utils.protoc"

function parser.register(fileset , path)
	local all = {}
	local files = {}
    --pb:addpath("D:/project/svn/1.28/Assets/MyPlugins/LuaFramework/Lua")
	--pb:addpath(Util.LuaResPath)
	--pb:addpath(Util.LuaResPath.."/net/")
	--pb:addpath(Util.LuaPath)
	--pb:addpath(Util.LuaPath.."/net/")
	pb:addpath(path)
	if type(fileset) == "string" then
		fileset = { fileset }
	end
	for _, filename in ipairs(fileset) do
		--log.r("parser.register",filename)
        pb:loadfile(filename..".proto")
	end
