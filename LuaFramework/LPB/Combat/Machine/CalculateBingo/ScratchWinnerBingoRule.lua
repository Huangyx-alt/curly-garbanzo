...
local ScratchWinnerBingoRule = BaseBingoRule:New("ScratchWinnerBingoRule")
local this = ScratchWinnerBingoRule
--setmetatable(ScratchWinnerBingoRule, {__index = BaseBingoRule})
...
function ScratchWinnerBingoRule:Start(loadData, wish_item)
    bowlCount = {}
    wishItems = {}
    currWishList = {}
    if wish_item then
    ...
    for i = 1, cardCount do
        bowlCount[i] = {
            drinkCount = { 0, 0, 0, 0, 1}, --4个酒杯的酒量，满4就bingo
            bingo = {}, --能形成bingo的酒杯ID
    ...
    MaxWaterCount = {}
    for i = 1, 5 do
        local maxCount = Csv.GetCollectiveMaxCount(i)
        MaxWaterCount[i] = maxCount
    end
    this.model = ModelList.BattleModel:GetCurrModel()
end

function ScratchWinnerBingoRule:CalculateBingo(cardId, cellIndex, ...)
...
function ScratchWinnerBingoRule:GetBingoType(cardId, cellIndex)
...
function ScratchWinnerBingoRule:OnDataChange(cardId, cellIndex, ...)
    local arg = {...}
...
function ScratchWinnerBingoRule:CalculateJackpot(cardId)
    if bowlCount[cardId].jackpot == 1 then
        return false 
    end
    return bowlCount[cardId].drinkCount[5] >= MaxWaterCount[5]
end

function ScratchWinnerBingoRule:ForceBingo()
    local max = 0
    local type = 0
    for i = 1, #bowlCount do
        if bowlCount[i] > max and bowlCount[i] < MaxWaterCount[i] then
            max = bowlCount[i]
            type = i
        end
    end
...
--- 计算是否出现wish,
function ScratchWinnerBingoRule:CalculateCardWish(cardId)
