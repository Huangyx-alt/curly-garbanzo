...
local ScratchWinnerLogicCard = BaseLogicCard:New("ScratchWinnerLogicCard")
local this = ScratchWinnerLogicCard
this.moduleName = "ScratchWinnerLogicCard"
...
function ScratchWinnerLogicCard:InitLogic()
...
function ScratchWinnerLogicCard:InitCardBowls()
...
function ScratchWinnerLogicCard:Register()
    Event.AddListener(EventName.Event_Logic_Collect_Item, self.AddLogicBowlDrink, self)
end

function ScratchWinnerLogicCard:UnRegister()
    Event.RemoveListener(EventName.Event_Logic_Collect_Item, self.AddLogicBowlDrink, self)
end

--- 增加酒水数量
function ScratchWinnerLogicCard:AddLogicBowlDrink(cardId, cellIndex, bowlType, addCount, isMax)
...
function ScratchWinnerLogicCard:IsOtherSideHaveOneJackpot(currCardId)
...
function ScratchWinnerLogicCard:IsTypeBingo(cardID, type)
    cardID = tonumber(cardID)
...
function ScratchWinnerLogicCard:IsAllJackpot()
    local cardCount = self.model:GetCardCount()
    for i = 1, cardCount do
...
function ScratchWinnerLogicCard:CheckAllCardJackpotHandle(currCardId)
...
function ScratchWinnerLogicCard:CheckDoubleJackpotHandle(currCardId)
    if ModelList.BattleModel:IsRocket() then return end
...
--- 取消两个Jackpot的卡牌后移
function ScratchWinnerLogicCard:StopCardBackMove()
...
--- 获取最接近满杯的酒杯类型
function ScratchWinnerLogicCard:GetBowlCount(cardId)
    cardId = tonumber(cardId)
...
---
function ScratchWinnerLogicCard:GetMaxCount(cardId)
    return maxCount
end

--- 获取卡牌bingo数量
function ScratchWinnerLogicCard:GetBowlBingoCount(cardId)
    cardId = tonumber(cardId)
    local bingoCount = 0
...
--- 获取收集进度最快的材料
function ScratchWinnerLogicCard:GetFasterBowlType(cardId, excludeTypes)
    cardId = tonumber(cardId)
    local ret, temp
...
---收集进度最快的材料的所有格子
---@param cardId table
function ScratchWinnerLogicCard:GetSkillNeedCells(cardId, excludeTypes)
...
---获取类型材料对应的格子
---@param cardId table
function ScratchWinnerLogicCard:GetCellsByMaterialType(cardId, type)
    cardId = tonumber(cardId)
