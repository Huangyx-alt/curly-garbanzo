...
local LeetoleManLogicCard = BaseLogicCard:New("LeetoleManLogicCard")
local this = LeetoleManLogicCard
this.moduleName = "LeetoleManLogicCard"

local bowlTable={}
--- 禁止点击卡牌
local forbidClickTable={}

function LeetoleManLogicCard:InitLogic()
...
function LeetoleManLogicCard:InitCardBowls()
...
function LeetoleManLogicCard:Register()
    Event.AddListener(EventName.Event_Logic_Collect_Item,self.AddLogicBowlDrink,self)
end

function LeetoleManLogicCard:UnRegister()
    Event.RemoveListener(EventName.Event_Logic_Collect_Item,self.AddLogicBowlDrink,self)
end

--- 增加酒水数量
function LeetoleManLogicCard:AddLogicBowlDrink(cardId,cellIndex,bowlType,addCount,isMax)
...
function LeetoleManLogicCard:IsOtherSideHaveOneJackpot(currCardId)
...
function LeetoleManLogicCard:IsTypeBingo(cardID, type)
    cardID = tonumber(cardID)
...
function LeetoleManLogicCard:IsAllJackpot()
    local cardCount = self.model:GetCardCount()
    for i = 1, cardCount do
...
function LeetoleManLogicCard:CheckAllCardJackpotHandle(currCardId)
...
function LeetoleManLogicCard:CheckDoubleJackpotHandle(currCardId)
    if ModelList.BattleModel:IsRocket() then return end
...
--- 取消两个Jackpot的卡牌后移
function LeetoleManLogicCard:StopCardBackMove()
...
--- 获取最接近满杯的酒杯类型
function LeetoleManLogicCard:GetBowlCount(cardId)
    cardId = tonumber(cardId)
...
---
function LeetoleManLogicCard:GetMaxCount(cardId)
    cardId = tonumber(cardId)
    local playid = ModelList.CityModel.GetPlayIdByCity()
    local maxCount1 = Csv.GetCollectiveMaxCount(1,playid)
    local maxCount2 = Csv.GetCollectiveMaxCount(2,playid)
...
--- 获取卡牌bingo数量
function LeetoleManLogicCard:GetBowlBingoCount(cardId)
    cardId = tonumber(cardId)
    local bingoCount = 0
...
--- 获取收集进度最快的材料
function LeetoleManLogicCard:GetFasterBowlType(cardId, excludeTypes)
    cardId = tonumber(cardId)
    local ret, temp
...
---收集进度最快的材料的所有格子
---@param cardId table
function LeetoleManLogicCard:GetSkillNeedCells(cardId, excludeTypes)
...
---获取类型材料对应的格子
---@param cardId table
function LeetoleManLogicCard:GetCellsByMaterialType(cardId, type)
    cardId = tonumber(cardId)
