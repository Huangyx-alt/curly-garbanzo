...
local NewChristmasLogicCard = BaseLogicCard:New("NewChristmasLogicCard")
local this = NewChristmasLogicCard
this.moduleName = "NewChristmasLogicCard"
local bowlTable = {}
local MaxCollectCount = {}

function NewChristmasLogicCard:InitLogic()
    self.model = ModelList.BattleModel:GetCurrModel()
    self:InitCardBowls()
    self:BaseInit()
end

function NewChristmasLogicCard:InitCardBowls()
...
function NewChristmasLogicCard:Register()
    Event.AddListener(EventName.Event_Logic_Collect_Item, self.AddLogicBowlDrink, self)
end

function NewChristmasLogicCard:UnRegister()
    Event.RemoveListener(EventName.Event_Logic_Collect_Item, self.AddLogicBowlDrink, self)
end

function NewChristmasLogicCard:AddLogicBowlDrink(cardId, cellIndex, bowlType, addCount, isMax)
...
function NewChristmasLogicCard:GetBingoType(cardId, bingoCount)
...
function NewChristmasLogicCard:IsOtherSideHaveOneJackpot(currCardId)
...
function NewChristmasLogicCard:IsTypeBingo(cardID, type)
    cardID = tonumber(cardID)
    local temp = bowlTable[cardID][type] or 0
    return temp >= MaxCollectCount[type]
end

function NewChristmasLogicCard:IsAllJackpot()
    local cardCount = self.model:GetCardCount()
    for i = 1, cardCount do
...
function NewChristmasLogicCard:CheckAllCardJackpotHandle(currCardId)
...
function NewChristmasLogicCard:CheckDoubleJackpotHandle(currCardId)
    if ModelList.BattleModel:IsRocket() then
        return
    end
...
function NewChristmasLogicCard:StopCardBackMove()
    if this.loopMoveCard then
        LuaTimer:Remove(this.loopMoveCard)
    end
end

function NewChristmasLogicCard:GetBowlCount(cardId)
    cardId = tonumber(cardId)
    for i = 1, #bowlTable[cardId] do
...
function NewChristmasLogicCard:GetMaxCount(cardId)
    return MaxCollectCount
end

function NewChristmasLogicCard:GetBowlBingoCount(cardId)
    cardId = tonumber(cardId)
    local bingoCount = 0
...
function NewChristmasLogicCard:GetFasterBowlType(cardId, excludeTypes)
    cardId = tonumber(cardId)
    local ret, temp
...
function NewChristmasLogicCard:GetSkillNeedCells(cardId, excludeTypes)
...
function NewChristmasLogicCard:GetCellsByMaterialType(cardId, type)
    cardId = tonumber(cardId)
