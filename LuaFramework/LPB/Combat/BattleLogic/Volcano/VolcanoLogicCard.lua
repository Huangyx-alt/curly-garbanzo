...
local VolcanoLogicCard = BaseLogicCard:New("VolcanoLogicCard")
local this = VolcanoLogicCard
this.moduleName = "VolcanoLogicCard"
local bowlTable = {}
local MaxCollectCount = {}

function VolcanoLogicCard:InitLogic()
    self.model = ModelList.BattleModel:GetCurrModel()
    self:InitCardBowls()
    self:BaseInit()
end

function VolcanoLogicCard:InitCardBowls()
...
function VolcanoLogicCard:GetBingoType(cardId, bingoCount)
...
function VolcanoLogicCard:IsOtherSideHaveOneJackpot(currCardId)
...
function VolcanoLogicCard:IsTypeBingo(cardID, type)
    cardID = tonumber(cardID)
    local temp = bowlTable[cardID][type] or 0
    return temp >= MaxCollectCount[type]
end

function VolcanoLogicCard:IsAllJackpot()
    local cardCount = self.model:GetCardCount()
    for i = 1, cardCount do
...
function VolcanoLogicCard:CheckAllCardJackpotHandle(currCardId)
...
function VolcanoLogicCard:CheckDoubleJackpotHandle(currCardId)
    if ModelList.BattleModel:IsRocket() then
        return
    end
...
function VolcanoLogicCard:StopCardBackMove()
    if this.loopMoveCard then
        LuaTimer:Remove(this.loopMoveCard)
    end
end

function VolcanoLogicCard:GetBowlCount(cardId)
    cardId = tonumber(cardId)
    return bowlTable[cardId]
end

function VolcanoLogicCard:GetMaxCount(cardId)
    return MaxCollectCount
end

function VolcanoLogicCard:GetBowlBingoCount(cardId)
    cardId = tonumber(cardId)
    local bingoCount = 0
...
function VolcanoLogicCard:GetNextMaterialCell(cardId, isPreCalculateBingoSkill)
    cardId = tonumber(cardId)
    local curModel = ModelList.BattleModel:GetCurrModel()
    local cardData = curModel:GetRoundData(cardId)
    local ret = {}
    table.each(cardData.cards, function(cell)
        table.each(cell.hide_gift, function(v)
            local cfg = Csv.GetData("item", v.id, "result")
            if cfg and cfg[1] == 44 then
                --cell.isGained 用于判断格子是否已经被打开
                --cell.isUsedForBingoSkill 用于判断格子是否被技能占用
                --cell.isMoveTarget 用于判断格子是否即将被打开

                --检查龙是否可以到达火山
                local checkPath = false
                if curModel.dragonController then
                    local dragon = curModel.dragonController:GetDragon(cardId, cell.index)
                    if dragon then
                        checkPath = dragon:CheckHavePathToEnd()
                    end
                end
                
                if not checkPath and not cell.isGained and not cell.isMoveTarget and not cell.isUsedForBingoSkill then
