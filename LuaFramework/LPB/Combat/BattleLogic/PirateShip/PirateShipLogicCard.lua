...
local PirateShipLogicCard = BaseLogicCard:New("PirateShipLogicCard")
local this = PirateShipLogicCard
this.moduleName = "PirateShipLogicCard"
local bowlTable = {}
local MaxCollectCount = {}

function PirateShipLogicCard:InitLogic()
    self.model = ModelList.BattleModel:GetCurrModel()
    self:InitCardBowls()
    self:BaseInit()
end

function PirateShipLogicCard:InitCardBowls()
...
function PirateShipLogicCard:Register()
    Event.AddListener(EventName.Event_Logic_Collect_Item, self.AddLogicBowlDrink, self)
end

function PirateShipLogicCard:UnRegister()
    Event.RemoveListener(EventName.Event_Logic_Collect_Item, self.AddLogicBowlDrink, self)
end

function PirateShipLogicCard:AddLogicBowlDrink(cardId, cellIndex, bowlType, addCount, isMax)
...
function PirateShipLogicCard:GetBingoType(cardId, bingoCount)
...
function PirateShipLogicCard:IsOtherSideHaveOneJackpot(currCardId)
...
function PirateShipLogicCard:IsTypeBingo(cardID, type)
    cardID = tonumber(cardID)
    local temp = bowlTable[cardID][type] or 0
    return temp >= MaxCollectCount[type]
end

function PirateShipLogicCard:IsAllJackpot()
    local cardCount = self.model:GetCardCount()
    for i = 1, cardCount do
...
function PirateShipLogicCard:CheckAllCardJackpotHandle(currCardId)
...
function PirateShipLogicCard:CheckDoubleJackpotHandle(currCardId)
    if ModelList.BattleModel:IsRocket() then
        return
    end
...
function PirateShipLogicCard:StopCardBackMove()
    if this.loopMoveCard then
        LuaTimer:Remove(this.loopMoveCard)
    end
end

function PirateShipLogicCard:GetBowlCount(cardId)
    cardId = tonumber(cardId)
    return bowlTable[cardId]
end

function PirateShipLogicCard:GetMaxCount(cardId)
    return MaxCollectCount
end

function PirateShipLogicCard:GetBowlBingoCount(cardId)
    cardId = tonumber(cardId)
    local bingoCount = 0
...
function PirateShipLogicCard:GetNextMaterialCell(cardId, isPreCalculateBingoSkill)
    cardId = tonumber(cardId)
    ...
    table.each(cardData.cards, function(cell)
        table.each(cell.hide_gift, function(v)
            ...
            if cfg and cfg[1] == 41 then
                --cell.isGained �����жϸ����Ƿ��Ѿ�����
                --cell.isUsedForBingoSkill �����жϸ����Ƿ񱻼���ռ�ã���������ͷ���淨����Ч
                --cell.isMoveTarget �����жϸ����Ƿ񼴽����򿪣���������ͷ���淨����Ч
