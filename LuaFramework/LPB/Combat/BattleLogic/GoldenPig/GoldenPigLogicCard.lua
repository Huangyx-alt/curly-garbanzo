...
local GoldenPigLogicCard = BaseLogicCard:New("GoldenPigLogicCard")
local this = GoldenPigLogicCard
this.moduleName = "GoldenPigLogicCard"
local bowlTable = {}
local MaxCollectCount = {}

function GoldenPigLogicCard:InitLogic()
    self.model = ModelList.BattleModel:GetCurrModel()
    self:InitCardBowls()
    self:BaseInit()
end

function GoldenPigLogicCard:InitCardBowls()
...
function GoldenPigLogicCard:GetBingoType(cardId, bingoCount)
...
function GoldenPigLogicCard:IsOtherSideHaveOneJackpot(currCardId)
...
function GoldenPigLogicCard:IsTypeBingo(cardID, type)
    cardID = tonumber(cardID)
    local temp = bowlTable[cardID][type] or 0
    return temp >= MaxCollectCount[type]
end

function GoldenPigLogicCard:IsAllJackpot()
    local cardCount = self.model:GetCardCount()
    for i = 1, cardCount do
...
function GoldenPigLogicCard:CheckAllCardJackpotHandle(currCardId)
...
function GoldenPigLogicCard:CheckDoubleJackpotHandle(currCardId)
    if ModelList.BattleModel:IsRocket() then
        return
    end
...
function GoldenPigLogicCard:StopCardBackMove()
    if this.loopMoveCard then
        LuaTimer:Remove(this.loopMoveCard)
    end
end

function GoldenPigLogicCard:GetBowlCount(cardId)
    cardId = tonumber(cardId)
    for i = 1, #bowlTable[cardId] do
...
function GoldenPigLogicCard:GetMaxCount(cardId)
    return MaxCollectCount
end

function GoldenPigLogicCard:GetBowlBingoCount(cardId)
    cardId = tonumber(cardId)
    local bingoCount = 0
...
function GoldenPigLogicCard:GetFasterBowlType(cardId, excludeTypes)
    cardId = tonumber(cardId)
    local ret, temp
...
function GoldenPigLogicCard:GetSkillNeedCells(cardId, excludeTypes)
...
function GoldenPigLogicCard:GetCellsByMaterialType(cardId, type)
    cardId = tonumber(cardId)
