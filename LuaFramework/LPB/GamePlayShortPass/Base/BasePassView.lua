...
local BasePassView = class("BasePassView",BaseViewEx)
local this = BasePassView
this.viewType = CanvasSortingOrderManager.LayerType.TopConsole
local csv_data_name = "task_pass"
function BasePassView:ctor(id)
    self.id = id 
end
...
function BasePassView:BuildFsm()
    self:DisposeFsm()
    self._fsm = Fsm.CreateFsm("BasePassView",self,{
...
function BasePassView:DisposeFsm()
    if self._fsm then
        self._fsm:Dispose()
        self._fsm = nil
    end
...
function BasePassView:GetModel()
...
function BasePassView:HideCoverageEntity()
    self:NotifyHideCoverageEntity()
end

function BasePassView:ShowCoverageEntity()
    self:NotifyShowCoverageEntity()
end

function BasePassView:GetRewardLevel()
...
function BasePassView:Awake(obj)
    self:on_init()
end

function BasePassView:OnEnable()
    ModelList.GuideModel:OpenUI("BasePassView")
...
function BasePassView:InitIcon()
...
function BasePassView:OnDestroy()
...
function BasePassView:InitGoldPassInfo()
...
function BasePassView:OnDisable()
...
function BasePassView:SetMaskItem(item, posY)
...
function BasePassView:UpdateCrossLinePos()
    local y = 0 - (self:GetModel():GetLevel() * itemHeight) - initialOffset
    log.log("BasePassView:UpdateCrossLinePos", self:GetModel():GetLevel(), y)
    fun.set_gameobject_pos(self.crossLine.gameObject,0, y, 0, true)
    fun.set_gameobject_pos(self.cover_mask.gameObject,0, y, 0, true)
    --self.cover_mask.sizeDelta = Vector2.New(930, content_height)
end

function BasePassView:SetExp(isFirst)
...
function BasePassView:RecursionShowExpAnimation(level,toLevel,exp,toExp)
    local curLevel = self:GetModel():GetLevel()
...
function BasePassView:PlayBingoPassEnter()
...
function BasePassView:PlayBingoPassExit()
    AnimatorPlayHelper.Play(self.anima,{"end","BingoPassView_endlgreen"},false,function()
        self:CloseSelf()
    end)
end

function BasePassView:DisableDragAbility()
    self.scrollView.enabled = false
end

function BasePassView:EnableDragAbility()
    self.scrollView.enabled = true
end

function BasePassView:on_btn_help_click()
...
function BasePassView:on_btn_activate_click() 
    if(isdebug)then 
...
function BasePassView:OnShowPurchaseView()
...
function BasePassView:AfterRecommendClose(closeMethod)
...
function BasePassView:on_btn_close_click()
    self._fsm:GetCurState():PlayExit(self._fsm)  --debug lxq
    -- Facade.SendNotification(NotifyName.CloseUI,self)
end

-- function BasePassView:on_btn_bigowin_click()
...
function BasePassView:OnUpdataBingoPassInfo()
...
function BasePassView:OnBingoPassWatchAd(passItem)
    self._fsm:GetCurState():BaseGamePassBaseWatchAd(self._fsm,passItem)
end

function BasePassView:OnGemUnlockLevel(passItem)
    self._fsm:GetCurState():GemUnlockLevel(self._fsm,passItem)
end

function BasePassView:OnClaimReward(passItem)
    self._fsm:GetCurState():ClaimReward(self._fsm,passItem)
end

function BasePassView:OnReceiveReward()
    if self:GetModel():IsAnyPayment() then
...
function BasePassView:OnReceiveRewardFinish()
...
function BasePassView:ReceiveRewardPaid()
...
function BasePassView:ReceiveRewardFree()
    -- local rewards = self:GetModel():GetRewardList()
...
function BasePassView:OnExpired()
    self._fsm:GetCurState():Expired(self._fsm)
end

function BasePassView:CloseSelf()
...
function BasePassView:SetCloseCallback(callback)
    self.closeCallback = callback
end

function BasePassView:SetSkipReopenHallCityView(isSkip)
    self.skipReopenHallCityView = isSkip
end

function BasePassView:OnActivateGoldenPassPayResult(code)
...
function BasePassView:Purchasing_success()
...
function BasePassView:Purchasing_failure()
    self:MutualTaskFinish()
end

function BasePassView:OnRefresh()
    self:Purchasing_success()
end

function BasePassView:RegisterUIEVent()
    Event.AddListener(EventName.Event_ShortPassView_Refresh, self.OnRefresh, self)
end

function BasePassView:UnRegisterUIEVent()
