...
this.atlasName = "HorseRacingPurchaseAtlas"
...
function PassPurchaseView:CloseSelf(closeMethod)
    if closeMethod == self.CloseMethod.normal and self.isActiveByPopup then
        log.log("Facade.SendNotification(NotifyName.GamePlayShortPassView.PopupHorseRacingPassPurchaseOrderFinish) 1")
        Event.Brocast(NotifyName.GamePlayShortPassView.PopupHorseRacingPassPurchaseOrderFinish)
    end
...
function PassPurchaseView:Purchasing_success()
    ...
    if self.isActiveByPopup then
        params.onCloseCallback = function()
            log.log("Facade.SendNotification(NotifyName.GamePlayShortPassView.PopupHorseRacingPassPurchaseOrderFinish) 2")
            Event.Brocast(NotifyName.GamePlayShortPassView.PopupHorseRacingPassPurchaseOrderFinish)
        end
    end
