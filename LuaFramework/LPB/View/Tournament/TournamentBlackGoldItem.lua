...
local TournamentBlackGoldItem = TournamentCurrentItem:New()
local this = TournamentBlackGoldItem
...
--设置前3名玩家
function TournamentBlackGoldItem:SetTopPlayer()
	...
	if self._data and self._data.stageInfo and self._data.stageInfo.topPlayers then
		...
		for i, v in ipairs(topPlayers) do
			...
			local TournamentTopPlayerItem = require "View/Tournament/TournamentTopPlayerItem"
			TournamentTopPlayerItem:SetItem(item,v);
			
			--table.insert(self.topPlayer,TournamentTopPlayerItem);
		end
	end
end

--设置普通item
function TournamentBlackGoldItem:SetNormalItem()
...
--设置黑金段位
function TournamentBlackGoldItem:SetBlackGoldItem()
...
--设置玩家item
function TournamentBlackGoldItem:SetPlayer(item,playerInfo)
