...
local TournamentNoArriveItem = TournamentArriveItem:New()
local this = TournamentNoArriveItem
...
function TournamentNoArriveItem:New(isNowShowState,isInitState)
	local o = {}
	self.__index = self
...
function TournamentNoArriveItem:OnDispose()
...
function TournamentNoArriveItem:SetItem(data,index,startScore,isUpTier,name,curTiers)
	self._data = data;
	self.bgIndex = index;
...
function TournamentNoArriveItem:SetItemBg()
...
function TournamentNoArriveItem:SetTierInfo()
...
--设置阶段奖励
function TournamentNoArriveItem:SetStageItems(startScore)
	self.stageItemList = {};
	self._stageRratios = {};
	if self._data then
		...
		for i, v in ipairs(self._data.stageList) do
			...
			--log.log("dghdgh0007, TournamentNoArriveItem SetStageItems i, ratio, v.score, hight, sliderHeight, bottomHight, ", i, ratio, v.score, hight, sliderHeight, bottomHight)
...
--显示阶段人数
function TournamentNoArriveItem:ShowStagePlayerNum()
	local sliderHeight = fun.get_component(self.tierSlider,fun.RECT).rect.height;
...
function TournamentNoArriveItem:ShowChangePush()
...
function TournamentNoArriveItem:CalculatePos()
...
--显示第一到第五名
function TournamentNoArriveItem:ShowFirstStagePlayer()
...
function TournamentNoArriveItem:SetOnePlayer(data,posY,posX,childIndex,showName,index,isCurrent)
	local go=nil
	if isCurrent then
