--段位奖励item
local TournamentTierInfoItem = BaseView:New("TournamentTierInfoItem")
local this = TournamentTierInfoItem
...
function TournamentTierInfoItem:New()
	local o = {}
	self.__index = self
...
function TournamentTierInfoItem:Awake()
	self:on_init()
end

function TournamentTierInfoItem:OnEnable()
	self._init = true
	--self:OnShow();
end

function TournamentTierInfoItem:OnShow()
	
end

function TournamentTierInfoItem:OnDisable()
	self._init = nil
	self._data = nil
...
function TournamentTierInfoItem:SetStage(isArrive)
	if self.bgover then
...
function TournamentTierInfoItem:SetData(data,bgIndex)
	self._data = data;
	self.bgIndex = bgIndex;
	if self._init and self._data then
		
		--显示奖励
		--if self.isShowFirst==true then
			--显示奖杯图标
			...
			--显示积分
			local scoreRange = ModelList.TournamentModel:GetScoreRangeByTiers(self._data.tier)
...
function TournamentTierInfoItem:SetRewardItem()
...
function TournamentTierInfoItem:SetBg()
	--框
	if self.bg then
		self.bg.sprite = AtlasManager:GetSpriteByName("TournamentScoreAtlas",TournamentScoreBgConfig[self.bgIndex][8]);
	end
	self.trophyBg.sprite = AtlasManager:GetSpriteByName("TournamentScoreAtlas",TournamentScoreBgConfig[self.bgIndex][4]);
	self.txtBg.sprite = AtlasManager:GetSpriteByName("TournamentScoreAtlas",TournamentScoreBgConfig[self.bgIndex][5]);
	--背景
	if self.bg1 then
		self.bg1.sprite = AtlasManager:GetSpriteByName("TournamentScoreAtlas",TournamentScoreBgConfig[self.bgIndex][2]);
	end
	
	local isTrueGol = ModelList.TournamentModel:CheckIsTrueGoldUser();
	if self.bg3 then
		self.bg3.sprite = AtlasManager:GetSpriteByName("TournamentScoreAtlas",TournamentScoreBgConfig[self.bgIndex][12]);
		--if isTrueGol then
		...
		rewardBg.sprite = AtlasManager:GetSpriteByName("TournamentScoreAtlas",spName);
	end
...
--奖励飞行动画
function TournamentTierInfoItem:PlayTierAwardAnim(data,targetPos,callBack)
	UISound.play("weekly_round_up")
	for i = 1, #data do
		if not self.reward_item  then
			--打开 TournamentScoreView 生成全部地图时 会卡顿 self.reward_item 有可能出现空情况
