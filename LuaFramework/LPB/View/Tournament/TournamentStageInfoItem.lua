--阶段奖励item
local TournamentStageInfoItem = BaseView:New("TournamentStageInfoItem")
local this = TournamentStageInfoItem
...
function TournamentStageInfoItem:New()
	local o = {}
	self.__index = self
	setmetatable(o,self)
	return o
...
function TournamentStageInfoItem:Awake()
	self:on_init()
end

function TournamentStageInfoItem:OnEnable()
	self._init = true
	self._isPlayAnim = false;
end

function TournamentStageInfoItem:OnDisable()
	self._init = nil
	--self._data = nil
...
function TournamentStageInfoItem:SetLockVisible(data,myScore, startScore , forceLock)
...
function TournamentStageInfoItem:SetData(data,bgIndex,myScore,ratio,startScore,forceLock)
	self._data = data
...
function TournamentStageInfoItem:SetBg()
	self.bg.sprite = AtlasManager:GetSpriteByName("TournamentScoreAtlas",TournamentScoreBgConfig[self.bgIndex][8]);
	self.scoreBg.sprite = AtlasManager:GetSpriteByName("TournamentScoreAtlas",TournamentScoreBgConfig[self.bgIndex][9]);
	self.unLock.sprite = AtlasManager:GetSpriteByName("TournamentScoreAtlas",TournamentScoreBgConfig[self.bgIndex][7]);
	self.lock.sprite = AtlasManager:GetSpriteByName("TournamentScoreAtlas",TournamentScoreBgConfig[self.bgIndex][6]);
...
--奖励飞行动画
function TournamentStageInfoItem:FlyRewardToTopArea()
