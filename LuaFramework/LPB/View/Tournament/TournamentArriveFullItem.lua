...
local TournamentArriveFullItem = TournamentArriveItem:New()
local this = TournamentArriveFullItem

local TournamentTierInfoItem = require "View/Tournament/TournamentTierInfoItem"
local TournamentStageInfoItem = require "View/Tournament/TournamentStageInfoItem"
...
function TournamentArriveFullItem:New(isNowShowState,isInitState)
	local o = {}
	self.__index = self
...
function TournamentArriveFullItem:OnDispose()
...
function TournamentArriveFullItem:SetItem(data,index,startScore,isUpTier)
	self._data = data;
	self.bgIndex = index;
...
function TournamentArriveFullItem:SetItemBg()
...
function TournamentArriveFullItem:SetTierInfo()
...
function TournamentArriveFullItem:SetMyInfo(startScore)
...
function TournamentArriveFullItem:PlayTierItemAnim(animName)
...
function TournamentArriveFullItem:SetSliderBg(isRush)
...
--Slider动画
function TournamentArriveFullItem:PlaySliderAnim(startScore,startRatio)
...
--设置阶段奖励
function TournamentArriveFullItem:SetStageItems(startScore)
	self.stageItemList = {};
	self._stageRratios = {};
	if self._data then
		...
		for i, v in ipairs(self._data.stageList) do
			...
			if go then
				...
				local view = TournamentStageInfoItem:New();
...
--显示阶段人数
function TournamentArriveFullItem:ShowStagePlayerNum()
	local sliderHeight = fun.get_component(self.tierSlider,fun.RECT).rect.height;
...
function TournamentArriveFullItem:CalculatePos()
...
--显示第一到第五名
function TournamentArriveFullItem:ShowFirstStagePlayer()
...
function TournamentArriveFullItem:SetOnePlayer(data,posY,posX,childIndex,showName,index)
	local go=nil
	if self.isNowShowState==true and self.isChangeShowState==true then
		if #HeadObjectPool>0 then
			go = ObjectPop(2)
			fun.set_parent(go,self.root)
			fun.set_active(go,true)
		else
...
--计算scrollView的垂直滑动值
function TournamentArriveFullItem:CalculateScrollPercent(startScore)
...
function TournamentArriveFullItem:HasSprintBuff()
...
function TournamentArriveFullItem:ShowChangePush()
