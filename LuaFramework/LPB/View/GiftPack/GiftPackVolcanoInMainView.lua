...
local GiftPackVolcanoInMainView = GiftPackBaseView:New('GiftPackVolcanoInMainView', "GiftPackNewYearAtlas")
local this = GiftPackVolcanoInMainView
...
function GiftPackVolcanoInMainView:Awake(obj)
    self:on_init()
...
function GiftPackVolcanoInMainView:OnEnable()
    Facade.RegisterView(self)
    --   this:ClearMailList()
    self:BuildFsm(self)
    self:Bi_Tracker()
...
function GiftPackVolcanoInMainView:OnDisable()
...
function GiftPackVolcanoInMainView:ClearCountDown()
...
function GiftPackVolcanoInMainView:ShowItemGrid()
    ...
    self.wxlbjinglong03:ShowSprite(xls.icon)
end

function GiftPackVolcanoInMainView:ShowItemGridData(step,flag)
    _data = ModelList.GiftPackModel:GetShowGiftPack()
...
function GiftPackVolcanoInMainView:movetoGrid(step,tflag)
    step = step or 1
    local rect = fun.get_component(self.item,fun.RECT)
    local itemHeight = rect.rect.height
    local y = step-1<=0 and 50 or  (((step-1) * itemHeight)-topHeight)
    ...
    if tflag then
        ...
        -- local itemView = ItemEntityCache[step]
        -- if(itemView and fun.is_null(itemView.go)==false)then 
        --     y = math.abs(fun.get_localposition_y(itemView.go)) -rect.rect.height
        -- end
    end
 
    log.r("lxq movetoGrid",y)
    Anim.move_ease(self.Content,pos.x , y  ,0,0.5,true,DG.Tweening.Ease.Flash,function()

    end)
end

function GiftPackVolcanoInMainView:ClearMailList()
...
function GiftPackVolcanoInMainView:GetItemViewInstance(index)
...
function GiftPackVolcanoInMainView:ShowDetailGift()
...
function GiftPackVolcanoInMainView:SetLeftTime(endTimeStamp)
...
function GiftPackVolcanoInMainView:OnDestroy()
    log.g("GiftPackVolcanoInMainView.OnDestroy ing !")
    Facade.RemoveView(self)
    self:Destroy()
    self:ClearMailList()
end

function GiftPackVolcanoInMainView:CloseFunc()
    Facade.SendNotification(NotifyName.HideUI, this)
end

function GiftPackVolcanoInMainView:ClosePackView()
    if _Delopptimer2 ~= nil then
        LuaTimer:Remove(_Delopptimer2)
    end
...
function GiftPackVolcanoInMainView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackVolcanoInMainView:OnBuySuccess(needClose,itemData)
...
function GiftPackVolcanoInMainView:showGetRewardEffect(itemData,isFree)
...
function GiftPackVolcanoInMainView:SetTimer()
...
--领取成功
function GiftPackVolcanoInMainView.OnClaimEndlessGiftSuccess(data)
...
--领取失败
function GiftPackVolcanoInMainView.OnClaimEndlessGiftFail(data)
    --提示下领取失败
end

--查询成功
function GiftPackVolcanoInMainView.OnFetchEndlessGiftSuccess(data)
    selfstep = data.step
    local tmpflag = false
...
--领取失败
function GiftPackVolcanoInMainView.OnFetchEndlessGiftFail(data)
    --提示下查询失败
end
