...
local GiftPackMemberView = GiftPackBaseView:New("GiftPackMemberView")
local this = GiftPackMemberView
...
function GiftPackMemberView:New(viewName, atlasName)
    local o = {}
    o.viewName = viewName
    o.atlasName = atlasName
    setmetatable(o,{__index = GiftPackMemberView})
    return o
end

function GiftPackMemberView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackMemberView:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    UISound.play("saleboth")
end

function GiftPackMemberView:OnDisable()
...
function GiftPackMemberView:ClearCountDown()
...
function GiftPackMemberView:CloseUIShiny(obj)
...
function GiftPackMemberView:GetLastClickTime()
    return lastClickTime or 0
end

function GiftPackMemberView:on_btn_buy1_click()
...
function GiftPackMemberView:on_btn_buy2_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackMemberView:on_btn_buy3_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackMemberView:on_btn_buy_all_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackMemberView:SetExtra(extraUI, extraParent, bonus)
...
function GiftPackMemberView:SetItemGrid(itemId, itemNum, itemGrid)
...
function GiftPackMemberView:ShowItemGrid()
...
function GiftPackMemberView:ClearItems()
    for i = 1, 3 do
...
function GiftPackMemberView:SetItem(itemUI, id, pos, btn)
...
function GiftPackMemberView:SetLastItem(id, pos)
    local xls = Csv.GetData("pop_up", id)
    if xls then
...
function GiftPackMemberView:GetItemGrid(pos,index,prefab, parent)
...
function GiftPackMemberView:HideGridItem()
    for k, v in pairs(itemList) do
        for z, w in pairs(v) do
            if w then
...
function GiftPackMemberView:ShowDetailGift()
...
function GiftPackMemberView:SetButton(button, id, pos)
...
function GiftPackMemberView:SetLeftTime(endTimeStamp)
...
function GiftPackMemberView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackMemberView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, self)
end


function GiftPackMemberView:ClosePackView()
...
function GiftPackMemberView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackMemberView:OnBuySuccess(needClose, itemData)
    self:ChangeState(self, "GiftPackShowState", itemData)
end

function GiftPackMemberView:GetGiftPos(id)
    ...
    elseif id == product_list[4] then
        ...
        log.log("GiftPackMemberView:GetGiftPos(id) id error", id)
        return self:GetItemPos(itemListPos.Up)
    end
end

function GiftPackMemberView:GetItemPos(pos)
    local posTable ={}
    if pos ~= 4 then
