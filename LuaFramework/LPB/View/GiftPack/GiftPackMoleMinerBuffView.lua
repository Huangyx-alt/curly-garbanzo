...
---@class  GiftPackMoleMinerBuffView: GiftPackBaseView
local GiftPackMoleMinerBuffView = GiftPackBaseView:New('GiftPackMoleMinerBuffView', "GiftPackMoleMinerBuffAtlasInMain")
local this = GiftPackMoleMinerBuffView
...
function GiftPackMoleMinerBuffView:Awake(obj)
    this:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackMoleMinerBuffView.OnEnable()
    this:BuildFsm(this)
    this:Bi_Tracker()
end

function GiftPackMoleMinerBuffView:on_x_update()
end

function GiftPackMoleMinerBuffView:ShowDetailGift()
...
function GiftPackMoleMinerBuffView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackMoleMinerBuffView.GetItemOrResource(id, param)
    if id > 1000 then
...
function GiftPackMoleMinerBuffView:SetLeftProduct(id)
...
function GiftPackMoleMinerBuffView:SetRightProduct(id)
    local xls = Csv.GetData("pop_up", id)
...
function GiftPackMoleMinerBuffView:SetLeftTime(endTimeStamp)
...
function GiftPackMoleMinerBuffView:SetContent(id, txt, content)
    id = tonumber(id)
    if fun.is_null(txt) then
        return
    end
    if id == 23 or id == 24 or id == 902602 then
...
function GiftPackMoleMinerBuffView.OnDisable()
    this:stop_x_update()
end

function GiftPackMoleMinerBuffView.OnDestroy()
    this:Destroy()
end

function GiftPackMoleMinerBuffView:on_close()
    this:stop_x_update()
end

function GiftPackMoleMinerBuffView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, this)
end

function GiftPackMoleMinerBuffView:CutDonwTarget()
    this:CloseFunc()
    this.main_effect:Play("end")
end

function GiftPackMoleMinerBuffView:on_btn_close_click()
    ...
    Facade.SendNotification(NotifyName.HideUI, ViewList.GiftPackMoleMinerBuffView)
    ModelList.GiftPackModel:CloseView()
end

this.last_click_time1 = 0
function GiftPackMoleMinerBuffView:on_btn_left_buy_click()
...
function GiftPackMoleMinerBuffView:on_btn_right_buy_click()
    if this._fsm:GetCurName() ~= "GiftPackEnterState" then return end
    if canDownBuy then
        local click_time = UnityEngine.Time.time
...
function GiftPackMoleMinerBuffView:CloseUIShiny(obj)
...
function GiftPackMoleMinerBuffView:SetUIImageGray(btn_obj,active)
    if active then
...
function GiftPackMoleMinerBuffView:OnBuySuccess(needClose,itemData)
    this:ChangeState(this,"GiftPackShowState",itemData)
end

function GiftPackMoleMinerBuffView:GetGiftPos(id)
