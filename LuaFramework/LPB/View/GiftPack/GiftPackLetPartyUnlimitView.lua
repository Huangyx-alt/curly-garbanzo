...
local GiftPackLetPartyUnlimitView = GiftPackBaseView:New('GiftPackLetPartyUnlimitView')
local this = GiftPackLetPartyUnlimitView
...
function GiftPackLetPartyUnlimitView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackLetPartyUnlimitView:BuildFsm(owner)
    FsmCreator:Create("GiftPackLetPartyUnlimitView",owner,{
...
function GiftPackLetPartyUnlimitView:OnEnable()
    ...
    Event.AddListener(EventName.UnlimitSale_Click_Purchase, self.SetClickPurchaseState, self)
end



function GiftPackLetPartyUnlimitView:OnDisable()
    ...
    self:ClearCountDown() 
    self:CleanChildView()
    Event.RemoveListener(EventName.UnlimitSale_Click_Purchase, self.SetClickPurchaseState, self)
end


function GiftPackLetPartyUnlimitView:CleanChildView()
...
function GiftPackLetPartyUnlimitView:ShowDetailGift()
...
function GiftPackLetPartyUnlimitView:ShowItemGrid()
    data = ModelList.GiftPackModel:GetShowGiftPack()
    log.log("lxq GiftPackLetPartyUnlimitView",data)
    self:CollectProduct(data.pId,data.giftInfo) 
    self:InitShopItem()
end
 

function GiftPackLetPartyUnlimitView:CleanShopItem()
...
function GiftPackLetPartyUnlimitView:InitShopItemPos()
...
function GiftPackLetPartyUnlimitView:InitShopItem()
    self:CleanShopItem()
...
function GiftPackLetPartyUnlimitView:ClearCountDown()
...
function GiftPackLetPartyUnlimitView:CloseUIShiny(obj)
...
function GiftPackLetPartyUnlimitView:on_btn_up_buy_click()
...
function GiftPackLetPartyUnlimitView:GetLastClickTime(pos)
    return clickTimeList or 0
end


function GiftPackLetPartyUnlimitView:SetItemGrid(itemId, itemNum , itemGrid)
...
function GiftPackLetPartyUnlimitView:GetItemGrid(pos,index,prefab, parent)
...
function GiftPackLetPartyUnlimitView:HideGridItem()
...
function GiftPackLetPartyUnlimitView:SetButton(button, id,pos)
...
function GiftPackLetPartyUnlimitView:SetLeftTime(endTimeStamp)
...
function GiftPackLetPartyUnlimitView:CollectProduct(pId,giftInfo)
    product_list = {}
    ...
    log.log("整理的商品数据 " , product_list)
    table.sort(product_list)
end

function GiftPackLetPartyUnlimitView.OnDestroy()
    this:Destroy()
end

function GiftPackLetPartyUnlimitView:on_close()
    --Event.Brocast(Notes.CARD_COLLIDER_OPEN, true)
end

function GiftPackLetPartyUnlimitView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, this)
end

function GiftPackLetPartyUnlimitView:ClosePackView()
...
function GiftPackLetPartyUnlimitView:on_btn_close_click()
    self:ClosePackView()
end

function  GiftPackLetPartyUnlimitView:OnBuySuccess(needClose,itemData)
...
function  GiftPackLetPartyUnlimitView:GetGiftPos(id)
...
function GiftPackLetPartyUnlimitView:SetItemVisiable(giftPackSweetmeatsItemView,visiable)
...
function GiftPackLetPartyUnlimitView:GetItemPos(pos)
     return self.fullItemPos[pos]
end

function GiftPackLetPartyUnlimitView:GetItemViewInstance(index)
...
function GiftPackLetPartyUnlimitView:GetLocalGiftData()
...
function GiftPackLetPartyUnlimitView:ShowItemGridData(step,isShow)
    local count = 0
    local tempData = ModelList.GiftPackModel:GetShowGiftPack()
    ...
    --
    --local iconName = Csv.GetData("pop_up", tempData.giftInfo[1].id, "icon")
    --if iconName == "cEntChildrensDaydiamond" then
    --    self.lbvalentineslove11:ShowSprite("childrensdaysale04wxlb")
    --else
    --    self.lbvalentineslove11:ShowSprite("childrensdaysale05wxlb")
    --end
end



function GiftPackLetPartyUnlimitView:FullItemGridData(step,view,pos)
    local tempData = ModelList.GiftPackModel:GetShowGiftPack()
    local tmpData =self:GetLocalGiftData()
    local count = 0 
    for _,v in ipairs(tmpData) do 
...
--查询成功
function GiftPackLetPartyUnlimitView.OnFetchEndlessGiftSuccess(data)
...
function GiftPackLetPartyUnlimitView.TestMove()
...
function GiftPackLetPartyUnlimitView:MoveItem(step) 
    if(isMoveing)then 
        log.log("是否移动 isMoveing",isMoveing)
        return 
    end
    isMoveing = true 
...
function GiftPackLetPartyUnlimitView:MoveToLast(view)
    local pos = 6
    local i = 6
    while i>1 do
...
function GiftPackLetPartyUnlimitView:MoveAllItem(flyTime,delay) 
    local delayCout = 0 
    for i=1,5 do
...
function GiftPackLetPartyUnlimitView:SetViewByPos(view,pos)
    ItemEntityCache[pos] = view
end
function GiftPackLetPartyUnlimitView:HideAllItemDir()
    for k,v in pairs(ItemEntityCache) do
        v:SetDirection(7)  --隐藏所有方向 
    end
end
function GiftPackLetPartyUnlimitView:ShowAllItemDir()
    local islastPos = 1
    local isFull = true 
    for k,v in pairs(ItemEntityCache) do
...
function GiftPackLetPartyUnlimitView:RemoveItemViewByPos(index)
...
function GiftPackLetPartyUnlimitView:on_btn_up_Free_click() --免费的
...
function GiftPackLetPartyUnlimitView:showGetRewardEffect(itemData,isFree)
...
function GiftPackLetPartyUnlimitView:CheckAutoClose()
...
--领取成功
function GiftPackLetPartyUnlimitView.OnClaimEndlessGiftSuccess(data)
 
    local tmpdata  ={ itemData = data.reward,step = data.step}
    this:showGetRewardEffect(tmpdata,true)
    log.r("lxq  GiftPackLetPartyUnlimitView.OnClaimEndlessGiftSuccess 领取成功")
    --现实第一个
end

--领取失败
function GiftPackLetPartyUnlimitView.OnClaimEndlessGiftFail(data)
    --提示下领取失败
    log.r("lxq GiftPackLetPartyUnlimitView.OnClaimEndlessGiftFail")
end

--领取失败
function GiftPackLetPartyUnlimitView.OnFetchEndlessGiftFail(data)
    --提示下查询失败
    log.r("lxq GiftPackLetPartyUnlimitView.OnFetchEndlessGiftFail")
end

function GiftPackLetPartyUnlimitView:SetClickPurchaseState()
    self:SetClickPurchase()
end
