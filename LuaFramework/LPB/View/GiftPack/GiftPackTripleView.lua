...
---@class  GiftPackTripleView: GiftPackBaseView
local GiftPackTripleView = GiftPackBaseView:New('GiftPackTripleView', "GiftPackAtlas")
local this = GiftPackTripleView
...
function GiftPackTripleView:Awake(obj)
...
function GiftPackTripleView.OnEnable()
...
function GiftPackTripleView:on_x_update()
...
function GiftPackTripleView:ShowDetailGift()
...
function GiftPackTripleView:CollectProduct(pId,giftInfo)
    product_list = {}
...
function GiftPackTripleView:SetUpProduct(id)
...
function GiftPackTripleView:SetContent(id,txt,content)
...
function GiftPackTripleView:SetDownLeftProduct(id)
    local xls = Csv.GetData("pop_up", id)
    local item1 = xls.item_description[1][1]
...
function GiftPackTripleView:SetDownRightProduct(id)
    local xls = Csv.GetData("pop_up", id)
    local item1 = xls.item_description[1][1]
...
function GiftPackTripleView:SetLeftTime(endTimeStamp)
...
function GiftPackTripleView:SetBounsAndMore(obj, bonus, more)
...
function GiftPackTripleView.OnDisable()
    --Facade.RemoveView(this)
    this:stop_x_update()
    --Event.Brocast(Notes.CARD_COLLIDER_OPEN, true)
end

function GiftPackTripleView.OnDestroy()
    this:Destroy()
end

function GiftPackTripleView:on_close()
    this:stop_x_update()
    --Event.Brocast(Notes.CARD_COLLIDER_OPEN, true)
end

function GiftPackTripleView:CloseFunc()
    --this.main_effect:Play("end")
    Facade.SendNotification(NotifyName.HideDialog, this)

end

function GiftPackTripleView:ShowTip(tipName)

end

function GiftPackTripleView:InitTip()

end

function GiftPackTripleView:CutDonwTarget()
    this:CloseFunc()
    this.main_effect:Play("end")
end

function GiftPackTripleView:on_btn_sure_click()
...
function GiftPackTripleView:on_btn_cancle_click()
...
function GiftPackTripleView:on_btn_close_click()
    if this._fsm and this._fsm:GetCurName() == "GiftPackShowState" then return end
    if this.loopTime then
        LuaTimer:Remove(this.loopTime)
        this.loopTime = nil
    end
    Facade.SendNotification(NotifyName.CloseUI, ViewList.GiftPackTripleView)
    ModelList.GiftPackModel:CloseView()
end

function GiftPackTripleView:on_btn_buy_click()
end

this.last_click_time1 = 0
function GiftPackTripleView:on_btn_up_buy_click()
...
function GiftPackTripleView:on_btn_bl_buy_click()
    if this._fsm:GetCurName() ~= "GiftPackEnterState" then return end
    if canBLClick then
        local click_time = UnityEngine.Time.time
...
function GiftPackTripleView:on_btn_br_buy_click()
    if this._fsm:GetCurName() ~= "GiftPackEnterState" then return end
    if canBRClick then
        local click_time = UnityEngine.Time.time
...
function GiftPackTripleView:CloseUIShiny(obj)
...
function  GiftPackTripleView:OnBuySuccess(needClose,itemData)
    this:ChangeState(this,"GiftPackShowState",itemData)
end

function  GiftPackTripleView:GetGiftPos(id)
    if id == product_list[1] then
        --return this.up_reward_icon1.transform.position,this.up_reward_icon2.transform.position,
        --this.up_reward_icon3.transform.position,this.cityExp1.transform.position
        return fun.get_gameobject_pos(this.up_reward_icon1,false),fun.get_gameobject_pos(this.up_reward_icon2,false),
                fun.get_gameobject_pos(this.up_reward_icon3,false),fun.get_gameobject_pos(this.cityExp1,false)

    elseif id == product_list[2] then
        return fun.get_gameobject_pos(this.bl_reward_icon,false), fun.get_gameobject_pos(this.bl_cost_icon,false), fun.get_gameobject_pos(this.cityExp2,false),nil
    else
        return fun.get_gameobject_pos(this.br_reward_icon,false),    fun.get_gameobject_pos(this.br_cost_icon,false),
        fun.get_gameobject_pos(this.cityExp3.transform,false),nil
    end
end
