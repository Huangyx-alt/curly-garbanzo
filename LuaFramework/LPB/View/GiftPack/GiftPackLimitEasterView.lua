...
local GiftPackLimitEasterView = GiftPackBaseView:New("GiftPackLimitEasterView", "GiftPack_HotFixPack_GiftPackLimitEaster_AutoAtlasInMain")
local this = GiftPackLimitEasterView
...
function GiftPackLimitEasterView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackLimitEasterView:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    UISound.play("sale_ultrabet")
end

function GiftPackLimitEasterView:OnDisable()
...
function GiftPackLimitEasterView:ClearCountDown()
...
function GiftPackLimitEasterView:CloseUIShiny(obj)
...
function GiftPackLimitEasterView:GetLastClickTime()
    return lastClickTime or 0
end

function GiftPackLimitEasterView:on_btn_up_buy_click()
...
function GiftPackLimitEasterView:on_btn_buy2_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackLimitEasterView:SetExtra(extraUI, extraParent, bonus)
...
function GiftPackLimitEasterView:SetItemGrid(itemId, itemNum, itemGrid)
...
function GiftPackLimitEasterView:ShowItemGrid()
...
function GiftPackLimitEasterView:SetSellInfo(id,buyCount)
...
function GiftPackLimitEasterView:SetItem(itemUI, id, pos, btn)
...
function GiftPackLimitEasterView:SetCanBuyCount(cout)
...
function GiftPackLimitEasterView:GetItemGrid(pos,index,prefab, parent)
...
function GiftPackLimitEasterView:HideGridItem()
    for k, v in pairs(itemList) do
        for z, w in pairs(v) do
            if w then
...
function GiftPackLimitEasterView:ShowDetailGift()
...
function GiftPackLimitEasterView:SetButton(button, id, pos)
...
function GiftPackLimitEasterView:SetLeftTime(endTimeStamp)
...
function GiftPackLimitEasterView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackLimitEasterView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, self)
end

function GiftPackLimitEasterView:ClosePackView()
...
function GiftPackLimitEasterView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackLimitEasterView:OnBuySuccess(needClose, itemData)
    self:ChangeState(self, "GiftPackShowState", itemData)
end

function GiftPackLimitEasterView:GetGiftPos(id)
...
function GiftPackLimitEasterView:GetItemPos(pos)
    local posTable ={}
