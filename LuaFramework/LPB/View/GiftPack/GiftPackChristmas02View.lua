...
local GiftPackChristmas02View = GiftPackBaseView:New('GiftPackChristmas02View')
local this = GiftPackChristmas02View
...
function GiftPackChristmas02View:New()
    local o = {}
    setmetatable(o,{__index = GiftPackChristmas02View})
    return o
end

function GiftPackChristmas02View:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackChristmas02View:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    UISound.play("sale_joker")
end

function GiftPackChristmas02View:OnDisable()
...
function GiftPackChristmas02View:ClearCountDown()
...
function GiftPackChristmas02View:CloseUIShiny(obj)
...
function GiftPackChristmas02View:on_btn_buy1_click()
...
function GiftPackChristmas02View:GetLastClickTime(pos)
    return clickTimeList or 0
end

function GiftPackChristmas02View:on_btn_buy2_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then return end
...
function GiftPackChristmas02View:on_btn_buy3_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then return end
...
function GiftPackChristmas02View:SetExtra(extraUI,extraParent , bonus)
...
function GiftPackChristmas02View:SetItemGrid(itemId, itemNum , itemGrid)
...
function GiftPackChristmas02View:ShowItemGrid()
...
function GiftPackChristmas02View:SetItem(itemUI,id, pos, btn)
...
function GiftPackChristmas02View:GetItemGrid(pos,index,prefab, parent)
...
function GiftPackChristmas02View:HideGridItem()
    for k ,v in pairs(itemList) do
        for z,w in pairs(v) do
            if w then
...
function GiftPackChristmas02View:ShowDetailGift()
...
function GiftPackChristmas02View:SetButton(button, id,pos)
...
function GiftPackChristmas02View:SetLeftTime(endTimeStamp)
...
function GiftPackChristmas02View:CollectProduct(pId,giftInfo)
    product_list = {}
...
function GiftPackChristmas02View:OnDestroy()
    self:Destroy()
end

function GiftPackChristmas02View:on_close()
    --Event.Brocast(Notes.CARD_COLLIDER_OPEN, true)
end

function GiftPackChristmas02View:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, this)
end

function GiftPackChristmas02View:ClosePackView()
...
function GiftPackChristmas02View:on_btn_close_click()
    self:ClosePackView()
end

function  GiftPackChristmas02View:OnBuySuccess(needClose,itemData)
    self:ChangeState(self,"GiftPackShowState",itemData)
end

function  GiftPackChristmas02View:GetGiftPos(id)
...
function GiftPackChristmas02View:GetItemPos(pos)
    local posTable ={}
