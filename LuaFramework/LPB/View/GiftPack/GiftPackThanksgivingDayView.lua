...
local GiftPackThanksGivingDayView = GiftPackBaseView:New('GiftPackThanksGivingDayView')
local this = GiftPackThanksGivingDayView
...
function GiftPackThanksGivingDayView:Awake(obj)
    this:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackThanksGivingDayView.OnEnable()
    this:BuildFsm(this)
    this:Bi_Tracker()
end

function GiftPackThanksGivingDayView:on_x_update()
end

function GiftPackThanksGivingDayView:ShowDetailGift()
...
function GiftPackThanksGivingDayView:CollectProduct(pId, giftInfo)
    product_list = {}
...
local SetCityExp = function (vipText, item_description, count)
    if #item_description > count - 1 then
        fun.set_active(vipText.transform.parent, true)
        vipText.text = fun.formatNum(item_description[count][2])
    else
        --fun.set_active(btnObj, false)
    end
end

function GiftPackThanksGivingDayView.GetItemOrResource(id, param)
    if id > 1000 then
        return Csv.GetData("item", id, param), "ItemAtlas"
    else
        return Csv.GetData("resources", id, "more_icon"), "ItemAtlas"
    end
end

function GiftPackThanksGivingDayView:SetUpProduct(id)
    local xls = Csv.GetData("pop_up", id)
    local item1 = xls.item_description[1][1]
    local icon1, atlasName = self.GetItemOrResource(tonumber(item1), "more_icon")
    ...
    local icon2, atlasName = self.GetItemOrResource(tonumber(item2), "more_icon")
    ...
    this:SetBounsAndMore(1, xls.bonus, xls.more)
    SetBuyBtnDescripe(self.btn_up_buy, xls.limit_description)
    SetCityExp(this.vipexp1, xls.item_description, 3)
end

function GiftPackThanksGivingDayView:SetDownProduct(id)
    local xls = Csv.GetData("pop_up", id)
    local item1 = xls.item_description[1][1]
    local icon1, atlasName = self.GetItemOrResource(tonumber(item1), "more_icon")
    ...
    this:SetBounsAndMore(2, xls.bonus, xls.more)
    SetBuyBtnDescripe(self.btn_down_buy, xls.limit_description)
    SetCityExp(this.vipexp2, xls.item_description, 3)
end

function GiftPackThanksGivingDayView:SetLeftTime(endTimeStamp)
...
function GiftPackThanksGivingDayView:SetContent(id, txt, content)
    id = tonumber(id)
...
function GiftPackThanksGivingDayView:SetBounsAndMore(id, bonus, more)
    ...
    if bonus and  bonus > 0 then
        fun.set_active(this["extra"..id],true)
        this["textExtra"..id].text = bonus.."%"
    end
end

function GiftPackThanksGivingDayView.OnDisable()
    --Facade.RemoveView(this)
    this:stop_x_update()
    --Event.Brocast(Notes.CARD_COLLIDER_OPEN,true)
end

function GiftPackThanksGivingDayView.OnDestroy()
    this:Destroy()
end

function GiftPackThanksGivingDayView:on_close()
    this:stop_x_update()
    --Event.Brocast(Notes.CARD_COLLIDER_OPEN,true)
end

function GiftPackThanksGivingDayView:CloseFunc()
    --this.main_effect:Play("end")
    Facade.SendNotification(NotifyName.CloseUI, this)
end

function GiftPackThanksGivingDayView:CutDonwTarget()
    this:CloseFunc()
    this.main_effect:Play("end")
end

function GiftPackThanksGivingDayView:on_btn_close_click()
...
function GiftPackThanksGivingDayView:on_btn_up_buy_click()
...
function GiftPackThanksGivingDayView:on_btn_down_buy_click()
    if this._fsm:GetCurName() ~= "GiftPackEnterState" then return end
    if canDownBuy then
        local click_time = UnityEngine.Time.time
...
function GiftPackThanksGivingDayView:CloseUIShiny(obj)
...
function GiftPackThanksGivingDayView:OnBuySuccess(needClose, itemData)
    this:ChangeState(this, "GiftPackShowState", itemData)
end

function GiftPackThanksGivingDayView:GetGiftPos(id)
    ...
    elseif id == product_list[2] then
        ...
        return fun.get_gameobject_pos(this.down_reward_icon1, false),
            fun.get_gameobject_pos(this.down_reward_icon2, false), fun.get_gameobject_pos(vipexp, false), nil
    else
        local vipexp = fun.find_child(this.cityExp1, "vipexp")
