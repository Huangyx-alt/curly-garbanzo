...
--- DateTime: 15/7/2024 下午 12:28
---

--挖矿buff礼包
local GiftPackBaseView = require("View.GiftPack.GiftPackBaseView")
---@class  GiftPackScratchWinnerBuffView: GiftPackBaseView
local GiftPackScratchWinnerBuffView = GiftPackBaseView:New('GiftPackScratchWinnerBuffView', "GiftPackScratchWinnerBuffAtlas")
local this = GiftPackScratchWinnerBuffView
...
function GiftPackScratchWinnerBuffView:Awake(obj)
    this:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackScratchWinnerBuffView.OnEnable()
    this:BuildFsm(this)
    this:Bi_Tracker()
end

function GiftPackScratchWinnerBuffView:on_x_update()
end

function GiftPackScratchWinnerBuffView:ShowDetailGift()
...
function GiftPackScratchWinnerBuffView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackScratchWinnerBuffView.GetItemOrResource(id, param)
    if id > 1000 then
...
function GiftPackScratchWinnerBuffView:SetLeftProduct(id)
...
function GiftPackScratchWinnerBuffView:SetRightProduct(id)
    local xls = Csv.GetData("pop_up", id)
...
function GiftPackScratchWinnerBuffView:SetLeftTime(endTimeStamp)
...
function GiftPackScratchWinnerBuffView:SetContent(id, txt, content)
    id = tonumber(id)
    if fun.is_null(txt) then
        return
    end
    --- flag:短令牌 每次增加新的短令牌都要修改
    if id == 23 or id == 24 or id == 902902 then
...
function GiftPackScratchWinnerBuffView.OnDisable()
    this:stop_x_update()
end

function GiftPackScratchWinnerBuffView.OnDestroy()
    this:Destroy()
end

function GiftPackScratchWinnerBuffView:on_close()
    this:stop_x_update()
end

function GiftPackScratchWinnerBuffView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, this)
end

function GiftPackScratchWinnerBuffView:CutDonwTarget()
    this:CloseFunc()
    this.main_effect:Play("end")
end

function GiftPackScratchWinnerBuffView:on_btn_close_click()
...
function GiftPackScratchWinnerBuffView:on_btn_left_buy_click()
...
function GiftPackScratchWinnerBuffView:on_btn_right_buy_click()
    if this._fsm:GetCurName() ~= "GiftPackEnterState" then return end
    if canDownBuy then
        local click_time = UnityEngine.Time.time
...
function GiftPackScratchWinnerBuffView:CloseUIShiny(obj)
...
function GiftPackScratchWinnerBuffView:SetUIImageGray(btn_obj,active)
    if active then
...
function GiftPackScratchWinnerBuffView:OnBuySuccess(needClose,itemData)
    this:ChangeState(this,"GiftPackShowState",itemData)
end

function GiftPackScratchWinnerBuffView:GetGiftPos(id)
