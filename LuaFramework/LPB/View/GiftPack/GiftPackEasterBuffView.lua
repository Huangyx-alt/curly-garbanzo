...
---@class  GiftPackEasterBuffView: GiftPackBaseView
local GiftPackEasterBuffView = GiftPackBaseView:New('GiftPackEasterBuffView', "GiftPackEasterBuffAtlasInMain")
local this = GiftPackEasterBuffView
...
function GiftPackEasterBuffView:Awake(obj)
    this:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackEasterBuffView.OnEnable()
    this:BuildFsm(this)
    this:Bi_Tracker()
end

function GiftPackEasterBuffView:on_x_update()
end

function GiftPackEasterBuffView:ShowDetailGift()
...
function GiftPackEasterBuffView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackEasterBuffView.GetItemOrResource(id, param)
    if id > 1000 then
...
function GiftPackEasterBuffView:SetLeftProduct(id)
...
function GiftPackEasterBuffView:SetRightProduct(id)
    local xls = Csv.GetData("pop_up", id)
...
function GiftPackEasterBuffView:SetLeftTime(endTimeStamp)
...
function GiftPackEasterBuffView:SetContent(id, txt, content)
    id = tonumber(id)
    if fun.is_null(txt) then
        return
    end
    if id == 23 or id == 24 or id == 902102 then
...
function GiftPackEasterBuffView.OnDisable()
    this:stop_x_update()
end

function GiftPackEasterBuffView.OnDestroy()
    this:Destroy()
end

function GiftPackEasterBuffView:on_close()
    this:stop_x_update()
end

function GiftPackEasterBuffView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, this)
end

function GiftPackEasterBuffView:CutDonwTarget()
    this:CloseFunc()
    this.main_effect:Play("end")
end

function GiftPackEasterBuffView:on_btn_close_click()
    ...
    Facade.SendNotification(NotifyName.HideUI, ViewList.GiftPackEasterBuffView)
    ModelList.GiftPackModel:CloseView()
end

this.last_click_time1 = 0
function GiftPackEasterBuffView:on_btn_left_buy_click()
...
function GiftPackEasterBuffView:on_btn_right_buy_click()
    if this._fsm:GetCurName() ~= "GiftPackEnterState" then return end
    if canDownBuy then
        local click_time = UnityEngine.Time.time
...
function GiftPackEasterBuffView:CloseUIShiny(obj)
...
function GiftPackEasterBuffView:SetUIImageGray(btn_obj,active)
    if active then
...
function GiftPackEasterBuffView:OnBuySuccess(needClose,itemData)
    this:ChangeState(this,"GiftPackShowState",itemData)
end

function GiftPackEasterBuffView:GetGiftPos(id)
