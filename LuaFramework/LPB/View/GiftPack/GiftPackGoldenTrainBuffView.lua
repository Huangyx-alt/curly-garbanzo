...
---@class  GiftPackGoldenTrainBuffView: GiftPackBaseView
local GiftPackGoldenTrainBuffView = GiftPackBaseView:New('GiftPackGoldenTrainBuffView', "GiftPackGoldenTrainBuffAtlas")
local this = GiftPackGoldenTrainBuffView
...
function GiftPackGoldenTrainBuffView:Awake(obj)
    this:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackGoldenTrainBuffView.OnEnable()
    this:BuildFsm(this)
    this:Bi_Tracker()
end

function GiftPackGoldenTrainBuffView:on_x_update()
end

function GiftPackGoldenTrainBuffView:ShowDetailGift()
...
function GiftPackGoldenTrainBuffView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackGoldenTrainBuffView.GetItemOrResource(id, param)
    if id > 1000 then
...
function GiftPackGoldenTrainBuffView:SetLeftProduct(id)
...
function GiftPackGoldenTrainBuffView:SetRightProduct(id)
    local xls = Csv.GetData("pop_up", id)
...
function GiftPackGoldenTrainBuffView:SetLeftTime(endTimeStamp)
...
function GiftPackGoldenTrainBuffView:SetContent(id, txt, content)
    id = tonumber(id)
    if fun.is_null(txt) then
        return
    end
    --- flag:短令牌 每次增加新的短令牌都要修改
    if id == 23 or id == 24 or id == 903002 then
...
function GiftPackGoldenTrainBuffView.OnDisable()
    this:stop_x_update()
end

function GiftPackGoldenTrainBuffView.OnDestroy()
    this:Destroy()
end

function GiftPackGoldenTrainBuffView:on_close()
    this:stop_x_update()
end

function GiftPackGoldenTrainBuffView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, this)
end

function GiftPackGoldenTrainBuffView:CutDonwTarget()
    this:CloseFunc()
    this.main_effect:Play("end")
end

function GiftPackGoldenTrainBuffView:on_btn_close_click()
...
function GiftPackGoldenTrainBuffView:on_btn_left_buy_click()
...
function GiftPackGoldenTrainBuffView:on_btn_right_buy_click()
    if this._fsm:GetCurName() ~= "GiftPackEnterState" then return end
    if canDownBuy then
        local click_time = UnityEngine.Time.time
...
function GiftPackGoldenTrainBuffView:CloseUIShiny(obj)
...
function GiftPackGoldenTrainBuffView:SetUIImageGray(btn_obj,active)
    if active then
...
function GiftPackGoldenTrainBuffView:OnBuySuccess(needClose,itemData)
    this:ChangeState(this,"GiftPackShowState",itemData)
end

function GiftPackGoldenTrainBuffView:GetGiftPos(id)
