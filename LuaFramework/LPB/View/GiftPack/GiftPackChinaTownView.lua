...
local GiftPackChinaTownView = GiftPackBaseView:New("GiftPackChinaTownView", "GiftPackUlbetViewAtlas")
local this = GiftPackChinaTownView
...
function GiftPackChinaTownView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackChinaTownView:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    UISound.play("sale_ultrabet")
end

function GiftPackChinaTownView:OnDisable()
...
function GiftPackChinaTownView:ClearCountDown()
...
function GiftPackChinaTownView:CloseUIShiny(obj)
...
function GiftPackChinaTownView:GetLastClickTime()
    return lastClickTime or 0
end

function GiftPackChinaTownView:on_btn_up_buy_click()
...
function GiftPackChinaTownView:on_btn_buy2_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackChinaTownView:SetExtra(extraUI, extraParent, bonus)
...
function GiftPackChinaTownView:SetItemGrid(itemId, itemNum, itemGrid)
...
function GiftPackChinaTownView:ShowItemGrid()
...
function GiftPackChinaTownView:SetSellInfo(id,buyCount)
...
function GiftPackChinaTownView:SetItem(itemUI, id, pos, btn)
...
function GiftPackChinaTownView:SetCanBuyCount(cout)
...
function GiftPackChinaTownView:GetItemGrid(pos,index,prefab, parent)
...
function GiftPackChinaTownView:HideGridItem()
    for k, v in pairs(itemList) do
        for z, w in pairs(v) do
            if w then
...
function GiftPackChinaTownView:ShowDetailGift()
...
function GiftPackChinaTownView:SetButton(button, id, pos)
...
function GiftPackChinaTownView:SetLeftTime(endTimeStamp)
...
function GiftPackChinaTownView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackChinaTownView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, self)
end

function GiftPackChinaTownView:ClosePackView()
...
function GiftPackChinaTownView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackChinaTownView:OnBuySuccess(needClose, itemData)
    self:ChangeState(self, "GiftPackShowState", itemData)
end

function GiftPackChinaTownView:GetGiftPos(id)
...
function GiftPackChinaTownView:GetItemPos(pos)
    local posTable ={}
