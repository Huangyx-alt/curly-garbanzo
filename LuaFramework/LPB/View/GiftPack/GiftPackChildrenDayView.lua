...
local GiftPackChildrenDayView = GiftPackBaseView:New('GiftPackChildrenDayView', "GiftPackChildrenDayAtlas")
local this = GiftPackChildrenDayView
...
function GiftPackChildrenDayView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackChildrenDayView:OnEnable()
    self:BuildFsm(this)
    self:Bi_Tracker()
    UISound.play("sale_joker")
end

function GiftPackChildrenDayView:OnDisable()
...
function GiftPackChildrenDayView:ClearCountDown()
...
function GiftPackChildrenDayView:CloseUIShiny(obj)
...
function GiftPackChildrenDayView:on_btn_buy1_click()
...
function GiftPackChildrenDayView:GetLastClickTime(pos)
    return clickTimeList or 0
end

function GiftPackChildrenDayView:on_btn_buy2_click()
    if this._fsm:GetCurName() ~= "GiftPackEnterState" then return end
...
function GiftPackChildrenDayView:on_btn_buy3_click()
    if this._fsm:GetCurName() ~= "GiftPackEnterState" then return end
...
function GiftPackChildrenDayView:SetExtra(extraUI,extraParent , bonus)
...
function GiftPackChildrenDayView:SetItemGrid(itemId, itemNum , itemGrid)
...
function GiftPackChildrenDayView:ShowItemGrid()
...
function GiftPackChildrenDayView:SetItem(itemUI,id, pos, btn)
...
function GiftPackChildrenDayView:GetItemGrid(pos,index,prefab, parent)
...
function GiftPackChildrenDayView:HideGridItem()
    for k ,v in pairs(itemList) do
        for z,w in pairs(v) do
            if w then
...
function GiftPackChildrenDayView:ShowDetailGift()
...
function GiftPackChildrenDayView:SetButton(button, id,pos)
...
function GiftPackChildrenDayView:SetLeftTime(endTimeStamp)
...
function GiftPackChildrenDayView:CollectProduct(pId,giftInfo)
    product_list = {}
...
function GiftPackChildrenDayView.OnDestroy()
    this:Destroy()
end

function GiftPackChildrenDayView:on_close()
    --Event.Brocast(Notes.CARD_COLLIDER_OPEN, true)
end

function GiftPackChildrenDayView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, this)
end

function GiftPackChildrenDayView:ClosePackView()
...
function GiftPackChildrenDayView:on_btn_close_click()
    self:ClosePackView()
end

function  GiftPackChildrenDayView:OnBuySuccess(needClose,itemData)
    this:ChangeState(this,"GiftPackShowState",itemData)
end

function  GiftPackChildrenDayView:GetGiftPos(id)
...
function GiftPackChildrenDayView:GetItemPos(pos)
    local posTable ={}
