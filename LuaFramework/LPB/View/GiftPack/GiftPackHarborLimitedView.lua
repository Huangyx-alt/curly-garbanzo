...
local GiftPackHarborLimitedView = GiftPackBaseView:New("GiftPackHarborLimitedView", "GiftPackHarborLimitedViewAtlas")
local this = GiftPackHarborLimitedView
...
function GiftPackHarborLimitedView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackHarborLimitedView:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    self:InitReportData()
...
function GiftPackHarborLimitedView:OnDisable()
...
function GiftPackHarborLimitedView:ClearCountDown()
...
function GiftPackHarborLimitedView:CloseUIShiny(obj)
...
function GiftPackHarborLimitedView:GetLastClickTime()
    return lastClickTime or 0
end

function GiftPackHarborLimitedView:on_btn_up_buy_click()
...
function GiftPackHarborLimitedView:on_btn_buy2_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackHarborLimitedView:SetExtra(extraUI, extraParent, bonus)
...
function GiftPackHarborLimitedView:SetItemGrid(itemId, itemNum, itemGrid)
...
function GiftPackHarborLimitedView:ShowItemGrid()
...
function GiftPackHarborLimitedView:SetSellInfo(id,buyCount)
...
function GiftPackHarborLimitedView:SetItem(itemUI, id, pos, btn)
...
function GiftPackHarborLimitedView:SetCanBuyCount(cout)
    self.txt_soldnum.text = tostring(cout)
end

function GiftPackHarborLimitedView:GetItemGrid(pos,index,prefab, parent)
...
function GiftPackHarborLimitedView:HideGridItem()
    for k, v in pairs(itemList) do
        for z, w in pairs(v) do
            if w then
...
function GiftPackHarborLimitedView:ShowDetailGift()
...
function GiftPackHarborLimitedView:SetButton(button, id, pos)
...
function GiftPackHarborLimitedView:SetLeftTime(endTimeStamp)
...
function GiftPackHarborLimitedView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackHarborLimitedView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, self)
end

function GiftPackHarborLimitedView:ClosePackView()
...
function GiftPackHarborLimitedView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackHarborLimitedView:OnBuySuccess(needClose, itemData)
    self:SetPurchaseState(BingoBangEntry.reportPurchaseState.PurchaseSuccess)
    self:ChangeState(self, "GiftPackShowState", itemData)
end

function GiftPackHarborLimitedView:GetGiftPos(id)
...
function GiftPackHarborLimitedView:GetItemPos(pos)
    local posTable ={}
