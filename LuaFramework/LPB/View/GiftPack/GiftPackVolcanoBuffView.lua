...
---@class  GiftPackVolcanoBuffView: GiftPackBaseView
local GiftPackVolcanoBuffView = GiftPackBaseView:New('GiftPackVolcanoBuffView', "GiftPackVolcanoBuffAtlasInMain")
local this = GiftPackVolcanoBuffView
...
function GiftPackVolcanoBuffView:Awake(obj)
    this:on_init()
    self._isInit = true
    --- �޸����ֻ����״ν�����治��ʾ����
...
function GiftPackVolcanoBuffView.OnEnable()
    this:BuildFsm(this)
    this:Bi_Tracker()
end

function GiftPackVolcanoBuffView:on_x_update()
end

function GiftPackVolcanoBuffView:ShowDetailGift()
...
function GiftPackVolcanoBuffView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackVolcanoBuffView.GetItemOrResource(id, param)
    if id > 1000 then
...
function GiftPackVolcanoBuffView:SetLeftProduct(id)
...
function GiftPackVolcanoBuffView:SetRightProduct(id)
    local xls = Csv.GetData("pop_up", id)
...
function GiftPackVolcanoBuffView:SetContent(id, txt, content)
    id = tonumber(id)
    if fun.is_null(txt) then
        return
    end
    if id == 35 or id == 36 or id == 902202 then
...
function GiftPackVolcanoBuffView.OnDisable()
    this:stop_x_update()
end

function GiftPackVolcanoBuffView.OnDestroy()
    this:Destroy()
end

function GiftPackVolcanoBuffView:on_close()
    this:stop_x_update()
end

function GiftPackVolcanoBuffView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, this)
end

function GiftPackVolcanoBuffView:CutDonwTarget()
    this:CloseFunc()
    this.main_effect:Play("end")
end

function GiftPackVolcanoBuffView:on_btn_close_click()
...
function GiftPackVolcanoBuffView:on_btn_left_buy_click()
...
function GiftPackVolcanoBuffView:on_btn_right_buy_click()
    if this._fsm:GetCurName() ~= "GiftPackEnterState" then return end
    if canDownBuy then
        local click_time = UnityEngine.Time.time
...
function GiftPackVolcanoBuffView:CloseUIShiny(obj)
...
function GiftPackVolcanoBuffView:SetUIImageGray(btn_obj,active)
    if active then
...
function GiftPackVolcanoBuffView:OnBuySuccess(needClose,itemData)
    this:ChangeState(this,"GiftPackShowState",itemData)
end

function GiftPackVolcanoBuffView:GetGiftPos(id)
