...
local GiftPackEasterView = GiftPackBaseView:New("GiftPackEasterView", "GiftPackUlbetViewAtlas")
local this = GiftPackEasterView
...
function GiftPackEasterView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackEasterView:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    UISound.play("sale_ultrabet")
end

function GiftPackEasterView:OnDisable()
...
function GiftPackEasterView:ClearCountDown()
...
function GiftPackEasterView:CloseUIShiny(obj)
...
function GiftPackEasterView:GetLastClickTime()
    return lastClickTime or 0
end

function GiftPackEasterView:on_btn_up_buy_click()
...
function GiftPackEasterView:on_btn_buy2_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackEasterView:SetExtra(extraUI, extraParent, bonus)
...
function GiftPackEasterView:SetItemGrid(itemId, itemNum, itemGrid)
...
function GiftPackEasterView:ShowItemGrid()
...
function GiftPackEasterView:SetSellInfo(id,buyCount)
...
function GiftPackEasterView:SetItem(itemUI, id, pos, btn)
...
function GiftPackEasterView:SetCanBuyCount(cout)
...
function GiftPackEasterView:GetItemGrid(pos,index,prefab, parent)
...
function GiftPackEasterView:HideGridItem()
    for k, v in pairs(itemList) do
        for z, w in pairs(v) do
            if w then
...
function GiftPackEasterView:ShowDetailGift()
...
function GiftPackEasterView:SetButton(button, id, pos)
...
function GiftPackEasterView:SetLeftTime(endTimeStamp)
...
function GiftPackEasterView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackEasterView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, self)
end

function GiftPackEasterView:ClosePackView()
...
function GiftPackEasterView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackEasterView:OnBuySuccess(needClose, itemData)
    self:ChangeState(self, "GiftPackShowState", itemData)
end

function GiftPackEasterView:GetGiftPos(id)
...
function GiftPackEasterView:GetItemPos(pos)
    local posTable ={}
