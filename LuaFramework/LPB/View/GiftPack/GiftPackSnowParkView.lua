...
local GiftPackSnowParkView = GiftPackBaseView:New('GiftPackSnowParkView', "GiftPackSnowParkAtlas")
local this = GiftPackSnowParkView
...
function GiftPackSnowParkView:Awake(obj)
    self:on_init()
...
function GiftPackSnowParkView:OnEnable()
    Facade.RegisterView(self)
    --   this:ClearMailList()
    self:BuildFsm(self)
    self:Bi_Tracker()
...
function GiftPackSnowParkView:OnDisable()
...
function GiftPackSnowParkView:ClearCountDown()
...
function GiftPackSnowParkView:ShowItemGrid()
    ...
    if xls and xls.icon == "cEntchristmascoin" then
        fun.set_active(self.titleBg1, true)
        fun.set_active(self.titleBg2, false)
    else
        fun.set_active(self.titleBg1, false)
        fun.set_active(self.titleBg2, true)
    end
end

function GiftPackSnowParkView:ShowItemGridData(step,flag)
    _data = ModelList.GiftPackModel:GetShowGiftPack()
...
function GiftPackSnowParkView:movetoGrid(step,tflag)
    step = step or 1
...
function GiftPackSnowParkView:ClearMailList()
...
function GiftPackSnowParkView:GetItemViewInstance(index)
...
function GiftPackSnowParkView:ShowDetailGift()
...
function GiftPackSnowParkView:SetLeftTime(endTimeStamp)
...
function GiftPackSnowParkView:OnDestroy()
    log.g("GiftPackSnowParkView.OnDestroy ing !")
    Facade.RemoveView(self)
    self:Destroy()
    self:ClearMailList()
end

function GiftPackSnowParkView:CloseFunc()
    Facade.SendNotification(NotifyName.HideUI, this)
end

function GiftPackSnowParkView:ClosePackView()
    ...
    self:ClearCountDown()
    Facade.SendNotification(NotifyName.HideUI, ViewList.GiftPackSnowParkView)
    ModelList.GiftPackModel:CloseView()
end

function GiftPackSnowParkView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackSnowParkView:OnBuySuccess(needClose,itemData)
...
function GiftPackSnowParkView:showGetRewardEffect(itemData,isFree)
...
function GiftPackSnowParkView:SetTimer()
...
--领取成功
function GiftPackSnowParkView.OnClaimEndlessGiftSuccess(data)
...
--领取失败
function GiftPackSnowParkView.OnClaimEndlessGiftFail(data)
    --提示下领取失败
end

--查询成功
function GiftPackSnowParkView.OnFetchEndlessGiftSuccess(data)
    selfstep = data.step
    local tmpflag = false
...
--领取失败
function GiftPackSnowParkView.OnFetchEndlessGiftFail(data)
    --提示下查询失败
end
