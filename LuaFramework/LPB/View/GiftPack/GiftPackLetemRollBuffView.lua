...
---@class  GiftPackLetemRollBuffView: GiftPackBaseView
local GiftPackLetemRollBuffView = GiftPackBaseView:New('GiftPackLetemRollBuffView', "GiftPackLetemRollBuffAtlas")
local this = GiftPackLetemRollBuffView
...
function GiftPackLetemRollBuffView:Awake(obj)
    this:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackLetemRollBuffView.OnEnable()
    this:BuildFsm(this)
    this:Bi_Tracker()
end

function GiftPackLetemRollBuffView:on_x_update()
end

function GiftPackLetemRollBuffView:ShowDetailGift()
...
function GiftPackLetemRollBuffView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackLetemRollBuffView.GetItemOrResource(id, param)
    if id > 1000 then
...
function GiftPackLetemRollBuffView:SetLeftProduct(id)
...
function GiftPackLetemRollBuffView:SetRightProduct(id)
    local xls = Csv.GetData("pop_up", id)
...
function GiftPackLetemRollBuffView:SetLeftTime(endTimeStamp)
...
function GiftPackLetemRollBuffView:SetContent(id, txt, content)
    id = tonumber(id)
    if fun.is_null(txt) then
        return
    end
    if id == 23 or id == 24 or id == 903302 then
...
function GiftPackLetemRollBuffView.OnDisable()
    this:stop_x_update()
end

function GiftPackLetemRollBuffView.OnDestroy()
    this:Destroy()
end

function GiftPackLetemRollBuffView:on_close()
    this:stop_x_update()
end

function GiftPackLetemRollBuffView:CloseFunc()
    Facade.SendNotification(NotifyName.CloseUI, this)
end

function GiftPackLetemRollBuffView:CutDonwTarget()
    this:CloseFunc()
    this.main_effect:Play("end")
end

function GiftPackLetemRollBuffView:on_btn_close_click()
...
function GiftPackLetemRollBuffView:on_btn_left_buy_click()
...
function GiftPackLetemRollBuffView:on_btn_right_buy_click()
    if this._fsm:GetCurName() ~= "GiftPackEnterState" then return end
    if canDownBuy then
        local click_time = UnityEngine.Time.time
...
function GiftPackLetemRollBuffView:CloseUIShiny(obj)
...
function GiftPackLetemRollBuffView:SetUIImageGray(btn_obj,active)
    if active then
...
function GiftPackLetemRollBuffView:OnBuySuccess(needClose,itemData)
    this:ChangeState(this,"GiftPackShowState",itemData)
end

function GiftPackLetemRollBuffView:GetGiftPos(id)
