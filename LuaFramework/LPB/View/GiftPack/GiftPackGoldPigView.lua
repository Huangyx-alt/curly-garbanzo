...
local GiftPackGoldPigView = GiftPackBaseView:New("GiftPackGoldPigView", "GiftPackUlbetViewAtlas")
local this = GiftPackGoldPigView
...
function GiftPackGoldPigView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackGoldPigView:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    UISound.play("sale_ultrabet")
end

function GiftPackGoldPigView:OnDisable()
...
function GiftPackGoldPigView:ClearCountDown()
...
function GiftPackGoldPigView:CloseUIShiny(obj)
...
function GiftPackGoldPigView:GetLastClickTime()
    return lastClickTime or 0
end

function GiftPackGoldPigView:on_btn_up_buy_click()
...
function GiftPackGoldPigView:on_btn_buy2_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackGoldPigView:SetExtra(extraUI, extraParent, bonus)
...
function GiftPackGoldPigView:SetItemGrid(itemId, itemNum, itemGrid)
...
function GiftPackGoldPigView:ShowItemGrid()
...
function GiftPackGoldPigView:SetSellInfo(id,buyCount)
...
function GiftPackGoldPigView:SetItem(itemUI, id, pos, btn)
...
function GiftPackGoldPigView:SetCanBuyCount(cout)
...
function GiftPackGoldPigView:GetItemGrid(pos,index,prefab, parent)
...
function GiftPackGoldPigView:HideGridItem()
    for k, v in pairs(itemList) do
        for z, w in pairs(v) do
            if w then
...
function GiftPackGoldPigView:ShowDetailGift()
...
function GiftPackGoldPigView:SetButton(button, id, pos)
...
function GiftPackGoldPigView:SetLeftTime(endTimeStamp)
...
function GiftPackGoldPigView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackGoldPigView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, self)
end

function GiftPackGoldPigView:ClosePackView()
...
function GiftPackGoldPigView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackGoldPigView:OnBuySuccess(needClose, itemData)
    self:ChangeState(self, "GiftPackShowState", itemData)
end

function GiftPackGoldPigView:GetGiftPos(id)
...
function GiftPackGoldPigView:GetItemPos(pos)
    local posTable ={}
