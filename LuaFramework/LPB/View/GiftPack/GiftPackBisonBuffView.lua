...
---@class  GiftPackBisonBuffView: GiftPackBaseView
local GiftPackBisonBuffView = GiftPackBaseView:New('GiftPackBisonBuffView', "GiftPackMoleMinerBuffAtlasInMain")
local this = GiftPackBisonBuffView
...
function GiftPackBisonBuffView:Awake(obj)
    this:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackBisonBuffView.OnEnable()
    this:BuildFsm(this)
    this:Bi_Tracker()
end

function GiftPackBisonBuffView:on_x_update()
end

function GiftPackBisonBuffView:ShowDetailGift()
...
function GiftPackBisonBuffView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackBisonBuffView.GetItemOrResource(id, param)
    if id > 1000 then
...
function GiftPackBisonBuffView:SetLeftProduct(id)
...
function GiftPackBisonBuffView:SetRightProduct(id)
    local xls = Csv.GetData("pop_up", id)
...
function GiftPackBisonBuffView:SetLeftTime(endTimeStamp)
...
function GiftPackBisonBuffView:SetContent(id, txt, content)
    id = tonumber(id)
...
function GiftPackBisonBuffView.OnDisable()
    this:stop_x_update()
end

function GiftPackBisonBuffView.OnDestroy()
    this:Destroy()
end

function GiftPackBisonBuffView:on_close()
    this:stop_x_update()
end

function GiftPackBisonBuffView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, this)
end

function GiftPackBisonBuffView:CutDonwTarget()
    this:CloseFunc()
    this.main_effect:Play("end")
end

function GiftPackBisonBuffView:on_btn_close_click()
...
function GiftPackBisonBuffView:on_btn_left_buy_click()
...
function GiftPackBisonBuffView:on_btn_right_buy_click()
    if this._fsm:GetCurName() ~= "GiftPackEnterState" then return end
    if canDownBuy then
        local click_time = UnityEngine.Time.time
...
function GiftPackBisonBuffView:CloseUIShiny(obj)
...
function GiftPackBisonBuffView:SetUIImageGray(btn_obj,active)
    if active then
...
function GiftPackBisonBuffView:OnBuySuccess(needClose,itemData)
    this:ChangeState(this,"GiftPackShowState",itemData)
end

function GiftPackBisonBuffView:GetGiftPos(id)
