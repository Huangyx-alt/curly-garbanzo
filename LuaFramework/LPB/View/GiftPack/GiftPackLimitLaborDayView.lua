...
--- DateTime: 20/8/2024 下午 2:26
---

local GiftPackBaseView = require("View.GiftPack.GiftPackBaseView")
local GiftPackLimitLaborDayView = GiftPackBaseView:New("GiftPackLimitLaborDayView", "GiftPackLimitLaborDayAtlas")
local this = GiftPackLimitLaborDayView
...
function GiftPackLimitLaborDayView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackLimitLaborDayView:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    UISound.play("sale_ultrabet")
end

function GiftPackLimitLaborDayView:OnDisable()
...
function GiftPackLimitLaborDayView:ClearCountDown()
...
function GiftPackLimitLaborDayView:CloseUIShiny(obj)
...
function GiftPackLimitLaborDayView:GetLastClickTime()
    return lastClickTime or 0
end

function GiftPackLimitLaborDayView:on_btn_up_buy_click()
...
function GiftPackLimitLaborDayView:on_btn_buy2_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackLimitLaborDayView:SetExtra(extraUI, extraParent, bonus)
...
function GiftPackLimitLaborDayView:SetItemGrid(itemId, itemNum, itemGrid)
...
function GiftPackLimitLaborDayView:ShowItemGrid()
...
function GiftPackLimitLaborDayView:SetSellInfo(id,buyCount)
...
function GiftPackLimitLaborDayView:SetItem(itemUI, id, pos, btn)
...
function GiftPackLimitLaborDayView:SetCanBuyCount(cout)
...
function GiftPackLimitLaborDayView:GetItemGrid(pos,index,prefab, parent)
...
function GiftPackLimitLaborDayView:HideGridItem()
    for k, v in pairs(itemList) do
        for z, w in pairs(v) do
            if w then
...
function GiftPackLimitLaborDayView:ShowDetailGift()
...
function GiftPackLimitLaborDayView:SetButton(button, id, pos)
...
function GiftPackLimitLaborDayView:SetLeftTime(endTimeStamp)
...
function GiftPackLimitLaborDayView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackLimitLaborDayView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, self)
end

function GiftPackLimitLaborDayView:ClosePackView()
...
function GiftPackLimitLaborDayView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackLimitLaborDayView:OnBuySuccess(needClose, itemData)
    self:ChangeState(self, "GiftPackShowState", itemData)
end

function GiftPackLimitLaborDayView:GetGiftPos(id)
...
function GiftPackLimitLaborDayView:GetItemPos(pos)
    local posTable ={}
