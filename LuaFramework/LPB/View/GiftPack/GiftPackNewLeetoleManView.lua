...
local GiftPackNewLeetoleManView = GiftPackBaseView:New("GiftPackNewLeetoleManView", "GiftPackUlbetViewAtlas")
local this = GiftPackNewLeetoleManView
...
function GiftPackNewLeetoleManView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackNewLeetoleManView:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    UISound.play("sale_ultrabet")
end

function GiftPackNewLeetoleManView:OnDisable()
...
function GiftPackNewLeetoleManView:ClearCountDown()
...
function GiftPackNewLeetoleManView:CloseUIShiny(obj)
...
function GiftPackNewLeetoleManView:GetLastClickTime()
    return lastClickTime or 0
end

function GiftPackNewLeetoleManView:on_btn_up_buy_click()
...
function GiftPackNewLeetoleManView:on_btn_buy2_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackNewLeetoleManView:SetExtra(extraUI, extraParent, bonus)
...
function GiftPackNewLeetoleManView:SetItemGrid(itemId, itemNum, itemGrid)
...
function GiftPackNewLeetoleManView:ShowItemGrid()
...
function GiftPackNewLeetoleManView:SetSellInfo(id,buyCount)
...
function GiftPackNewLeetoleManView:SetItem(itemUI, id, pos, btn)
...
function GiftPackNewLeetoleManView:SetCanBuyCount(cout)
...
function GiftPackNewLeetoleManView:GetItemGrid(pos,index,prefab, parent)
...
function GiftPackNewLeetoleManView:HideGridItem()
    for k, v in pairs(itemList) do
        for z, w in pairs(v) do
            if w then
...
function GiftPackNewLeetoleManView:ShowDetailGift()
...
function GiftPackNewLeetoleManView:SetButton(button, id, pos)
...
function GiftPackNewLeetoleManView:SetLeftTime(endTimeStamp)
...
function GiftPackNewLeetoleManView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackNewLeetoleManView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, self)
end

function GiftPackNewLeetoleManView:ClosePackView()
    if self then
        if  self._fsm and self._fsm:GetCurName() == "GiftPackShowState" then
            return
        end
        self:ClearCountDown()
...
function GiftPackNewLeetoleManView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackNewLeetoleManView:OnBuySuccess(needClose, itemData)
    self:ChangeState(self, "GiftPackShowState", itemData)
end

function GiftPackNewLeetoleManView:GetGiftPos(id)
...
function GiftPackNewLeetoleManView:GetItemPos(pos)
    local posTable ={}
