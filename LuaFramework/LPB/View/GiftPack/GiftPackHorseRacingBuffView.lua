...
---@class  GiftPackHorseRacingBuffView: GiftPackBaseView
local GiftPackHorseRacingBuffView = GiftPackBaseView:New('GiftPackHorseRacingBuffView', "GiftPackHorseRacingBuffAtlas")
local this = GiftPackHorseRacingBuffView
...
function GiftPackHorseRacingBuffView:Awake(obj)
    this:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackHorseRacingBuffView.OnEnable()
    this:BuildFsm(this)
    this:Bi_Tracker()
end

function GiftPackHorseRacingBuffView:on_x_update()
end

function GiftPackHorseRacingBuffView:ShowDetailGift()
...
function GiftPackHorseRacingBuffView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackHorseRacingBuffView.GetItemOrResource(id, param)
    if id > 1000 then
...
function GiftPackHorseRacingBuffView:SetLeftProduct(id)
...
function GiftPackHorseRacingBuffView:SetRightProduct(id)
    local xls = Csv.GetData("pop_up", id)
...
function GiftPackHorseRacingBuffView:SetLeftTime(endTimeStamp)
...
function GiftPackHorseRacingBuffView:SetContent(id, txt, content)
    id = tonumber(id)
    if fun.is_null(txt) then
        return
    end
    --- flag:短令牌 每次增加新的短令牌都要修改
    if id == 23 or id == 24 or id == 902702 then
...
function GiftPackHorseRacingBuffView.OnDisable()
    this:stop_x_update()
end

function GiftPackHorseRacingBuffView.OnDestroy()
    this:Destroy()
end

function GiftPackHorseRacingBuffView:on_close()
    this:stop_x_update()
end

function GiftPackHorseRacingBuffView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, this)
end

function GiftPackHorseRacingBuffView:CutDonwTarget()
    this:CloseFunc()
    this.main_effect:Play("end")
end

function GiftPackHorseRacingBuffView:on_btn_close_click()
...
function GiftPackHorseRacingBuffView:on_btn_left_buy_click()
...
function GiftPackHorseRacingBuffView:on_btn_right_buy_click()
    if this._fsm:GetCurName() ~= "GiftPackEnterState" then return end
    if canDownBuy then
        local click_time = UnityEngine.Time.time
...
function GiftPackHorseRacingBuffView:CloseUIShiny(obj)
...
function GiftPackHorseRacingBuffView:SetUIImageGray(btn_obj,active)
    if active then
...
function GiftPackHorseRacingBuffView:OnBuySuccess(needClose,itemData)
    this:ChangeState(this,"GiftPackShowState",itemData)
end

function GiftPackHorseRacingBuffView:GetGiftPos(id)
