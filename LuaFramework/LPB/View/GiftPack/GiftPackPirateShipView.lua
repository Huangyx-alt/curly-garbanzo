...
local GiftPackPirateShipView = GiftPackBaseView:New("GiftPackPirateShipView", "GiftPackUlbetViewAtlas")
local this = GiftPackPirateShipView
...
function GiftPackPirateShipView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackPirateShipView:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    UISound.play("sale_ultrabet")
end

function GiftPackPirateShipView:OnDisable()
...
function GiftPackPirateShipView:ClearCountDown()
...
function GiftPackPirateShipView:CloseUIShiny(obj)
...
function GiftPackPirateShipView:GetLastClickTime()
    return lastClickTime or 0
end

function GiftPackPirateShipView:on_btn_up_buy_click()
...
function GiftPackPirateShipView:on_btn_buy2_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackPirateShipView:SetExtra(extraUI, extraParent, bonus)
...
function GiftPackPirateShipView:SetItemGrid(itemId, itemNum, itemGrid)
...
function GiftPackPirateShipView:ShowItemGrid()
...
function GiftPackPirateShipView:SetSellInfo(id,buyCount)
...
function GiftPackPirateShipView:SetItem(itemUI, id, pos, btn)
...
function GiftPackPirateShipView:SetCanBuyCount(cout)
...
function GiftPackPirateShipView:GetItemGrid(pos,index,prefab, parent)
...
function GiftPackPirateShipView:HideGridItem()
    for k, v in pairs(itemList) do
        for z, w in pairs(v) do
            if w then
...
function GiftPackPirateShipView:ShowDetailGift()
...
function GiftPackPirateShipView:SetButton(button, id, pos)
...
function GiftPackPirateShipView:SetLeftTime(endTimeStamp)
...
function GiftPackPirateShipView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackPirateShipView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, self)
end

function GiftPackPirateShipView:ClosePackView()
    if self then
...
function GiftPackPirateShipView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackPirateShipView:OnBuySuccess(needClose, itemData)
    self:ChangeState(self, "GiftPackShowState", itemData)
end

function GiftPackPirateShipView:GetGiftPos(id)
...
function GiftPackPirateShipView:GetItemPos(pos)
    local posTable ={}
