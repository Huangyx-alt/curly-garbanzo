...
local GiftPackRocketDoubleView = GiftPackBaseView:New('GiftPackRocketDoubleView',"GiftPackRocketAtlas")
local this = GiftPackRocketDoubleView
...
function GiftPackRocketDoubleView:Awake(obj)
    this:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackRocketDoubleView.OnEnable()
...
function GiftPackRocketDoubleView:on_x_update()
end

function GiftPackRocketDoubleView:ShowDetailGift()
...
function GiftPackRocketDoubleView:CollectProduct(pId,giftInfo)
    product_list = {}
...
function GiftPackRocketDoubleView.GetItemOrResource(id,param)
    if id > 1000 then
...
function GiftPackRocketDoubleView:SetUpProduct(id)
...
function GiftPackRocketDoubleView:SetDownProduct(id)
    local xls = Csv.GetData("pop_up", id)
    local item1 = xls.item_description[1][1]
    --local icon1 = Csv.GetData("resources", tonumber(item1) ,'icon')
    local icon1,atlasName = self.GetItemOrResource( tonumber(item1),"icon")
...
function GiftPackRocketDoubleView:SetLeftTime(endTimeStamp)
...
function GiftPackRocketDoubleView:SetContent(id,txt,content)
    id = tonumber(id)
...
function GiftPackRocketDoubleView:SetBounsAndMore(obj, bonus, more)
...
function GiftPackRocketDoubleView.OnDisable()
...
function GiftPackRocketDoubleView.OnDestroy()
    this:Destroy()
end

function GiftPackRocketDoubleView:on_close()
    this:stop_x_update()
    --Event.Brocast(Notes.CARD_COLLIDER_OPEN,true)
end

function GiftPackRocketDoubleView:CloseFunc()
...
function GiftPackRocketDoubleView:CutDonwTarget()
    this:CloseFunc()
    this.main_effect:Play("end")
end

function GiftPackRocketDoubleView:on_btn_close_click()
    if this._fsm:GetCurName() == "GiftPackShowState" then return end
    if this.loopTime then
        LuaTimer:Remove(    this.loopTime)
        this.loopTime = nil
    end
    Facade.SendNotification(NotifyName.CloseUI, ViewList.GiftPackRocketDoubleView)
    ModelList.GiftPackModel:CloseView()
end

this.last_click_time1 = 0
function GiftPackRocketDoubleView:on_btn_up_buy_click()
...
function GiftPackRocketDoubleView:on_btn_down_buy_click()
    if this._fsm:GetCurName() ~= "GiftPackEnterState" then return end
    if canDownBuy then
        local click_time = UnityEngine.Time.time
...
function GiftPackRocketDoubleView:CloseUIShiny(obj)
...
function  GiftPackRocketDoubleView:OnBuySuccess(needClose,itemData)
    this:ChangeState(this,"GiftPackShowState",itemData)
end

function  GiftPackRocketDoubleView:GetGiftPos(id)
    ...
    elseif id == product_list[2] then
        ...
        return this.down_reward_icon1.transform.position, this.down_reward_icon2.transform.position, vipexp.transform.position, nil
    else
        local vipexp = fun.find_child(this.cityExp1, "vipexp")
