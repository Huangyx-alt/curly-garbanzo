...
local GiftPackFathersDaySaleView = GiftPackBaseView:New("GiftPackFathersDaySaleView", "GiftPackFathersDaySaleAtlas")
local this = GiftPackFathersDaySaleView
...
function GiftPackFathersDaySaleView:New(viewName, atlasName)
    local o = {}
    o.viewName = viewName
    o.atlasName = atlasName
    setmetatable(o,{__index = GiftPackFathersDaySaleView})
    return o
end

function GiftPackFathersDaySaleView:Awake(obj)
    self:on_init()
    self._isInit = true
    --- 修复部分机型首次进入界面不显示问题
...
function GiftPackFathersDaySaleView:OnEnable()
    self:BuildFsm(self)
    self:Bi_Tracker()
    UISound.play("saleboth")
end

function GiftPackFathersDaySaleView:OnDisable()
...
function GiftPackFathersDaySaleView:ClearCountDown()
...
function GiftPackFathersDaySaleView:CloseUIShiny(obj)
...
function GiftPackFathersDaySaleView:GetLastClickTime()
    return lastClickTime or 0
end

function GiftPackFathersDaySaleView:on_btn_buy1_click()
...
function GiftPackFathersDaySaleView:on_btn_buy2_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackFathersDaySaleView:on_btn_buy3_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackFathersDaySaleView:on_btn_buy_all_click()
    if self._fsm:GetCurName() ~= "GiftPackEnterState" then
        return
    end
...
function GiftPackFathersDaySaleView:SetExtra(extraUI, extraParent, bonus)
...
function GiftPackFathersDaySaleView:SetItemGrid(itemId, itemNum, itemGrid)
...
function GiftPackFathersDaySaleView:ShowItemGrid()
...
function GiftPackFathersDaySaleView:ClearItems()
    for i = 1, 3 do
...
function GiftPackFathersDaySaleView:SetItem(itemUI, id, pos, btn)
...
function GiftPackFathersDaySaleView:SetLastItem(id, pos)
    local xls = Csv.GetData("pop_up", id)
    if xls then
...
function GiftPackFathersDaySaleView:GetItemGrid(pos,index,prefab, parent)
...
function GiftPackFathersDaySaleView:HideGridItem()
    for k, v in pairs(itemList) do
        for z, w in pairs(v) do
            if w then
...
function GiftPackFathersDaySaleView:ShowDetailGift()
...
function GiftPackFathersDaySaleView:SetButton(button, id, pos)
...
function GiftPackFathersDaySaleView:SetLeftTime(endTimeStamp)
...
function GiftPackFathersDaySaleView:CollectProduct(pId, giftInfo)
    product_list = {}
...
function GiftPackFathersDaySaleView:CloseFunc()
    Facade.SendNotification(NotifyName.HideDialog, self)
end


function GiftPackFathersDaySaleView:ClosePackView()
...
function GiftPackFathersDaySaleView:on_btn_close_click()
    self:ClosePackView()
end

function GiftPackFathersDaySaleView:OnBuySuccess(needClose, itemData)
    self:ChangeState(self, "GiftPackShowState", itemData)
end

function GiftPackFathersDaySaleView:GetGiftPos(id)
    ...
    elseif id == product_list[4] then
        ...
        log.log("GiftPackFathersDaySaleView:GetGiftPos(id) id error", id)
        return self:GetItemPos(itemListPos.Up)
    end
end

function GiftPackFathersDaySaleView:GetItemPos(pos)
    local posTable ={}
    if pos ~= 4 then
