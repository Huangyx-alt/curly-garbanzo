...
local ScreenWidth = 1080;--Screen.width;
local ScreenHeight = 1920;--Screen.height;

--创建View对象--
function PopView:New(viewName, atlasName,bundleName)
	return setmetatable({viewName = viewName,bundleName = bundleName, atlasName = atlasName}, this);
end

--显示View对象
function PopView:Show(args)
	self.args = args;
	logWarn("Show Pop UI:--->>" .. self.viewName);
	UIManager:ShowUI(self.viewName, self.TweenShow, self);
...
function PopView:Hide()
	logWarn("Hide Pop UI:--->>" .. self.viewName);
...
function PopView:Close()
	logWarn("Destroy Pop UI:--->>" .. self.viewName);
	if not self.isShow then return end
	SoundManager.Play(SoundName.Close);
	self:TweenHide();
	local function doClose()
		wait(TweenTime);
		UIManager:CloseUI(self.viewName);
		self.isLoaded = false;
	end
	start(doClose);
end
