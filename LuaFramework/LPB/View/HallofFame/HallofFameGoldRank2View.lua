...
local HallofFameShowNextRewardView = require "View/HallofFame/HallofFameShowNextRewardView"
local HallofFameShowTopOneView = require "View/HallofFame/HallofFameShowTopOneView"

local topOneDownItem = nil  --第一名降低到第二名用的UI

local HallofFameGoldRank2View = TopBarControllerView:New("HallofFameGoldRank2View", "TournamentAtlas")
local this = HallofFameGoldRank2View
...
function HallofFameGoldRank2View:Awake(obj)
    self:on_init()
end

function HallofFameGoldRank2View:OnEnable(params)
    ...
    log.log("初始进入修改")
    rank_index = 1
    --ModelList.HallofFameModel:SetTestData()
    self:InitRankItem()
...
function HallofFameGoldRank2View:ShowBuffEffect()
...
function HallofFameGoldRank2View:IsSelfHasSprintBuff()
    return ModelList.HallofFameModel:HasSprintBuff()
end

function HallofFameGoldRank2View:CheckShowTopPlayer()
...
function HallofFameGoldRank2View:CheckIsBlackTier()
    local lastTier = ModelList.HallofFameModel:GetClimbTiers(rank_index)
...
function HallofFameGoldRank2View:OnDisable()
    ...
    HallofFameShowTopOneView:OnDestroy()
    ModelList.HallofFameModel:CleartSettleInfo()
    topOneDownItem = nil
    self.myData = nil
end

function HallofFameGoldRank2View:BuildFsm()
    self:DisposeFsm()
    self._fsm = Fsm.CreateFsm("HallofFameGoldRank2View", self, {
...
function HallofFameGoldRank2View:DisposeFsm()
    if self._fsm then
        self._fsm:Dispose()
        self._fsm = nil
    end
...
function HallofFameGoldRank2View:on_btn_continue_click()
    self._fsm:GetCurState():PlayExite(self._fsm)
end

function HallofFameGoldRank2View:CloseTopBarParent()
    self:on_btn_continue_click()
end

function HallofFameGoldRank2View:PlayEnter()
...
function HallofFameGoldRank2View:initSpeedUp()
...
function HallofFameGoldRank2View:on_btn_speedUp_click()
...
function HallofFameGoldRank2View:PlayExite()
...
function HallofFameGoldRank2View:playChangeTier()
...
function HallofFameGoldRank2View:playChangeRoll()
    LuaTimer:SetDelayFunction(1.5, function()
        rank_index = rank_index + 1
        if ModelList.HallofFameModel:IsFameClimbRank(rank_index) then
            --需要这个
    ...
    Event.AddListener(NotifyName.HallofFame.FameShowNewTierContinue, self.ShowNewTierContinue, self)
    self:ShowNextReward()
...
function HallofFameGoldRank2View:ShowNextReward()
    HallofFameShowNextRewardView:OnEnable({ topOneRewardItem = self.topOneRewardItem, topOneItemList = self.topOneItemList, myNewRewardItem = self.myNewRewardItem, myNewRewardList = self.myNewRewardList, climbIndex = rank_index })
end

function HallofFameGoldRank2View:ShowNewTierContinue()
    AnimatorPlayHelper.Play(self.anima, { "changeexit", "TournamentBlackGoldSettleViewexit" }, false, 2, function()
    end, function()
        self._fsm:GetCurState():StartClimb(self._fsm)
        HallofFameShowNextRewardView:OnDestroy()
    end)
end

--获取自身初始节点位置
function HallofFameGoldRank2View:GetTargetInitIndex(climbData, climbListData)
    local lastIndex = climbListData.lastSectionIndex
    local currentIndex = climbListData.mySectionIndex
    return lastIndex
end

function HallofFameGoldRank2View:InitRankItem()
    local pos = 0
    local myUid = ModelList.PlayerInfoModel:GetUid()
    local climbData = ModelList.HallofFameModel:GetSettleClimbList(rank_index)
    local climbListData = ModelList.HallofFameModel:GetSettleClimbData(rank_index)
    ...
    for i = #climbData, 1, -1 do
        if myUid == climbData[i].uid and startIndex == climbData[i].order then
            if myRankItem == nil then
                ...
                myRankItem = HallofFameGoldMyRankItem:New(true)
            ...
            if climbData[i].uid ~= myUid then
                ...
                if rankItemList[itemIndex] == nil then
                    ...
                    item = HallofFameGoldRankItem:New(true)
                    item:SetParent(this)
    ...
    this.slider_tournament.value = 1 - ModelList.HallofFameModel:GetSettleClimbPreviousOrder(rank_index) / maxSection
    local tiers, difficulty = ModelList.HallofFameModel:GetSettleClimbPreviousTier(rank_index)
    self:SetTopReward(tiers, difficulty)
end

function HallofFameGoldRank2View:SetTopReward(tiers, difficulty)
...
function HallofFameGoldRank2View:StartClimb()
    if ModelList.HallofFameModel:IsClimbRank(rank_index) then
        if myRankItem then
...
function HallofFameGoldRank2View:FlyScore(callback)
    local lastTier = ModelList.HallofFameModel:GetClimbTiers(rank_index)
    local rank = ModelList.HallofFameModel:GetSettleClimbPreviousOrder(rank_index)
    local myScore = ModelList.HallofFameModel:GetSettleClimbCurrentScore(rank_index)
    if ModelList.HallofFameModel:CheckIsBlackTire(lastTier) and rank == 1 then
        --是最高段位第一名 加积分就结束流程
        HallofFameShowTopOneView:FlyScore(callback, myScore)
        log.log("加积分结束流程")
    else
        if myRankItem then
...
function HallofFameGoldRank2View:FinishClim(rankItem, index)
    myRankItem:SetRankData(nil)
    local myCurOrder = ModelList.HallofFameModel:GetSettleClimbCurrentOrder(rank_index)
...
function HallofFameGoldRank2View:ClimbTopOne(myRankItem, finishFunc)

    fun.set_active(self.HallofFameGoldRank2ViewShow, false)
    local topOnePos = fun.get_gameobject_pos(self.HallofFameGoldRank2ViewShow)
    ...
    local myData = ModelList.HallofFameModel:GetMyRankData(rank_index)
    Anim.move_ease(myRankItem.go, topOnePos.x, topOnePos.y, topOnePos.z, 0.5, false, DG.Tweening.Ease.InOutFlash, function()
        if self:CheckIsBlackTier() then
            HallofFameShowTopOneView:ClimbTopOne(myData)
            fun.set_active(self.HallofFameGoldRank2ViewShow, true)
        else
            fun.set_active(self.HallofFameGoldRank2ViewShow, false)
        end
...
function HallofFameGoldRank2View:DoClimbRankToTop()
    self:ClimbTopOne(myRankItem, function()
        self:FinishClim(myRankItem, rank_index)
    end)
end

function HallofFameGoldRank2View:DoClimbRankItem()
    local myCurOrdertPos = 0
    local temOrder = 50000
    local myCurOrder = ModelList.HallofFameModel:GetSettleClimbCurrentOrder(rank_index)
    local mypreviousOrder = ModelList.HallofFameModel:GetSettleClimbPreviousOrder(rank_index)
    local lastTier = ModelList.HallofFameModel:GetClimbTiers(rank_index)
    ...
    local callback = function()
        ...
        if 0 == callbackCount then
            if self:CheckEnterTopOne(myCurOrder) and ModelList.HallofFameModel:CheckIsBlackTire(lastTier) then
                --这里放第二名到第一名的变化内容
                self:ClimbTopOne(myRankItem, function()
                    self:FinishClim(myRankItem, rank_index)
                end)
...
function HallofFameGoldRank2View:CheckEnterTopOne(myCurOrder)
    return myCurOrder == 1
end

function HallofFameGoldRank2View:ShowContinuButton()
...
function HallofFameGoldRank2View:ClimbStage2()
    if ModelList.HallofFameModel:IsChangeTier(rank_index) then
        self:playChangeTier()
    else
...
function HallofFameGoldRank2View.OnResphonePlayerInfo()
    this._fsm:GetCurState():OnShowPlayerInfo(this._fsm)
end

function HallofFameGoldRank2View:OnShowPlayerInfo()

end

function HallofFameGoldRank2View:on_btn_help_click()
    self._fsm:GetCurState():ShowHelpView(self._fsm)
end

function HallofFameGoldRank2View:OnShowHelpView()
    Facade.SendNotification(NotifyName.ShowUI, ViewList.HallofFameGoldRewardView)
end

function HallofFameGoldRank2View:CheckIsMax()
...
function HallofFameGoldRank2View:OpenNextRankTrophyView()

end

function HallofFameGoldRank2View:OpenNextRankRewardView()
    ...
    if model:CheckIsTrueGoldUser() and model:GetSettleClimbCurrentOrder(rank_index) == 1 then
    ...
    nextRewardView = HallofFameRankNextReward:New(tiers)
    nextRewardView:SkipLoadShow(self.nextRewards.gameObject)
end

function HallofFameGoldRank2View:RankClickNextRewardBtn()
    self:StopDelayAutonReward()
    self:AnimCloeNextReward()
end

function HallofFameGoldRank2View:AddTopAreaEvent()
    Event.AddListener(NotifyName.HallofFame.FameRankClickNextRewardBtn, self.RankClickNextRewardBtn, self)

end

function HallofFameGoldRank2View:RemoveTopAreaEvent()
    Event.RemoveListener(NotifyName.HallofFame.FameRankClickNextRewardBtn, self.RankClickNextRewardBtn, self)

end

function HallofFameGoldRank2View:on_btn_OepnNextReward_click()
    self:AnimOpenNextReward()
end

function HallofFameGoldRank2View:on_btn_OepnNextRewardBlackGol_click()
    self:AnimOpenNextReward()
end

function HallofFameGoldRank2View:AnimOpenNextReward()
...
function HallofFameGoldRank2View:AnimRefreshNextReward()
    self:OpenNextRankRewardView()
    if nextRewardView then
        local tiers, difficulty = ModelList.HallofFameModel:GetSettleClimbPreviousTier(rank_index)
...
function HallofFameGoldRank2View:AnimCloeNextReward()
    if nextRewardView then
        nextRewardView:AnimCloseView()
    end
end

function HallofFameGoldRank2View:AnimOpenNextTrophy()
...
function HallofFameGoldRank2View:RefreshNextTrophy()
    -- if nextTrophyView ~= nil then
    --     local lastTiers,difficulty = ModelList.HallofFameModel:GetSettleClimbPreviousTier(rank_index)
    --     if nextTrophyView then
...
function HallofFameGoldRank2View:AnimCloeNextTrophy()
    -- if nextTrophyView then
    --     nextTrophyView:AnimCloseView()
    -- end
end

function HallofFameGoldRank2View:ShowNextTrophyFull()
    -- if nextTrophyView then
...
function HallofFameGoldRank2View:OepnNextTrophy()
    -- self:AnimOpenNextTrophy()
end

function HallofFameGoldRank2View:CheckShowTopArea()
    local model = ModelList.HallofFameModel
    if self:CheckIsMax() then
        --最高段位
        return
...
function HallofFameGoldRank2View:StopDelayAutonReward()
...
function HallofFameGoldRank2View.StopDelayAutonTrophy()
...
function HallofFameGoldRank2View:AnimRefreshRankData()
    -- if nextTrophyView then
...
--滚动结束后 检查玩家升到了第一名
function HallofFameGoldRank2View:CheckAnimEndToTopOne()
    local myRank = ModelList.HallofFameModel:GetChangeMyRank(rank_index)
...
--每经过一轮 替换model数据
function HallofFameGoldRank2View:UpdateShowNextTrophyScoreByIndex()
    ModelList.HallofFameModel:UpdateShowNextTrophyScoreByIndex(rank_index)
end

--检查上次的排名和这次新排名有变化
function HallofFameGoldRank2View:CheckHasClimb(newRank)
...
function HallofFameGoldRank2View:on_btn_continue_rank_click()
    if self:CheckIsBlackTier() then
        if HallofFameShowTopOneView:IsBingUI() then
            local topOneData = self:GetTopOneData()
            HallofFameShowTopOneView:ReplaceTopOne(topOneData)
        else
            self:ShowTopOnePlayer()
        end
    end
    ...
    fun.enable_button(self.btn_continue_rank, false)
    Event.Brocast(NotifyName.HallofFame.FameShowNewTierContinue)
end

function HallofFameGoldRank2View:ShowTopOnePlayer()
    fun.set_active(self.tournamentBlackGold, true)
    local topOneData = self:GetFakeTopOneData()
    local myUid = ModelList.PlayerInfoModel:GetUid()
    if myUid == topOneData.uid then
        --第一名是自己 分数使用lastscore
        local lastScore = ModelList.HallofFameModel:GetSettleClimbPreviousScore(rank_index)
...
function HallofFameGoldRank2View:on_btn_topOne_click()
    self:RequestPlayerInfo()
end

function HallofFameGoldRank2View:RequestPlayerInfo()
    local topOneData = self:GetTopOneData()
...
function HallofFameGoldRank2View:GetEmptyRankItem()
    if topOneDownItem then
        return topOneDownItem
    end

    local go = fun.get_instance(this.rank_item1, this.rank_content)
    topOneDownItem = HallofFameGoldRankItem:New(true)
...
--隐藏第二名
function HallofFameGoldRank2View:HideTopTwoItem()
    if topOneDownItem then
        topOneDownItem.transform.position = Vector3.New(10000, 10000, 0)
    end
end

function HallofFameGoldRank2View:GetTopOneData()
    local dt = ModelList.HallofFameModel:GetTrouamentClimbRankTopOneData(rank_index)
    return dt
end

function HallofFameGoldRank2View:GetFakeTopOneData()
    local dt = ModelList.HallofFameModel:GetTrouamentClimbRankFakeTopOneData(rank_index)
...
function HallofFameGoldRank2View:CheckSoltSpin()
