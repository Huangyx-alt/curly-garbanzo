local HallofFameArriveItem = BaseView:New("HallofFameArriveItem")
local this = HallofFameArriveItem
...
function HallofFameArriveItem:New(isNowShowState,isInitState)
	local o = {}
	self.__index = self
...
function HallofFameArriveItem:GetRoot()
	return self.root
end

function HallofFameArriveItem:SetParent(parent)
	self.parent = parent
end

function HallofFameArriveItem:Awake()
	self:on_init()
end

function HallofFameArriveItem:OnEnable(isSettle)
	self._init = true
...
function HallofFameArriveItem:OnDisable()
	self:OnDispose()
	self._init = nil
	self._data = nil
...
function HallofFameArriveItem:OnShow()
	
end

function HallofFameArriveItem:OnDispose()

end

function HallofFameArriveItem:SetItem(data,index,startScore,isUpTier,name)
	self._data = data
	self.bgIndex = index
...
function HallofFameArriveItem:SetSliderBg(isRush)
...
--设置段位信息
function HallofFameArriveItem:SetTierInfo(startScore)
...
function HallofFameArriveItem:PlayTierItemAnim(animName)
...
--设置背景相关图片
function HallofFameArriveItem:SetBg()
	local tiers = ModelList.HallofFameModel:GetTiers()
	self.tierUnLock.sprite = AtlasManager:GetSpriteByName("HallofFameScoreAtlas",TournamentScoreBgConfig[self.bgIndex][7])
	self.tierLock.sprite = AtlasManager:GetSpriteByName("HallofFameScoreAtlas",TournamentScoreBgConfig[self.bgIndex][6])
...
--显示分界线
function HallofFameArriveItem:ShowLine()
	local isShow = false
...
--设置背景颜色
function HallofFameArriveItem:SetItemBg()
...
--设置每个阶段玩家的数量
function HallofFameArriveItem:SetTierPlayerNum()
	local str = ""
...
--@params str 			显示的文本
function HallofFameArriveItem:SetPlayerItem(item,str)
...
--设置玩家头像
function HallofFameArriveItem:SetHeadIcon(uid,avatar,item,nickname)
...
--根据分数计算在上下限分数之间的占比
function HallofFameArriveItem:CalculateSliderValue(score)
	local scoreRange = ModelList.HallofFameModel:GetScoreRangeByTiers(self._tiers)
...
function HallofFameArriveItem:SetMyInfo(startScore)
...
--计算scrollView的垂直滑动值
function HallofFameArriveItem:CalculateScrollPercent(startScore)
...
--Slider动画
function HallofFameArriveItem:PlaySliderAnim(startScore)
	local scoreRange = ModelList.HallofFameModel:GetScoreRangeByTiers(self._tiers)
...
function HallofFameArriveItem:ShowChangePush()

end

function HallofFameArriveItem:ShowAnim()
	self._showAnim = true
end
