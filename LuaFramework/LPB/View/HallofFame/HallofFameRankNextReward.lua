local HallofFameRankNextReward = BaseView:New("HallofFameRankNextReward")
local this = HallofFameRankNextReward
...
function HallofFameRankNextReward:New(showTiers)
    local o = {}
    self.__index = self
...
function HallofFameRankNextReward:Awake()
    self:on_init()
end

function HallofFameRankNextReward:OnEnable()
    self:InitRewardList()
    self:ShowInfo()
end

function HallofFameRankNextReward:OnDisable()
    self:OnDispose()
end

function HallofFameRankNextReward:OnDestroy()
    nextRewardObjList = {}
    nextRewardList = {}
    self.showTiers = nil
...
function HallofFameRankNextReward:ShowInfo()
...
function HallofFameRankNextReward:OnDispose()

end

function HallofFameRankNextReward:on_btn_item_click()
    self:RequestPlayerTournamentInfo()
end


function HallofFameRankNextReward:on_btn_OepnBaseReward_click()
    self.openAutoCloseView = true
    Event.Brocast(NotifyName.HallofFame.FameRankClickNextRewardBtn)
end

function HallofFameRankNextReward:AnimOpenView()
...
function HallofFameRankNextReward:AnimCloseView()
    Anim.scale_to_xy(self.go,0, 1, 0.25,  function()
        self.canvasgroup.alpha = 0
    end)
end

function HallofFameRankNextReward:FirstAnimCloseView()
...
function HallofFameRankNextReward:InitRewardList()
...
function HallofFameRankNextReward:GetRankNextReward()
    local useTiers = nil
    if self.showTiers then
        useTiers = self.showTiers
    else
        useTiers = ModelList.HallofFameModel:GetTiers()
    end
    return self:GetNextRewardByTiers(useTiers)
end

function HallofFameRankNextReward:GetNextRewardByTiers(tiers)
...
function HallofFameRankNextReward:RefreshTiers(tiers)
...
function HallofFameRankNextReward:CheckHasOpen()
