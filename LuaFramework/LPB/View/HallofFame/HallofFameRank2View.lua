...
HallofFameRank2View = TopBarControllerView:New("HallofFameRank2View", "TournamentAtlas")
local this = HallofFameRank2View
...
function HallofFameRank2View:Awake(obj)
    self:on_init()
end

function HallofFameRank2View:OnEnable(params)
...
function HallofFameRank2View:ShowBuffEffect()
...
function HallofFameRank2View:IsSelfHasSprintBuff()
    return ModelList.HallofFameModel:HasSprintBuff()
end

function HallofFameRank2View:OnDisable()
...
function HallofFameRank2View:BuildFsm()
    self:DisposeFsm()
    self._fsm = Fsm.CreateFsm("HallofFameRank2View", self, {
...
function HallofFameRank2View:DisposeFsm()
    if self._fsm then
        self._fsm:Dispose()
        self._fsm = nil
    end
...
function HallofFameRank2View:on_btn_continue_click()
    self._fsm:GetCurState():PlayExite(self._fsm)
end

function HallofFameRank2View:CloseTopBarParent()
    self:on_btn_continue_click()
end

function HallofFameRank2View:PlayEnter()
...
function HallofFameRank2View:initSpeedUp()
...
function HallofFameRank2View:on_btn_speedUp_click()
...
function HallofFameRank2View:PlayExite()
...
function HallofFameRank2View:playChangeTier()
...
function HallofFameRank2View:playChangeRoll()
...
function HallofFameRank2View:ShowNextReward()
...
function HallofFameRank2View:ShowNewTierContinue()
...
--获取自身初始节点位置
function HallofFameRank2View:GetTargetInitIndex(climbData, climbListData)
    local lastIndex = climbListData.lastSectionIndex
    local currentIndex = climbListData.mySectionIndex
    return lastIndex
end

function HallofFameRank2View:InitRankItem()
    local pos = 0
    ...
    for i = #climbData, 1, -1 do
        if myUid == climbData[i].uid and startIndex == climbData[i].order then
            ...
            if myRankItem == nil then
                ...
                myRankItem = HallofFameMyRankItem:New(true)
            ...
            if climbData[i].uid ~= myUid then
                ...
                if rankItemList[itemIndex] == nil then
                    ...
                    item = HallofFameRankItem:New(true)
                    item:SetParent(this)
...
function HallofFameRank2View:SetTopReward(tiers, difficulty)
...
function HallofFameRank2View:StartClimb()
...
function HallofFameRank2View:FlyScore(callback)
    if myRankItem then
        myRankItem:FlyScore(callback, ModelList.HallofFameModel:GetSettleClimbCurrentScore(rank_index))
    end
end

function HallofFameRank2View:DoClimbRankItem()
...
function HallofFameRank2View:ShowContinuButton()
...
function HallofFameRank2View:ClimbStage2()
...
function HallofFameRank2View.OnResphonePlayerInfo()
    this._fsm:GetCurState():OnShowPlayerInfo(this._fsm)
end

function HallofFameRank2View:OnShowPlayerInfo()

end

function HallofFameRank2View:on_btn_help_click()
    self._fsm:GetCurState():ShowHelpView(self._fsm)
end

function HallofFameRank2View:OnShowHelpView()
    Facade.SendNotification(NotifyName.ShowUI, ViewList.HallofFameRewardView)
end

function HallofFameRank2View:CheckIsMax()
...
function HallofFameRank2View:OpenNextRankTrophyView()
    ...
    nextTrophyView = HallofFameRankNextTrophy:New(lastTiers)
    nextTrophyView:SkipLoadShow(self.nextTrophy.gameObject)
end

function HallofFameRank2View:OpenNextRankRewardView()
...
function HallofFameRank2View:AnimRefreshNextReward()
    self:OpenNextRankRewardView()
    if nextRewardView then
        local tiers, difficulty = ModelList.HallofFameModel:GetSettleClimbPreviousTier(rank_index)
...
function HallofFameRank2View:AnimCloeNextReward()
    if nextRewardView then
        nextRewardView:AnimCloseView()
    end
end

function HallofFameRank2View:AnimOpenNextTrophy()
...
function HallofFameRank2View:RefreshNextTrophy()
    if nextTrophyView ~= nil then
        local lastTiers, difficulty = ModelList.HallofFameModel:GetSettleClimbPreviousTier(rank_index)
        if nextTrophyView then
...
function HallofFameRank2View:AnimCloeNextTrophy()
    if nextTrophyView then
        nextTrophyView:AnimCloseView()
    end
end

function HallofFameRank2View:ShowNextTrophyFull()
    if nextTrophyView then
...
function HallofFameRank2View:OepnNextTrophy()
    self:AnimOpenNextTrophy()
end

function HallofFameRank2View:CheckShowTopArea()
    local model = ModelList.HallofFameModel
    if self:CheckIsMax() then
        --最高段位
        return
...
function HallofFameRank2View:StopDelayAutonReward()
...
function HallofFameRank2View.StopDelayAutonTrophy()
...
function HallofFameRank2View:AnimRefreshRankData()
    if nextTrophyView then
...
--滚动结束后 检查玩家升到了第一名
function HallofFameRank2View:CheckAnimEndToTopOne()
...
--每经过一轮 替换model数据
function HallofFameRank2View:UpdateShowNextTrophyScoreByIndex()
    ModelList.HallofFameModel:UpdateShowNextTrophyScoreByIndex(rank_index)
end

--检查上次的排名和这次新排名有变化
function HallofFameRank2View:CheckHasClimb(newRank)
...
function HallofFameRank2View:on_btn_continue_rank_click()
    fun.enable_button(self.btn_continue_rank, false)
    Event.Brocast(NotifyName.HallofFame.FameShowNewTierContinue)
    -- Facade.SendNotification(NotifyName.CloseUI,self)
end

function HallofFameRank2View:CheckSoltSpin()
