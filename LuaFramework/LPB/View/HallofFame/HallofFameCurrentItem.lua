...
local HallofFameCurrentItem = HallofFameNoArriveItem:New()
local this = HallofFameCurrentItem
...
function HallofFameCurrentItem:New(isNowShowState,isInitState)
	local o = {}
	self.__index = self
...
function HallofFameCurrentItem:OnEnable(isSettle)
	self._isSettle = isSettle
	self._init = true
	self:InitSprintBuff()
end

function HallofFameCurrentItem:SetItem(data,index,startScore,isUpTier,name)
	self._data = data
	self.bgIndex = index
...
function HallofFameCurrentItem:SetTierInfo(startScore)
...
function HallofFameCurrentItem:SetItemBg()
	...
	fun.SetSprite(bgLineImg,AtlasManager:GetSpriteByName("HallofFameScoreAtlas",TournamentScoreBgConfig[self.bgIndex][11]))
...
--设置自己的信息(头像、积分、进度条)
function HallofFameCurrentItem:SetMyInfo(startScore,isUpTier,startRatio)
	local scoreRange = ModelList.HallofFameModel:GetScoreRangeByTiers(self._tiers)
...
function HallofFameCurrentItem:SetSliderBg(isRush)
...
--Slider动画
function HallofFameCurrentItem:PlaySliderAnim(startScore,startRatio)
	self:SetSliderBg(true)
	local scoreRange = ModelList.HallofFameModel:GetScoreRangeByTiers(self._tiers)
...
function HallofFameCurrentItem:CalculateScrollPercent(startScore)
...
--加成相关
function HallofFameCurrentItem:InitSprintBuff()
...
function HallofFameCurrentItem:CheckSprintBuff()
...
function HallofFameCurrentItem:HasSprintBuff()
	return ModelList.HallofFameModel:HasSprintBuff()
end

function HallofFameCurrentItem:OnBtnTournamentBuffClick(target)
...
function HallofFameCurrentItem:GetBuffPosParams()
	local params = {}
	params.buffX = 20
	params.buffY = 20
	params.bubbleX = 0
...
--显示自己阶段第一到第五名玩家
function HallofFameCurrentItem:ShowCurFirstStagePlayer()
	local myRankInfo = ModelList.HallofFameModel:GetMyRankInfo()
...
--显示自己最近的玩家
function HallofFameCurrentItem:ShowNearPlayer(myRankInfo,myRatio,upStageRatio,downStageRatio,isShowFirst)
	local sliderHeight = fun.get_component(self.tierSlider,fun.RECT).rect.height
	local rankInfoList = ModelList.HallofFameModel:GetServerRankInfo()
	...
	if isShowFirst then
		if myRankInfo.order > 1 then
			if self._data and self._data.topPlayers then
				for _, playerData in ipairs(self._data.topPlayers) do
...
--显示阶段之间玩家数量
function HallofFameCurrentItem:ShowCurStagePlayerNum()
	local myRankInfo = ModelList.HallofFameModel:GetMyRankInfo()
...
function HallofFameCurrentItem:ShowChangePush()
	local count=fun.get_child_count(self.root)
	if count > 14 then
		for i = 1, count do
...
--获取在阶段区间
function HallofFameCurrentItem:GetStageIndex(myRankInfo)
