...
local HallofFameSettleView = BaseDialogView:New("HallofFameSettleView", "TournamentSettleTopPlayerAtlas")
local this = HallofFameSettleView
...
function HallofFameSettleView:Awake(obj)
    self:on_init()
end

function HallofFameSettleView:OnEnable(params)
    ...
    self:SetMyTrophy()
    CommonState.BuildFsm(self, "HallofFameSettleView")
    self:PlayEnterAnim()
    self:SetCollectInfo()
...
function HallofFameSettleView:SetIsTopPlayer()
...
function HallofFameSettleView:PlayEnterAnim()
    local myRank = ModelList.HallofFameModel:GetMyRank()
    local clipName = ""
    local motionName = ""
...
function HallofFameSettleView:OnDisable()
...
function HallofFameSettleView:DelayShowRewardByRank()
    local myRank = ModelList.HallofFameModel:GetMyRank()
...
function HallofFameSettleView:StartCountDown()
...
function HallofFameSettleView:SetTopPlayerInfo()
    local topPlayerInfo = ModelList.HallofFameModel:GetResultTopPlayerData()
    local myRank = ModelList.HallofFameModel:GetMyRank()
...
function HallofFameSettleView:SetPlayerInfo(index, playerInfo, playerObj, myRank)
...
function HallofFameSettleView:ReBindRankImg(playerObj, rank)
    if playerObj then
...
--排名在前三 放大背景 下移节点
function HallofFameSettleView:SetViewNear()
    local nearVlaue = 1.3
...
--排名在前三 放大背景 上移界面
function HallofFameSettleView:SetViewFar()
    local myRank = ModelList.HallofFameModel:GetMyRank()
    local nearVlaue = 1
    local farMoveDownPos = self.moveDown.transform.localPosition
...
function HallofFameSettleView:SetMyRewardInfo()
    local myRank = ModelList.HallofFameModel:GetMyRank()
    if myRank <= 3 and isTopPlayer then
...
function HallofFameSettleView:SetCollectInfo()
    local myRank = ModelList.HallofFameModel:GetMyRank()
    if myRank <= 3 and isTopPlayer then
...
function HallofFameSettleView:SetMyTrophy()
...
function HallofFameSettleView:TopPlayerCollectRewardCloseView()
...
function HallofFameSettleView:OepnTopPlayerBox(myRank)
    ...
    Facade.SendNotification(NotifyName.ShowUI, ViewList.HallofFameSettleRewardView, nil, false, { startMovePos = startMovePos, rewardBox = rewardBox })
end

function HallofFameSettleView:on_btn_collect_click()
...
function HallofFameSettleView:DelayReturnState()
...
function HallofFameSettleView:ResphoneRewardRequest()
    local rewards = ModelList.HallofFameModel:GetMyRewardItem()
    self:FlyRewardToTopArea()
end

function HallofFameSettleView:FlyRewardToTopArea()
    LuaTimer:SetDelayFunction(1, function()
...
function HallofFameSettleView:RequestReward()
...
function HallofFameSettleView:GetTopRewardBox(rank)
    return self.topRewardBoxList[rank]
end

function HallofFameSettleView:PlayExit()
...
function HallofFameSettleView:on_btn_continue_click()
    local animBtnConeTinue = fun.get_component(self.btn_continue, fun.ANIMATOR)
...
function HallofFameSettleView:UpdateTargetClickBoxBtnEnable(index, state)
    fun.enable_button(self["btn_box" .. index], state)
end

function HallofFameSettleView:OpenTopPlayerRewardTip(index, offset, dir, pivot)
...
function HallofFameSettleView:SetTopPlayerUseToShow()
    self.topPlayerReward = {}
...
function HallofFameSettleView:GetTopPlayerUseToShow(rankIndex)
...
function HallofFameSettleView:GetShowTipWidth(rewardsNum)
    return (rewardsNum - 1) * 150 + 180
end

function HallofFameSettleView.ReqWeekSettleError()
