local WinZoneConst = require "View/WinZone/WinZoneConst"
require "State/Common/CommonState"
local HallofFameScoreView = BaseView:New('HallofFameScoreView', "HallofFameScoreAtlas")
local this = HallofFameScoreView
...
local HallofFameNoArriveItem = require "View/HallofFame/HallofFameNoArriveItem"
local HallofFameGoldItem = require "View/HallofFame/HallofFameGoldItem"
local HallofFameTopTierInfoItem = require "View/HallofFame/HallofFameTopTierInfoItem"
local HallofFameRankUpView = require "View/HallofFame/HallofFameRankUpView"
local HallofFameArriveFullItem = require "View/HallofFame/HallofFameArriveFullItem"
...
HallFameCountObjectPool={}
HallFameHeadObjectPool={}

local itemList={}

function HallofFameScoreView:Awake(obj)
    --this.update_x_enabled = true
    self:on_init()
end

--默认加载当前段位，所以把当前段位的存入table ， 取的时候table是默认有值的，相同引用,默认取第一个就行
function HallFameObjectPop(itemType)
	if itemType==1 then
		return table.remove(HallFameCountObjectPool,1)
	elseif itemType==2 then
		return table.remove(HallFameHeadObjectPool,1)
	end
end
--不限最大数量，直接添加
function HallFameObjectPush(itemType,item)
	if itemType==1 then
		table.insert(HallFameCountObjectPool,item)
	elseif itemType==2 then
		table.insert(HallFameHeadObjectPool,item)
	end
end

function HallofFameScoreView:DisablebVictoryShopBtn()
    self.disablebVictoryShopBtn = true
end

function HallofFameScoreView:OnEnable(params)
    log.log("HallofFameScoreView:OnEnable params is ", params)
    showRangTop = UnityEngine.Screen.height * 2
    showRangDown = 0 - UnityEngine.Screen.height

    --获取总共段位数量
    totalTier = ModelList.HallofFameModel:GetTotalTrophy()
    tiers,difficulty = ModelList.HallofFameModel:GetTiers()
    Facade.RegisterView(self)
    self:RegisterEvent()
    CommonState.BuildFsm(self,"HallofFameScoreView")
    --界面入场动画
    ...
    if params then
        ...
        if not params.lastWeekRankInfo then
            lastWeekRankInfo = ModelList.HallofFameModel:GetMyRankInfo()
        else
    ...
    if self.disablebVictoryShopBtn then
        fun.set_active(self.btn_victory_shop, false)
    else
        fun.set_active(self.btn_victory_shop, true)
    end

    self.victory_item_count.text = ModelList.ItemModel.get_musicalnoteCount()
end

function HallofFameScoreView:GetUiCamera()
    local cameraGo = nil
    if isSettle then
...
--function HallofFameScoreView:on_x_update()
--end

--设置进度特效
function HallofFameScoreView:SetEffectVisible(visible)
    if visible then
...
--根据段位获取背景索引
function HallofFameScoreView:GetBgIndex(tiers)
...
--设置背景
function HallofFameScoreView:SetBg()
    if isSettle then
...
function HallofFameScoreView:OnDisable()
    log.log("HallofFameScoreView:OnDisable")
    Event.Brocast(EventName.Event_show_halloffame_score_view)
    ...
    animTier = 0
    maskCanClick = false
    HallofFameRankUpView:OnDisable()
    self:ClearBuffLoopTimer()
    self.disablebVictoryShopBtn = false
    HallFameCountObjectPool={}
	HallFameHeadObjectPool={}
end

--退出
function HallofFameScoreView:PlayExite()
...
function HallofFameScoreView:TryExite()
    if self.isShow then
        log.log("HallofFameScoreView:TryExite 成功")
        self:PlayExite()
    else
        log.log("HallofFameScoreView:TryExite 失败")
...
function HallofFameScoreView:ClearTimeList()
...
--初始化的时候，计算在屏幕中的段位ID
function HallofFameScoreView:GetVisableTierIndex(currTier,maxTier)
	if currTier==1 then
		return {1,2}
...
function HallofFameScoreView:SetPanel()
    --初始化列表
    ...
    for i = totalTier, 1,-1 do
        ...
        if i < tiers  then
            if isUpTier  then
                ...
                local scoreRange = ModelList.HallofFameModel:GetScoreRangeByTiers(i)
                if (oldScore and oldScore < scoreRange[2] and oldScore >= scoreRange[1]) or (oldScore and oldScore < scoreRange[2] and oldScore < scoreRange[1])  then
                    --真正开头的格子
                    self:LoadTierItems(self.arriveFullItem,HallofFameArriveFullItem,i,tierItemType.Arrive,isNowShowState,true)
                else
                    --已经结束的格子 本次流程不领奖
                    self:LoadTierItems(self.arriveItem,HallofFameArriveItem,i,tierItemType.Arrive,isNowShowState,true)
                end
            else
                self:LoadTierItems(self.arriveItem,HallofFameArriveItem,i,tierItemType.Arrive,isNowShowState,true)
            end
        elseif i == tiers then
            self:LoadTierItems(self.currentItem,HallofFameCurrentItem,i,tierItemType.Current,isNowShowState,true)
        else
            self:LoadTierItems(self.noArriveItem,HallofFameNoArriveItem,i,tierItemType.NoArrive,isNowShowState,true)
        end
    end
    ...
    if ModelList.HallofFameModel:CheckHasStateAward() then
        ModelList.HallofFameModel:C2S_RequestStageReward()
    end
...
function HallofFameScoreView:SetMaskVisible(visible)
    fun.set_active(self.btn_mask,visible)
end

--加载段位item
function HallofFameScoreView:LoadTierItems(prefab,view,index,tierType,isNowShowState,isInit)
...
function HallofFameScoreView:GetUIVisable(cam,ui)
...
--设置段位item
function HallofFameScoreView:SetTierItems()
	if tiers==1 then	
...
function HallofFameScoreView:IsCurrentItem(index, v, isInit)
    if #tierItemList < index then
		log.log("周榜流程问题检查 设置数据 IsCurrentItem 需要屏蔽 超出界限 index" , #tierItemList , index)
		return
	end
	local data = ModelList.HallofFameModel:GetStageInfo(index)
...
function HallofFameScoreView:SetItemListVisible()
...
--计算scroll滚动的百分比
function HallofFameScoreView:CalculateScrollPercent()
    ...
    local myInfo = ModelList.HallofFameModel:GetMyRankInfo()
    if not myInfo then
        return
    end

    local scoreRange = ModelList.HallofFameModel:GetScoreRangeByTiers(tiers)
...
--定位在开始的位置
function HallofFameScoreView:SetScrollStratPos()
    local totalHight1 = 320
    for i = totalTier, 1,-1 do
        if i > tiers then
    ...
    local totalHight2 = totalHight1
    local scoreRange = ModelList.HallofFameModel:GetScoreRangeByTiers(lastWeekTier)
...
--滑动至当前段位
function HallofFameScoreView:ScrollToCurrent()
...
function HallofFameScoreView:UpdateByScroll(value)
    if not isUpTier then
...
--监听滑动事件
function HallofFameScoreView:AddListenerScrollChaged()
...
function HallofFameScoreView:SetBottomArrow(type)
    ...
    if self.arrowStatus ~= type then
        ...
        if self.arrowStatus == 1 then
            self.arrow.sprite = AtlasManager:GetSpriteByName("HallofFameScoreAtlas","ZBJfTxFhjt02")
        elseif self.arrowStatus == 2 then
            self.arrow.sprite = AtlasManager:GetSpriteByName("HallofFameScoreAtlas","ZBJfTxFhjt01")
        end
...
--设置当前段位奖励
function HallofFameScoreView:SetMyRankTierAward()
    local data = ModelList.HallofFameModel:GetStageInfo(tiers)
    self.tierInfoView = HallofFameTopTierInfoItem:New()
...
function HallofFameScoreView:PlayMyTierItemAnim(animName)
...
--设置底部礼物盒子
function HallofFameScoreView:SetGiftBox()
    log.log("HallofFameScoreView:SetGiftBox remainTime is ", ModelList.HallofFameModel:GetRemainTime())
    local rank = ModelList.HallofFameModel:GetRankMySectionIndex()
...
function HallofFameScoreView:on_btn_close_click()
    self:PlayExite()
end

function HallofFameScoreView:on_btn_help_click()
    local viewName = ""
    if ModelList.HallofFameModel:CheckIsTrueGoldUser() then
        viewName = "HallofFameGoldRankView"
    else
        viewName = "HallofFameRankView"
    end
    Facade.SendNotification(NotifyName.ShowUI, ViewList[viewName])
end

function HallofFameScoreView:on_btn_gift_click()
    local rankInfo = ModelList.HallofFameModel:GetMyRankInfo()
...
function HallofFameScoreView:on_btn_victory_shop_click()
    if self._exit_callback then
        log.log("HallofFameScoreView:on_btn_victory_shop_click error has close callback")
    else
        self:PlayExite()
        Facade.SendNotification(NotifyName.ShopView.PopupShop, PopupViewType.show, nil, SHOP_TYPE.SHOP_TYPE_OFFERS, function() end)
    end
end

function HallofFameScoreView:OpenTopPlayerRewardTip(rewards,offset , dir, pivot)
    if not rewards then
        return
    end
...
--礼盒按钮点后弹出奖励面板的宽度
function HallofFameScoreView:GetShowTipWidth(rewardsNum)
    return (rewardsNum - 1) * 150 + 320
end

function HallofFameScoreView:on_btn_continue_click()
    self:PlayExite()
end

--箭头点击
function HallofFameScoreView:on_btn_open_click()
    self:ScrollToCurrent()
end

--领取阶段奖励回调
function HallofFameScoreView.OnTournamentStageReward(data,stateAwardInfo)
    this.showAnim = true
end

function HallofFameScoreView:PlayScoreChangeSound()
...
--播放当前段位动画
function HallofFameScoreView:PlayCurrentTierAnim(tiersIndex)
	local rankInfo = ModelList.HallofFameModel:GetMyRankInfo()
	if not rankInfo then return end
	local startAward = ModelList.HallofFameModel:GetRankLastAward(tiersIndex, difficulty)
	local topAward = ModelList.HallofFameModel:GetFirstAward(tiersIndex, difficulty)
...
function HallofFameScoreView:SetTrophyName(tier)
...
function HallofFameScoreView:HasSprintBuff()
    return ModelList.HallofFameModel:HasSprintBuff()
end

function HallofFameScoreView:FastShowReward()
	local scoreRange = ModelList.HallofFameModel:GetScoreRangeByTiers(animTier);
...
function HallofFameScoreView:on_btn_mask_click()
    ...
    HallofFameRankUpView:OnDestroy()
    animTier = 0
end

function HallofFameScoreView:UpdateDoubleBuffState()
    log.log("HallofFameScoreView:UpdateDoubleBuffState 1")
    if fun.is_null(self.img_double) then
        return
    end

    fun.set_active(self.img_double, false)
    local buffId = WinZoneConst.BuffId1
    log.log("HallofFameScoreView:UpdateDoubleBuffState 2", buffId)
    ...
    log.log("HallofFameScoreView:UpdateDoubleBuffState 3", remainTime, expireTime, curTime)
end

function HallofFameScoreView:SetBuffLeftTime(remainTime)
...
function HallofFameScoreView:ClearBuffLoopTimer()
...
function HallofFameScoreView.OnMusicNoteNumChange(params)
    if fun.is_not_null(this.victory_item_count) then
        this.victory_item_count.text = ModelList.ItemModel.get_musicalnoteCount()
    end
end

function HallofFameScoreView:RegisterEvent()
    Event.AddListener(EventName.Event_items_change, self.OnItemsChange, self)
end

function HallofFameScoreView:OnItemsChange(params)
    --Event.Brocast(EventName.Event_items_change,{items = value, type = key})
    if not params then
        return
    end

    if params.type == WinZoneConst.BuffId1 then --ITEMS_TYPE.ITEMS_TYPE_SPECIAL_GAME_BUFF
        self:UpdateDoubleBuffState()
    end
end
...
--- 结算数据检查有没有锤力器道具
function HallofFameScoreView:CheckSmash(isSettle)
    if isSettle then
...
function HallofFameScoreView:CheckSoltSpin(isSettle)
...
function HallofFameScoreView:UnRegisterEvent()
    Event.RemoveListener(EventName.Event_items_change, self.OnItemsChange, self)
end

this.NotifyList = {
    {notifyName = NotifyName.HallofFame.FameReqStageReward,func = this.OnTournamentStageReward},
    {notifyName = NotifyName.ShopView.MusicNoteNumChange, func = this.OnMusicNoteNumChange},
}
