...
function HallofFameTopPlayerItem:BindUI()
	if hasBindUI then
		return
	end
...
function HallofFameTopPlayerItem:SetItem(params)
...
function HallofFameTopPlayerItem:InitHeadIcon()
...
function HallofFameTopPlayerItem:InitFrameIcon()
	local myUid = ModelList.PlayerInfoModel:GetUid()
	local model = ModelList.PlayerInfoSysModel
	if myUid == tonumber(itemData.uid) then
...
function HallofFameTopPlayerItem:InitScore()
	textScore.text = fun.formatNum(itemData.score) or 0
end

function HallofFameTopPlayerItem:InitName()
	userName.text = itemData.nickname
	clubName.text = itemData.clubName
end

--显示自己的边框
function HallofFameTopPlayerItem:InitSelect()

end

return HallofFameTopPlayerItem