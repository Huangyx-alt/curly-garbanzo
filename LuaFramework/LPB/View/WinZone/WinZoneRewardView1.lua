local WinZoneRewardView1 = BaseDialogView:New('WinZoneRewardView1', "WinZoneReward1AtlasInMain")
local this = WinZoneRewardView1
...
function WinZoneRewardView1:Awake()
end

function WinZoneRewardView1:OnEnable()
    Facade.RegisterViewEnhance(self)
    self:ClearMutualTask()
    self.rewardItems = {}
end

function WinZoneRewardView1:on_after_bind_ref()
    self:InitView()
end

function WinZoneRewardView1:OnDisable()
    Facade.RemoveViewEnhance(self)
    self:ClearCloseCallback()
    self.data = nil
end

function WinZoneRewardView1:SetData(data)
...
function WinZoneRewardView1:InitView()
    ...
    if fun.is_not_null(self.anima) then
        local task = function()
            AnimatorPlayHelper.Play(self.anima, {"start", "WinZoneRewardView1_start"}, false, function()
...
function WinZoneRewardView1:InitReward()
...
function WinZoneRewardView1:CreateRewardItem(rewardData)
...
function WinZoneRewardView1:CollectRewards()
...
function WinZoneRewardView1:CloseSelf()
    if fun.is_not_null(self.anima) then
        local task = function()
            AnimatorPlayHelper.Play(self.anima, {"end", "WinZoneRewardView1_end"}, false, function()
                self:MutualTaskFinish()
...
function WinZoneRewardView1:AddCloseCallback(cb)
    log.log("WinZoneRewardView1:AddCloseCallback()")
    if not cb then
        return
    end
...
function WinZoneRewardView1:ClearCloseCallback()
    log.log("WinZoneRewardView1:ClearCloseCallback()")
    self.closeCallbackList = nil
end

function WinZoneRewardView1:ExecuteCloseCallback()
    log.log("WinZoneRewardView1:ExecuteCloseCallback()")
...
function WinZoneRewardView1:on_btn_close_click()
    self:CloseSelf()
end

function WinZoneRewardView1:on_btn_collect_click()
    local task = function()
        self:CollectRewards()
    end
