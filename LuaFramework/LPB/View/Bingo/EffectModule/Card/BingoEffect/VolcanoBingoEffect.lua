...
local VolcanoBingoEffect = BaseBingoEffect:New("VolcanoBingoEffect")
local this = VolcanoBingoEffect
setmetatable(VolcanoBingoEffect, BaseBingoEffect)
local private = {}

function VolcanoBingoEffect:ShowBingoOrJackpot(bingoData)
...
function VolcanoBingoEffect:BingoEffect(mapindex, anima_name, cellIndex, cardid, bingoType)
...
function VolcanoBingoEffect:JackpotEffect(mapindex, anima_name, cellIndex, cardid)
    local par = self.cardView:GetCardMap(mapindex)
    local pos = par.transform.position
...
function VolcanoBingoEffect:RampageBingoEffect(mapindex, anima_name, cellIndex, cardid)
    local par = self.cardView:GetCardMap(mapindex)
    local pos = par.transform.position

    local eff1 = GoPool.pop("VolcanoJackpot")
    if eff1 then
    ...
    self:ShowEffectOnSmallCard("VolcanoJackpot", mapindex, 2)
end

function VolcanoBingoEffect:ShowEffectOnSmallCard(pollName, cardId, effectType, anima_name)
...
function private.PlayBingoAudio(bingoCount)
    if bingoCount == 1 then
        ...
        UISound.play("lavabingo")
    elseif bingoCount == 2 then
        UISound.play("bingo_firework")
        UISound.play("lava2bingo")
    elseif bingoCount == 3 then
        UISound.play("bingo_firework")
        UISound.play("lava3bingo")
    elseif bingoCount == 4 then
        UISound.play("bingo_firework")
        UISound.play("lava4bingo")
    end
end
