...
local CandyBingoEffect = BaseBingoEffect:New("CandyBingoEffect")
local this = CandyBingoEffect
setmetatable(CandyBingoEffect, BaseBingoEffect)
local private = {}

function CandyBingoEffect:ShowBingoOrJackpot(bingoData)
...
function CandyBingoEffect:BingoEffect(mapindex, anima_name, cellIndex, cardid, bingoType)
    ...
    if eff1 then
        ...
        --fun.set_same_position_with(eff1, par)
...
function CandyBingoEffect:JackpotEffect(mapindex, anima_name, cellIndex, cardid)
    ...
    local eff1 = GoPool.pop("CandyJackpot")
    if eff1 then
        ...
        --eff1.gameObject:SetActive(false)
        fun.set_gameobject_pos(eff1, pos.x, pos.y, pos.z, false)
        fun.set_rect_offset_local_pos(eff1, -10, 11)
        --eff1.gameObject:SetActive(true)
    ...
    self:ShowEffectOnSmallCard("CandyJackpot", mapindex, 2)
end

function CandyBingoEffect:ShowEffectOnSmallCard(pollName, cardId, effectType, anima_name)
...
function private.PlayBingoAudio(bingoCount)
	if bingoCount == 1 then
		...
		UISound.play("candysweetsbingo")
	elseif bingoCount == 2 then
		UISound.play("bingo_firework")
		UISound.play("candysweetsdoublebingo")
	elseif bingoCount == 3 then
		UISound.play("bingo_firework")
		UISound.play("candysweetstriplebingo")
	elseif bingoCount == 4 then
		UISound.play("jackpot_firework")
		UISound.play("candysweetsjackpot")
	end
end
