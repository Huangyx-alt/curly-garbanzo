...
local PirateShipBingoEffect = BaseBingoEffect:New("PirateShipBingoEffect")
local this = PirateShipBingoEffect
setmetatable(PirateShipBingoEffect, BaseBingoEffect)
local private = {}

function PirateShipBingoEffect:ShowBingoOrJackpot(bingoData, cb)
    for k, v in pairs(bingoData) do
        ...
        --if v.jackpot == 0 then
            ...
            if bingoCount < 5 then
                self:BingoEffect(mIndex, anima_name, v.first_num, mIndex, bingoType, cb)
            else
        ...
        --    self:JackpotEffect(mIndex, "jackpot", v.first_num, mIndex, cb)
        --end
    end
end

function PirateShipBingoEffect:BingoEffect(mapindex, anima_name, cellIndex, cardid, bingoType, cb)
...
function PirateShipBingoEffect:JackpotEffect(mapindex, anima_name, cellIndex, cardid, cb)
    local par = self.cardView:GetCardMap(mapindex)
    local pos = par.transform.position
...
function PirateShipBingoEffect:RampageBingoEffect(mapindex, anima_name, cellIndex, cardid, cb)
    local par = self.cardView:GetCardMap(mapindex)
    local pos = par.transform.position

    local eff1 = GoPool.pop("PirateShipJackpot")
    if eff1 then
    ...
    --self:ShowEffectOnSmallCard("PirateShipJackpot", mapindex, 2)
end

function PirateShipBingoEffect:ShowEffectOnSmallCard(pollName, cardId, effectType, anima_name)
...
function PirateShipBingoEffect:GetBingoAnimaName(bingoCount)
    local anima_name, spineAnimName = "bingo", "enter"
...
function PirateShipBingoEffect:GetBingoPrefabName(bingoCount)
    return "PirateShipbingo"
end
...
function private.PlayBingoAudio(bingoCount)
    if bingoCount == 1 then
        ...
        UISound.play("piratebingo")
    elseif bingoCount == 2 then
        UISound.play("bingo_firework")
        UISound.play("piratedoublebingo")
    elseif bingoCount == 3 then
        UISound.play("bingo_firework")
        UISound.play("piratetriplebingo")
    elseif bingoCount == 4 then
        UISound.play("bingo_firework")
        UISound.play("piratequadrabingo")    
    elseif bingoCount == 5 then
        UISound.play("bingo_firework")
