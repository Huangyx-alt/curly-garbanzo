...
local DragonFortuneBingoEffect = BaseBingoEffect:New("DragonFortuneBingoEffect")
local this = DragonFortuneBingoEffect
setmetatable(DragonFortuneBingoEffect, BaseBingoEffect)
local private = {}

function DragonFortuneBingoEffect:ShowBingoOrJackpot(bingoData)
...
function DragonFortuneBingoEffect:BingoEffect(mapindex, anima_name, cellIndex, cardid, bingoType)
...
function DragonFortuneBingoEffect:JackpotEffect(mapindex, anima_name, cellIndex, cardid)
    local par = self.cardView:GetCardMap(mapindex)
    local pos = fun.get_gameobject_pos(par)
...
function DragonFortuneBingoEffect:RampageBingoEffect(mapindex, anima_name, cellIndex, cardid)
    local par = self.cardView:GetCardMap(mapindex)
    local pos = par.transform.position

    local eff1 = GoPool.pop("DragonFortuneJackpot")
    if eff1 then
    ...
    self:ShowEffectOnSmallCard("DragonFortuneJackpot", mapindex, 2)
end

function DragonFortuneBingoEffect:ShowEffectOnSmallCard(pollName, cardId, effectType, anima_name)
...
function private.PlayBingoAudio(bingoCount)
    if bingoCount == 1 then
        ...
        UISound.play("dragonBingo")
    elseif bingoCount == 2 then
        UISound.play("bingo_firework")
        UISound.play("dragonDoublebingo")
    elseif bingoCount == 3 then
        UISound.play("bingo_firework")
        UISound.play("dragonTriplebingo")
    elseif bingoCount == 4 then
        UISound.play("bingo_firework")
        UISound.play("dragonQuadrabingo")
    end
end
