...
local GoldenPigBingoEffect = BaseBingoEffect:New("GoldenPigBingoEffect")
local this = GoldenPigBingoEffect
setmetatable(GoldenPigBingoEffect, BaseBingoEffect)
local private = {}

function GoldenPigBingoEffect:ShowBingoOrJackpot(bingoData)
...
function GoldenPigBingoEffect:BingoEffect(mapindex, anima_name, cellIndex, cardid, bingoType)
...
function GoldenPigBingoEffect:JackpotEffect(mapindex, anima_name, cellIndex, cardid)
    local par = self.cardView:GetCardMap(mapindex)
    local pos = fun.get_gameobject_pos(par)
...
function GoldenPigBingoEffect:RampageBingoEffect(mapindex, anima_name, cellIndex, cardid)
    local par = self.cardView:GetCardMap(mapindex)
    local pos = fun.get_gameobject_pos(par)

    local eff1 = GoPool.pop("GoldenPigJackpot")
    if eff1 then
    ...
    self:ShowEffectOnSmallCard("GoldenPigJackpot", mapindex, 2)
end

function GoldenPigBingoEffect:ShowEffectOnSmallCard(pollName, cardId, effectType, anima_name)
...
function private.PlayBingoAudio(bingoCount)
    if bingoCount == 1 then
        ...
        UISound.play("goldenpigBingo")
    elseif bingoCount == 2 then
        UISound.play("bingo_firework")
        UISound.play("goldenpigDoublebingo")
    elseif bingoCount == 3 then
        UISound.play("bingo_firework")
        UISound.play("goldenpigTriplebingo")
    elseif bingoCount == 4 then
        UISound.play("bingo_firework")
        UISound.play("goldenpigQuadrabingo")
    end
end
