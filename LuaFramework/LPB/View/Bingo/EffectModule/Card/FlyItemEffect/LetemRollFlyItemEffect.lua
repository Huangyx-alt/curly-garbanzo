...
local LetemRollFlyItemEffect = BaseFlyItemEffect:New("LetemRollFlyItemEffect")
local this = LetemRollFlyItemEffect
...
function this:CreateTreasureOld(cardId, cellIndex)
    ...
    if treasureItems then
        ...
        if 57 == data.result[1] then --收集到的道具
            local effectName = ""
            if 3 == data.result[2] or 2 == data.result[2] then
                ...
                if groupCells then
                    ...
                    if not isAllSigned then
                        pos = pos / #groupCells
                        log.log("LetemRollFlyItemEffect:CreateTreasure 00 cardId, cellIndex, #groupCells, indexList, pos ", cardId, cellIndex, #groupCells, indexList, pos)
            ...
            elseif data.result[2] == 1 then --单个
            ...
            --if data.result[2] ~= 1 then
                ...
                log.log("LetemRollFlyItemEffect:CreateTreasure 01 cardId, cellIndex, data.result[2] ", cardId, cellIndex, data.result[2])
            --end
            --飞道具
...
function this:CreateTreasure(cardId, cellIndex)
    ...
    if treasureItems then
        local data = Csv.GetData("item", treasureItems.id)
        if 57 == data.result[1] then --收集到的道具
            CalculateBingoMachine.OnDataChange(cardId, cellIndex, data.result[2], 1)
            local effectName = ""
            if 2 == data.result[2] or 3 == data.result[2] then
                local ext = curCell:GetExtInfo()
                local groupCells = ext and ext.groupCells
                if groupCells then
                    --group中的格子是否都被盖章
                    ...
                    log.log("LetemRollFlyItemEffect:CreateTreasure error ", data, ext)
                end
            elseif data.result[2] == 1 then --单个
                Event.Brocast(EventName.Event_Logic_Collect_Item, cardId, cellIndex, data.result[2], 1)
                Event.Brocast(EventName.Event_Sign_Cell_Background, data.result[2], cardId, cellIndex, data.icon, nil, nil)
                log.log("LetemRollFlyItemEffect:CreateTreasure 02 cardId, cellIndex, data.result[2] ", cardId, cellIndex, data.result[2])
...
function this:ShowSkillBingo(cardId, cellIndex)
    ...
    --播放Bingo技能
    BattleEffectCache:GetSkillPrefabFromCache("LetemRollskill1deng", cellData.obj, function(obj)
        fun.set_same_position_with_but_z_zero(obj, cellData.obj)
        self:SameScaleWithCard(obj, cardId)
        UISound.play("xxxkerosene")
        ...
        LuaTimer:SetDelayFunction(1, function()
            table.each(cellPosList, function(cellID, k)
                    ...
                    BattleEffectCache:GetSkillPrefabFromCache("LetemRollskill1dengget", cellData.obj, function()
