...
local MonopolySignEffect = BaseSignEffect:New("MonopolySignEffect")
setmetatable(MonopolySignEffect, BaseSignEffect)
local this = MonopolySignEffect
local itemShowCache = {}

function MonopolySignEffect:RegisterEvent()
    itemShowCache = {}
    -- Event.AddListener(EventName.Event_Sign_Cell_Background, self.CellBgEffect, self)
end

function MonopolySignEffect:UnRegisterEvent()
    -- Event.RemoveListener(EventName.Event_Sign_Cell_Background, self.CellBgEffect, self)
end

function MonopolySignEffect:TriggerSingleBingo(cardId, cellIndex)
    --ModelList.BattleModel:GetCurrModel():GetRoundData(cardId)
    Event.Brocast(EventName.Event_Show_Skill_Bingo, cardId, cellIndex)
end

function MonopolySignEffect:SignCardEffect(cardId, index, cardCell, signType, delay, self_bingo, giftLen)
...
function MonopolySignEffect:SetJokerBgOnSign(ref_temp, cardId, cellIndex)
