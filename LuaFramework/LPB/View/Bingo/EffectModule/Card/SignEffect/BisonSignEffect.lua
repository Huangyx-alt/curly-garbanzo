...
local BisonSignEffect = BaseSignEffect:New("BisonSignEffect")
setmetatable(BisonSignEffect, BaseSignEffect)
local this = BisonSignEffect
local itemShowCache = {}

function BisonSignEffect:RegisterEvent()
    itemShowCache = {}
    Event.AddListener(EventName.Event_Sign_Cell_Background, self.CellBgEffect, self)
end

function BisonSignEffect:UnRegisterEvent()
    Event.RemoveListener(EventName.Event_Sign_Cell_Background, self.CellBgEffect, self)
end

function BisonSignEffect:TriggerSingleBingo(cardId, cellIndex)
    --ModelList.BattleModel:GetCurrModel():GetRoundData(cardId)
    Event.Brocast(EventName.Event_Show_Skill_Bingo, cardId, cellIndex)
end

--格子盖章效果
function BisonSignEffect:SignCardEffect(cardId, index, cardCell, signType, delay, self_bingo, giftLen)
...
--- 格子底部印章效果
function BisonSignEffect:CellBgEffect(treasureType, cardId, cellIndex, indexList, part, pos)
        ...
        if not part then
            if itemShowCache[cardId] and itemShowCache[cardId][groupID] then
                ...
                if indexList and #indexList == 0 then
                    ...
                    --]]
                    log.log("BisonSignEffect:CellBgEffect play in anim 1")
                end
                log.log("BisonSignEffect:CellBgEffect 00", cardId, cellIndex, part)
                return
            end
        else
            if itemShowCache[cardId] and itemShowCache[cardId][groupID] then
                ...
                if indexList and #indexList == 0 then
                    ...
                    anim:Play("in")
                    log.log("BisonSignEffect:CellBgEffect play in anim 2")
                end
                log.log("BisonSignEffect:CellBgEffect 01 不重复创建", cardId, cellIndex, part)
                return
            end
        end
    end
    ...
    if pos then
        ...
        log.log("BisonSignEffect:CellBgEffect 02 setPos", diEffectName, pos, treasureType, cardId, cellIndex, part)
    else
        local cellObj = cellData:GetCellObj()
        pos = cellObj.transform.position
        fun.set_gameobject_pos(diEffect, pos.x, pos.y, pos.z, false)
        log.log("BisonSignEffect:CellBgEffect 03 setPos", diEffectName, treasureType, cardId, cellIndex, part)
    end
...
--- 显示格子点击提示
function BisonSignEffect:ShowNormalCellTip(cell, cardid, cellIndex)
...
--设置新的JokerBg
function BisonSignEffect:SetJokerBgOnSign(ref_temp, cardId, cellIndex)
