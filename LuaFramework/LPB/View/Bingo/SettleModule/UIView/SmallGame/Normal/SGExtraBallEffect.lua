local SGExtraBallEffect = BaseChildView:New()
local this = SGExtraBallEffect
this.__index = this
...
function SGExtraBallEffect:New()
    local o = {}
...
--@param num 格子数字
function SGExtraBallEffect:SetType(cardId, cellIndex, effectIndex, num,isBigIdle)
...
--- 是否是大的idle动画
function SGExtraBallEffect:IsBigIdle()
    return self.isBigIdle
end

--- 是否是大的idle动画
function SGExtraBallEffect:PlayIdle(isBig)
    if isBig then
        self.Anima:Play("idleda",0,0)
        self.isBigIdle =true
        if fun.is_not_null(self.go) then
...
--- 隐藏
function SGExtraBallEffect:SetActive(isActive)
    if self then
...
--播放命中动画
function SGExtraBallEffect:PlayHit()
    if self then
...
function SGExtraBallEffect.RefreshSelfData(item, rank, reward)
    item.text_ranked.text = rank
    item.text_reward.text = fun.formatNum(reward)
end

function SGExtraBallEffect.RefreshOtherData(item, rank)
    item.text_ranked.text = rank
end

function SGExtraBallEffect.RefreshOtherPlayerTrophy(item, tiers)
...
function SGExtraBallEffect.RefreshMyTrophy(item, tiers)
