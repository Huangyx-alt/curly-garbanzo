...
local DrinkingSettleDetailView = BingoSettleDetailView:New('DrinkingSettleDetailView')
local this = DrinkingSettleDetailView
...
function DrinkingSettleDetailView:OnDisable()
    BingoSettleDetailView.OnDisable(self)
    self.newCount = 0
end

function DrinkingSettleDetailView:InitPuzzleShow()

end

function DrinkingSettleDetailView:InitOthers()
    ...
    if gameInfo then
        log.log("道具id错误 id " , gameInfo.chipId)
        log.log("道具id错误 all " , gameInfo)
...
function DrinkingSettleDetailView:StartFinalSequence(cmd)
    --local miniGameCfg = Csv.GetData("new_minigame", this.MiniGamID)
    --if not miniGameCfg then
