...
function GuideState:on_x_update()
    if self._owner.SGMoneyBuildingMainView then
        if self._owner.SGMoneyBuildingMainView:GetCurrentAnimatorStateInfo(0):IsName("idle") then
            self:stop_x_update()
            self:PlayGuideEnter()
        end
    end
...
function GuideState:PlayGuideEnter()
    if not ModelList.GuideModel:IsGuideComplete(91) and ModelList.BattleModel:IsGameing() then
...
function GuideState:LoadAtlasAndShowGuide()
    local guideAtlas = "SGMoneyBuildingGuideAtlas"
