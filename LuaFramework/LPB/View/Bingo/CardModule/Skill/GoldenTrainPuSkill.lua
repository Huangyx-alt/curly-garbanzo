...
local GoldenTrainPuSkill = BaseSkill:New("GoldenTrainPuSkill")
local this = GoldenTrainPuSkill
local private = {}

function GoldenTrainPuSkill:ShowSkill(cardId, cellIndex, powerId, skillId, serverExtraPos)
    if skillId == 138 then
...
-----------------------------------------------------------skill 1-----------------------------------------------------------Begin
function GoldenTrainPuSkill:ShowSkill01(cardId, cellIndex, powerId, skillId, serverExtraPos)
            ...
            table.each(cardsInfo and cardsInfo.beginSkillData, function(data)
                if not extraPos then
                    table.each(data.effect, function(v)
                        if not extraPos then
                            if v.itemId == 2030 and fun.is_include(pos, v.posList) then
                                extraPos = v.extraPos
                            end
                        end
                    end)
    ...
    log.log("GoldenTrainPuSkill:ShowSkill01(cardId, cellIndex, , , serverExtraPos), extraPos", cardId, cellIndex, serverExtraPos, extraPos)
    ----测试用代码
    --if GetTableLength(extraPos) == 0 then
    --    extraPos = { 44, 43 }
    --end
    UISound.play("goldentrainstamper")
    LuaTimer:SetDelayFunction(0, function()
        table.each(extraPos, function(cellID, k)
            ...
            if cellData:IsNotSign() then
                ...
                BattleEffectCache:GetSkillPrefabFromCache("GoldenTrainskill1yinzhang", cellData.obj, function()
