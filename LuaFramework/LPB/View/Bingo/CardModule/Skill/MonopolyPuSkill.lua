...
local MonopolyPuSkill = BaseSkill:New("MonopolyPuSkill")
local this = MonopolyPuSkill
local private = {}

function MonopolyPuSkill:ShowSkill(cardId, cellIndex, powerId, skillId, serverExtraPos)
...
function MonopolyPuSkill:ShowFly(mapObj, cellObj, cardID, targetCellIndex, powerId, cellIndex)
    local targetCell = this.cardView:GetCardCell(tonumber(cardID), targetCellIndex)
    BattleEffectCache:GetSkillPrefabFromCache("Monopolyskill2feiji", nil, function(obj)
        ...
        Anim.smooth_move(obj, retPath, flyTime, delayTime, 1, 8, function()
            ...
            BattleEffectPool:Recycle("Monopolyskill2feiji", obj)
        end)
    end, 0, cardID)
end

--控制起始角度和结束角度
function MonopolyPuSkill:GetOffsetDir(startPos, endPos)
...
function MonopolyPuSkill:GetBeizerList(pathNodes, segmentNum)
    local path = {}
...
---@param p2 table 控制点2
---@param p3 table 终点
function MonopolyPuSkill:CalculateCubicBezierPoint(t, p0, p1, p2, p3)
    local arg1 = 1 - t
