local LetemRollJackpotView = BaseChildView:New()
local this = LetemRollJackpotView
...
function LetemRollJackpotView:New()
    local o = {}
    this.__index = this
    setmetatable(o, this)
    return o
...
function LetemRollJackpotView:Init(jackpotRef, jackpotRuleId, jackPar)
...
function LetemRollJackpotView:OutScreenOpen()
...
function LetemRollJackpotView:Show()
    if this.isShowned then   return   end
    this.isShowned = true
    fun.set_active(this.jackpotRoot, true)
end

function LetemRollJackpotView:OnDisable()
    this.jackpotList = nil
    this.itemList = {}
