local MonopolyJackpotView = BaseChildView:New()
local this = MonopolyJackpotView
...
function MonopolyJackpotView:New()
    local o = {}
    this.__index = this
    setmetatable(o, this)
    return o
...
function MonopolyJackpotView:Init(jackpotRef, jackpotRuleId, jackPar)
...
function MonopolyJackpotView:OutScreenOpen()
...
function MonopolyJackpotView:Show()
    if this.isShowned then   return   end
    this.isShowned = true
    fun.set_active(this.jackpotRoot, false)
end

function MonopolyJackpotView:OnDisable()
    this.jackpotList = nil
    this.itemList = {}
