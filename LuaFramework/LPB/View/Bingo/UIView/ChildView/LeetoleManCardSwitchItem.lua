...
local LeetoleManCardSwitchItem = BaseChildView:New() --Clazz(BaseChildView,"LeetoleManCardSwitchItem")
local this = LeetoleManCardSwitchItem
this.cardPage = {}     --switch 分页1

function LeetoleManCardSwitchItem:New(go)
    self = {}
    setmetatable(self,{__index = LeetoleManCardSwitchItem})
    --self._data = cellData
    return self
end
...
---初始化switch 分页卡牌
function LeetoleManCardSwitchItem:InitMap(go,type)
...
function LeetoleManCardSwitchItem:ShowAll(go,orderIndex)
    local ref = fun.get_component(go,fun.REFER)
    if ref then
        self.content = ref:Get("content")
        self.img_reap = ref:Get("img_reap")
...
function LeetoleManCardSwitchItem:Refresh(pageIndex, newCard,cardId,signInfo,cardObj)
            ...
            for k,v in pairs(newCard) do
                if  i == v.index then
                    ...
                    if IsAutoClick(v.cardId,v.index,cardId,signInfo) then
                        ...
                        BattleEffectCache:GetFreePrefabFromCache("LeetoleMangetxiaoka",items[i].obj,1,function(effObj)
                            if effObj then
...
function LeetoleManCardSwitchItem.OnDispose()
...
return LeetoleManCardSwitchItem