local DragonFortuneCardSwitchItem = BaseChildView:New()
local this = DragonFortuneCardSwitchItem
this.cardPage = {}
this.itemRoots = {}

function DragonFortuneCardSwitchItem:New(go)
    self = {}
    setmetatable(self, { __index = DragonFortuneCardSwitchItem })
    --self._data = cellData
    return self
end

---初始化switch 分页卡牌
function DragonFortuneCardSwitchItem:InitMap(go, type)
...
function DragonFortuneCardSwitchItem:ShowAll(go, orderIndex)
...
function DragonFortuneCardSwitchItem:Refresh(pageIndex, newCard, cardId, signInfo, cardObj)
            ...
            for k, v in pairs(newCard) do
                if i == v.index then
                    ...
                    if IsAutoClick(v.cardId, v.index, cardId, signInfo) then
                        BattleEffectCache:GetFreePrefabFromCache("DragonFortuneGetxiaoka", items[i].obj, 1, function(effObj)
...
---计算每个卡牌上星星的位置
function DragonFortuneCardSwitchItem:InitItemRewardCtrl()
    self.itemGroups = { }
...
function DragonFortuneCardSwitchItem:ShowItemRewardCtrl(pageIndex, cardID)
    --标识pageindex是否有显示
...
function DragonFortuneCardSwitchItem:IsGroupCellAllSigned(cardID, groupID)
...
function DragonFortuneCardSwitchItem.OnDispose()
    this.cardPage = {}
    this.itemRoots = {}
    this:Close()
end

return DragonFortuneCardSwitchItem