local CandyCardSwitchItem = BaseChildView:New()
local this = CandyCardSwitchItem
this.cardPage = {}
this.itemRoots = {}

function CandyCardSwitchItem:New(go)
    self = {}
    setmetatable(self, { __index = CandyCardSwitchItem })
    --self._data = cellData
    return self
end

---初始化switch 分页卡牌
function CandyCardSwitchItem:InitMap(go, type)
...
function CandyCardSwitchItem:ShowAll(go, orderIndex)
    local ref, ItemCtrlTemp = fun.get_component(go, fun.REFER)
    if ref then
        self.content = ref:Get("content")
        self.img_reap = ref:Get("img_reap")
        self.bowl1 = ref:Get("bowl1")
...
function CandyCardSwitchItem:Refresh(pageIndex, newCard, cardId, signInfo, cardObj)
            ...
            for k, v in pairs(newCard) do
                if i == v.index then
                    ...
                    if IsAutoClick(v.cardId, v.index, cardId, signInfo) then
                        ...
                        BattleEffectCache:GetFreePrefabFromCache("CandyGetxiaoka", items[i].obj, 1, function(effObj)
...
---计算每个卡牌上星星的位置
function CandyCardSwitchItem:InitItemRewardCtrl()
    self.itemGroups = { }
...
function CandyCardSwitchItem:ShowItemRewardCtrl(pageIndex, cardID)
    --标识pageindex是否有显示
...
function CandyCardSwitchItem.OnDispose()
