local PiggyBankCardSwitchItem = BaseChildView:New()
local this = PiggyBankCardSwitchItem
this.cardPage = {}
this.itemRoots = {}

function PiggyBankCardSwitchItem:New(go)
    self = {}
    setmetatable(self, { __index = PiggyBankCardSwitchItem })
    --self._data = cellData
    return self
end

---初始化switch 分页卡牌
function PiggyBankCardSwitchItem:InitMap(go, type)
...
function PiggyBankCardSwitchItem:ShowAll(go, orderIndex)
...
function PiggyBankCardSwitchItem:Refresh(pageIndex, newCard, cardId, signInfo, cardObj)
            ...
            for k, v in pairs(newCard) do
                if i == v.index then
                    ...
                    if IsAutoClick(v.cardId, v.index, cardId, signInfo) then
                        BattleEffectCache:GetFreePrefabFromCache("PiggyBankGetxiaoka", items[i].obj, 1, function(effObj)
...
---计算每个卡牌上星星的位置
function PiggyBankCardSwitchItem:InitItemRewardCtrl()
    self.itemGroups = {}
...
function PiggyBankCardSwitchItem:ShowItemRewardCtrl(pageIndex, cardID)
    --标识pageindex是否有显示
...
function PiggyBankCardSwitchItem:IsBroken(cardId, groupId)
...
function PiggyBankCardSwitchItem:IsGroupCellAllSigned(cardID, groupID)
...
function PiggyBankCardSwitchItem.OnDispose()
    this.cardPage = {}
    this.itemRoots = {}
    this:Close()
end

return PiggyBankCardSwitchItem