local ScratchWinnerCardSwitchItem = BaseChildView:New()
local this = ScratchWinnerCardSwitchItem
this.cardPage = {}

function ScratchWinnerCardSwitchItem:New(go)
    self = {}
    setmetatable(self, { __index = ScratchWinnerCardSwitchItem })
    --self._data = cellData
    return self
end
...
---初始化switch 分页卡牌
function ScratchWinnerCardSwitchItem:InitMap(go, type)
...
function ScratchWinnerCardSwitchItem:ShowAll(go, orderIndex)
    local ref = fun.get_component(go, fun.REFER)
    if ref then
        self.content = ref:Get("content")
        self.img_reap = ref:Get("img_reap")
...
function ScratchWinnerCardSwitchItem:Refresh(pageIndex, newCard, cardId, signInfo, cardObj)
            ...
            for k, v in pairs(newCard) do
                if i == v.index then
                    ...
                    if IsAutoClick(v.cardId, v.index, cardId, signInfo) then
                        ...
                        BattleEffectCache:GetFreePrefabFromCache("ScratchWinnergetxiaoka", items[i].obj, 1, function(effObj)
                            if effObj then
...
function ScratchWinnerCardSwitchItem.OnDispose()
    --self._gameObject = nil
    this.cardPage = {}
    this:Close()
end

return ScratchWinnerCardSwitchItem