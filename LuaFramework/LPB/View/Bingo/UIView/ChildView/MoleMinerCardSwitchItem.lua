local MoleMinerCardSwitchItem = BaseChildView:New()
local this = MoleMinerCardSwitchItem
this.cardPage = {}

function MoleMinerCardSwitchItem:New(go)
    self = {}
    setmetatable(self, { __index = MoleMinerCardSwitchItem })
    --self._data = cellData
    return self
end
...
---初始化switch 分页卡牌
function MoleMinerCardSwitchItem:InitMap(go, type)
...
function MoleMinerCardSwitchItem:ShowAll(go, orderIndex)
    local ref = fun.get_component(go, fun.REFER)
    if ref then
        self.content = ref:Get("content")
        self.img_reap = ref:Get("img_reap")
...
function MoleMinerCardSwitchItem:Refresh(pageIndex, newCard, cardId, signInfo, cardObj)
            ...
            for k, v in pairs(newCard) do
                if i == v.index then
                    ...
                    if IsAutoClick(v.cardId, v.index, cardId, signInfo) then
                        ...
                        BattleEffectCache:GetFreePrefabFromCache("MoleMinergetxiaoka", items[i].obj, 1, function(effObj)
                            if effObj then
...
function MoleMinerCardSwitchItem.OnDispose()
    --self._gameObject = nil
    this.cardPage = {}
    this:Close()
end

return MoleMinerCardSwitchItem