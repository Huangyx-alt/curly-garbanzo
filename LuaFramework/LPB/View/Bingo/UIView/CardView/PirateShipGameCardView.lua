...
---@class PirateShipGameCardView : GameCardView
local PirateShipGameCardView = GameCardView:New("PirateShipGameCardView")
local this = PirateShipGameCardView
...
function this:ShowMaxBetEffect(cb)
    if self.model:GetIsMaxBet() then
        local cardCount = self.model:GetCardCount()
        for i = 1, cardCount do
            self:GetCard(i):ShowMaxBetEffect(i)
        end
