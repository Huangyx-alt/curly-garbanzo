...
---@class ChristmasGameCardView : GameCardView
local ChristmasGameCardView = GameCardView:New("ChristmasGameCardView")
local this = ChristmasGameCardView
...
--- 重载激活
function ChristmasGameCardView:OnEnable(bingoView, bingosiRef, is_open_search)
...
function ChristmasGameCardView:InitChristmasData()
    self.cellBgEffectList = {}
end

function ChristmasGameCardView:InitCardBowls()
...
function ChristmasGameCardView:GetWineBowlPos(cardId, bowlType)
    if bowlType == 1 then
...
--- 卡牌盖章
function ChristmasGameCardView:CellSignChange(cardId, cellIndex)
    cardId = tonumber(cardId)
    self:GetCard(cardId):CellSignChange(cardId, cellIndex)
end

function ChristmasGameCardView:GetBingoType(cardId)
    cardId = tonumber(cardId)
    local bingoType = {}
...
--- bingo格子，直接倒满酒杯
function ChristmasGameCardView:IsDrinkMax(cardId, drinkType)
    cardId = tonumber(cardId)
    return bowlCount[cardId][drinkType] >= MaxWaterCount
end

--- 把两张有jackpot的卡移动到后面
function ChristmasGameCardView:JackpotCardMoveBackground(currCardId, moveCardId, currIndex, moveIndex)
...
function ChristmasGameCardView:RegisterExtraEvent()
    Event.AddListener(EventName.Event_View_Collect_Item, self.CellSignChange, self)
    Event.AddListener(EventName.Event_Move_Jackpot_Card_Background, self.JackpotCardMoveBackground, self)
end

function ChristmasGameCardView:UnRegisterExtraEvent()
    Event.RemoveListener(EventName.Event_View_Collect_Item, self.CellSignChange, self)
    Event.RemoveListener(EventName.Event_Move_Jackpot_Card_Background, self.JackpotCardMoveBackground, self)
end

function ChristmasGameCardView:StorageCellBgEffect(cardId, cellIndex, effect)
...
function ChristmasGameCardView:GetCellBgEffect(cardId, cellIndex)
