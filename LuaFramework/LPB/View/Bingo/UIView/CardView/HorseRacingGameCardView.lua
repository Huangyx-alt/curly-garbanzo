...
---@class HorseRacingGameCardView : GameCardView
local HorseRacingGameCardView = GameCardView:New("HorseRacingGameCardView")
local this = HorseRacingGameCardView

local private = {}
private.ItemTypeToFlyTarget = {"HorseRacing_item1", "HorseRacing_item2", "HorseRacing_item3", "HorseRacing_item4", "HorseRacing_item5"}
local bowlCount = {}
--- 记录新bingo的类型
local newBingoType = {}
...
function this:InitCardData()
    ...
    table.each(self.model:GetRoundData(), function(cardData, cardID)
        ...
        for i, v in ipairs(temp1) do
            local groupId = math.floor((v.index + 4) / 5)
...
function this:GmSignTest()
    local cardCount = self.model:GetCardCount()
    log.log("HorseRacingGameCardView:GmSignTest 一键达成jackpot succ", cardCount, startCardId)
