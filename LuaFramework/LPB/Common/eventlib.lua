...
function _M:Connect(handler)
    ...
    assert(type(handler) == "table", "Invalid handler. Expected function got " .. type(handler))
...
function _M:Disconnect(handler,owner)
    assert(self ~= nil and type(self) == "table", "Invalid Event (make sure you're using ':' not '.')")
    ...
    if not handler then
        ...
        for k, v in pairs(self.handlers) do
            if owner then
                if v.handler ==  handler and v.owner == owner then
                    self.handlers[k] = nil
                end
            else
...
function _M:Fire(...)--这里使用coroutine协同程序,会延迟执行，派发事件需要注意可能影响到接下来的代码，出现bug
    assert(self ~= nil and type(self) == "table", "Invalid Event (make sure you're using ':' not '.')")
    ...
    for k, v in pairs(self.handlers) do
        i = i + 1
        spawn(function()
            v.handler(v.owner,unpack(self.args))
            i = i - 1
            if i == 0 then
